...
PYTEST_PATH = str(
    (
        ...
        / "../../connectedhomeip/connectedhomeip/src/python_testing"
    ).resolve()
)
...
def load_test_commands(certification_tests: str, ci_branch: str):
    if not PYTEST_SSID or not PYTEST_PASSPHRASE:
        raise ValueError(
            "CI_GITLAB_PYTEST_SSID and CI_GITLAB_PYTEST_PASSPHRASE must be set as environment variables"
        )
    ...
    common_args = common_args_template.format(
        WIFI_SSID=PYTEST_SSID, WIFI_PASSPHRASE=PYTEST_PASSPHRASE
    )
    ...
    for test_case_name, test_config in test_cases.items():
        ...
        if ci_branch == test_config.get("skip_on_branch", ""):
            ...
            if "test_case" in test_config:
                ...
                command = (
                    f"python3 {script} {common_args} {storage_path} {args}".strip()
                )

            test_commands.append({"name": test_case_name, "command": command})
...
def execute_test_command(full_command, dut: Dut, retry_attempts=2):
    ...
    for attempt in range(retry_attempts):
        ...
        if (
            ("Executed" in results)
            and ("Passed" in results)
            and (results["Executed"] == results["Passed"])
        ):
...
def generate_markdown_results(results_table, chunk_id=None):
    summary_title = (
        f"Python Certification Test Results {chunk_id}" if chunk_id else "Test Results"
    )
    markdown_results = [
        "<!-- Expandable Section -->",
...
def update_mr_description_with_results(markdown_content, chunk_id=None):
    try:
        ...
        updated_description = ResultsFormatter.update_cert_test_results_section(
            description, markdown_content, chunk_id=chunk_id
        )
...
def run_python_certification_tests(
    ...
    command_chunks = {"1": test_commands[:mid_index], "2": test_commands[mid_index:]}
