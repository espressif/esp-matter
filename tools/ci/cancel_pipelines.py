...
def cancel_branch_pipelines(gitlab_api):
    ...
    if response.status_code == 200:
        ...
        for pipeline in running_pipelines:
            ...
            if (
                pipeline_id != int(gitlab_api.ci_pipeline_id)
                and source != "merge_request_event"
            ):
                print(
                    f"Cancelling branch pipeline with pipeline ID: {pipeline_id}, Source:{source}, Ref: {ref}"
                )
                gitlab_api.cancel_pipeline(pipeline_id)
            else:
                print(
                    f"Skipping current branch pipeline with pipeline ID: {gitlab_api.ci_pipeline_id}"
                )
    else:
        ...
        for pipeline in running_pipelines:
            ...
            if merge_request_id_pattern:
                ...
                if (
                    ...
                    and gitlab_api.ci_merge_request_iid
                    == merge_request_id_pattern.group(1)
                ):
                    if pipeline_id == int(gitlab_api.ci_pipeline_id):
                        print(
                            f"Skipping current MR pipeline with pipeline id: {pipeline_id} ref: {ref} source: {source}"
                        )
                    else:
                        print(
                            f"Cancelling MR pipeline with pipeline id: {pipeline_id} ref:{ref} source: {source}"
                        )
                        gitlab_api.cancel_pipeline(pipeline_id)
            else:
                print(
                    f"Skipping pipeline with pipeline ID: {pipeline_id} (ref: {ref}, source: {source})"
                )
