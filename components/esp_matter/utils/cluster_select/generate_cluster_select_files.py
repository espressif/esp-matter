...
def write_file_header(file):
    file.writelines(
        [
            f"# This file is generated by {os.path.basename(__file__)}\n",
...
def get_esp_matter_cluster_dirs(cluster_list):
    cluster_root = os.path.join(
        BASE_PATH, "components/esp_matter/data_model_provider/clusters"
    )
    result = {}
...
def generate_cluster_select_cmake(cluster_list, esp_matter_cluster_dirs, output_dir):
    with open(os.path.join(output_dir, "cluster_dir.cmake"), "w") as cmake_file:
        ...
        for cluster, dirs in cluster_list.items():
            ...
            for dir in dirs:
                cmake_file.write(
                    f'\t\tlist(APPEND temp_list "${{MATTER_SDK_PATH}}/src/app/clusters/{dir}")\n'
                )
...
def main():
    ...
    parser.add_argument(
        "-o",
        "--output-dir",
        default=os.path.join(BASE_PATH, "components/esp_matter/utils/cluster_select"),
    ...
    generate_cluster_select_cmake(
        cluster_list, esp_matter_cluster_dirs, args.output_dir
    )
