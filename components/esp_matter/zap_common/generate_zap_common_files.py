...
def get_args():
    parser = argparse.ArgumentParser(
        description="Script to generate files under zap_common directory",
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            BASE_PATH,
            "connectedhomeip/connectedhomeip/src/app/zap-templates/zcl/data-model/chip",
        ),
    ...
    parser.add_argument(
        "-o",
        "--output-dir",
        default=os.path.join(BASE_PATH, "components/esp_matter/zap_common"),
...
def get_clusters_from_xml_files(xml_files):
    ...
    for xml_file in xml_files:
        ...
        for child in root:
            if (
                child.tag == "cluster"
                and get_cluster_name(child) not in EXCLUDE_CLUSTERS
            ):
                ret.append(child)
    return ret
...
def generate_plugin_application_callbacks_h(xml_files, output_dir):
    with open(
        os.path.join(output_dir, "app/PluginApplicationCallbacks.h"), "w"
    ) as header_file:
...
def write_privileges(file, privileges, interaction_type, object_type):
    file.write(
        get_privileges_array(privileges, "cluster", interaction_type, object_type)
    )
    file.write(
        get_privileges_array(privileges, object_type, interaction_type, object_type)
    )
    file.write(
        get_privileges_array(privileges, "privilege", interaction_type, object_type)
    )
    file.write(
        "////////////////////////////////////////////////////////////////////////////////\n"
    )
