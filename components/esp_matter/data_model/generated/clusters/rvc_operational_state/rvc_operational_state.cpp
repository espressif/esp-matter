...
#include <rvc_operational_state_ids.h>
#include <binding.h>
...
{
    return esp_matter::command::create(cluster, Pause::Id, COMMAND_FLAG_NONE, NULL);
}

command_t *create_resume(cluster_t *cluster)
{
    return esp_matter::command::create(cluster, Resume::Id, COMMAND_FLAG_NONE, NULL);
}
...
command_t *create_go_home(cluster_t *cluster)
{
    return esp_matter::command::create(cluster, GoHome::Id, COMMAND_FLAG_ACCEPTED, NULL);
}
...
{
    cluster_t *cluster = esp_matter::cluster::create(endpoint, rvc_operational_state::Id, flags);
    VerifyOrReturnValue(cluster, NULL, ESP_LOGE(TAG, "Could not create cluster. cluster_id: 0x%08" PRIX32, rvc_operational_state::Id));
    if (flags & CLUSTER_FLAG_SERVER) {
        VerifyOrReturnValue(config != NULL, ABORT_CLUSTER_CREATE(cluster));
        if (config->delegate != nullptr) {
            static const auto delegate_init_cb = RvcOperationalStateDelegateInitCB;
            set_delegate_and_init_callback(cluster, delegate_init_cb, config->delegate);
        }
        static const auto plugin_server_init_cb = CALL_ONCE(MatterRvcOperationalStatePluginServerInitCallback);
        ...
        cluster::set_init_and_shutdown_callbacks(cluster, ESPMatterRvcOperationalStateClusterServerInitCallback,
                                                 ESPMatterRvcOperationalStateClusterServerShutdownCallback);
    }
