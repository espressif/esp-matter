...
{
    return esp_matter::attribute::create(cluster, CurrentMode::Id, ATTRIBUTE_FLAG_NONE, esp_matter_attr_val(value));
}
...
{
    cluster_t *cluster = esp_matter::cluster::create(endpoint, oven_mode::Id, flags);
    VerifyOrReturnValue(cluster, NULL, ESP_LOGE(TAG, "Could not create cluster. cluster_id: 0x%08" PRIX32, oven_mode::Id));
    if (flags & CLUSTER_FLAG_SERVER) {
        VerifyOrReturnValue(config != NULL, ABORT_CLUSTER_CREATE(cluster));
        if (config->delegate != nullptr) {
            static const auto delegate_init_cb = OvenModeDelegateInitCB;
            set_delegate_and_init_callback(cluster, delegate_init_cb, config->delegate);
        }
        static const auto plugin_server_init_cb = CALL_ONCE(MatterOvenModePluginServerInitCallback);
        ...
        cluster::set_init_and_shutdown_callbacks(cluster, ESPMatterOvenModeClusterServerInitCallback,
                                                 ESPMatterOvenModeClusterServerShutdownCallback);
    }
