...
#include <formaldehyde_concentration_measurement_ids.h>
#include <binding.h>
...
static const char *TAG = "formaldehyde_concentration_measurement_cluster";
...
{
    cluster_t *cluster = esp_matter::cluster::create(endpoint, formaldehyde_concentration_measurement::Id, flags);
    VerifyOrReturnValue(cluster, NULL, ESP_LOGE(TAG, "Could not create cluster. cluster_id: 0x%08" PRIX32, formaldehyde_concentration_measurement::Id));
    if (flags & CLUSTER_FLAG_SERVER) {
        VerifyOrReturnValue(config != NULL, ABORT_CLUSTER_CREATE(cluster));
        static const auto plugin_server_init_cb = CALL_ONCE(MatterFormaldehydeConcentrationMeasurementPluginServerInitCallback);
