...
#include <laundry_washer_mode_ids.h>
#include <binding.h>
...
{
    cluster_t *cluster = esp_matter::cluster::create(endpoint, laundry_washer_mode::Id, flags);
    VerifyOrReturnValue(cluster, NULL, ESP_LOGE(TAG, "Could not create cluster. cluster_id: 0x%08" PRIX32, laundry_washer_mode::Id));
    if (flags & CLUSTER_FLAG_SERVER) {
        ...
        static const auto plugin_server_init_cb = CALL_ONCE(MatterLaundryWasherModePluginServerInitCallback);
        ...
        cluster::set_init_and_shutdown_callbacks(cluster, ESPMatterLaundryWasherModeClusterServerInitCallback,
                                                 ESPMatterLaundryWasherModeClusterServerShutdownCallback);
    }
