...
{
    return esp_matter::command::create(cluster, SendKey::Id, COMMAND_FLAG_ACCEPTED, esp_matter_command_callback_send_key);
}

command_t *create_send_key_response(cluster_t *cluster)
{
    return esp_matter::command::create(cluster, SendKeyResponse::Id, COMMAND_FLAG_GENERATED, NULL);
}
...
{
    cluster_t *cluster = esp_matter::cluster::create(endpoint, keypad_input::Id, flags);
    VerifyOrReturnValue(cluster, NULL, ESP_LOGE(TAG, "Could not create cluster. cluster_id: 0x%08" PRIX32, keypad_input::Id));
    if (flags & CLUSTER_FLAG_SERVER) {
        ...
        static const auto plugin_server_init_cb = CALL_ONCE(MatterKeypadInputPluginServerInitCallback);
