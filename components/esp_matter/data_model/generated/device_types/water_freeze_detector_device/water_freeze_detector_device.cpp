...
endpoint_t *create(node_t *node, config_t *config, uint8_t flags, void *priv_data)
{
    ...
    VerifyOrReturnValue(endpoint != nullptr, NULL, ESP_LOGE("water_freeze_detector", "Failed to create endpoint"));

    cluster_t *descriptor_cluster = descriptor::create(endpoint, &(config->descriptor), CLUSTER_FLAG_SERVER);
    VerifyOrReturnValue(descriptor_cluster != nullptr, NULL, ESP_LOGE("water_freeze_detector", "Failed to create descriptor cluster"));

    VerifyOrReturnValue(add(endpoint, config) == ESP_OK, NULL, ESP_LOGE("water_freeze_detector", "Failed to add device type"));
    return endpoint;
}
