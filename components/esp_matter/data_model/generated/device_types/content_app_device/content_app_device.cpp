...
{
    return ESP_MATTER_CONTENT_APP_DEVICE_TYPE_ID;
}

uint8_t get_device_type_version()
{
    return ESP_MATTER_CONTENT_APP_DEVICE_TYPE_VERSION;
}
...
esp_err_t add(endpoint_t *endpoint, config_t *config)
{
    ...
    cluster::application_launcher::create(endpoint, &(config->application_launcher), CLUSTER_FLAG_SERVER);
    cluster::application_basic::create(endpoint, &(config->application_basic), CLUSTER_FLAG_SERVER);
    return ESP_OK;
}
