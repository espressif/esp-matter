...
endpoint_t *create(node_t *node, config_t *config, uint8_t flags, void *priv_data)
{
    ...
    VerifyOrReturnValue(endpoint != nullptr, NULL, ESP_LOGE("water_leak_detector", "Failed to create endpoint"));

    cluster_t *descriptor_cluster = descriptor::create(endpoint, &(config->descriptor), CLUSTER_FLAG_SERVER);
    VerifyOrReturnValue(descriptor_cluster != nullptr, NULL, ESP_LOGE("water_leak_detector", "Failed to create descriptor cluster"));

    VerifyOrReturnValue(add(endpoint, config) == ESP_OK, NULL, ESP_LOGE("water_leak_detector", "Failed to add device type"));
    return endpoint;
}
...
esp_err_t add(endpoint_t *endpoint, config_t *config)
{
    ...
    VerifyOrReturnValue(boolean_state != NULL, ESP_FAIL, ESP_LOGE("water_leak_detector", "Failed to create cluster: boolean_state"));
