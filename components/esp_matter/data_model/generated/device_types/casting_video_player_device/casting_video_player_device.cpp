...
{
    return ESP_MATTER_CASTING_VIDEO_PLAYER_DEVICE_TYPE_ID;
}

uint8_t get_device_type_version()
{
    return ESP_MATTER_CASTING_VIDEO_PLAYER_DEVICE_TYPE_VERSION;
}
...
esp_err_t add(endpoint_t *endpoint, config_t *config)
{
    ...
    cluster::media_playback::create(endpoint, &(config->media_playback), CLUSTER_FLAG_SERVER);
    cluster::keypad_input::create(endpoint, &(config->keypad_input), CLUSTER_FLAG_SERVER);
    cluster::content_launcher::create(endpoint, &(config->content_launcher), CLUSTER_FLAG_SERVER);
    return ESP_OK;
}
