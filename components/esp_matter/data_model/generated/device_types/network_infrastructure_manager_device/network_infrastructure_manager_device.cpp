...
endpoint_t *create(node_t *node, config_t *config, uint8_t flags, void *priv_data)
{
    ...
    VerifyOrReturnValue(endpoint != nullptr, NULL, ESP_LOGE("network_infrastructure_manager", "Failed to create endpoint"));

    cluster_t *descriptor_cluster = descriptor::create(endpoint, &(config->descriptor), CLUSTER_FLAG_SERVER);
    VerifyOrReturnValue(descriptor_cluster != nullptr, NULL, ESP_LOGE("network_infrastructure_manager", "Failed to create descriptor cluster"));

    VerifyOrReturnValue(add(endpoint, config) == ESP_OK, NULL, ESP_LOGE("network_infrastructure_manager", "Failed to add device type"));
    return endpoint;
}
...
esp_err_t add(endpoint_t *endpoint, config_t *config)
{
    ...
    cluster::wi_fi_network_management::create(endpoint, &(config->wi_fi_network_management), CLUSTER_FLAG_SERVER);
    cluster::thread_border_router_management::create(endpoint, &(config->thread_border_router_management), CLUSTER_FLAG_SERVER);
    cluster::thread_network_directory::create(endpoint, &(config->thread_network_directory), CLUSTER_FLAG_SERVER);
    return ESP_OK;
}
