...
esp_err_t list_registered_icd()
{
    ...
    while (iter->Next(info)) {
        ...
        LogErrorOnFailure(Encoding::BytesToHex(info.aes_key_handle.As<Crypto::Symmetric128BitsKeyByteArray>(), Crypto::kAES_CCM128_Key_Length,
                                               icd_aes_key_hex, sizeof(icd_aes_key_hex), Encoding::HexFlags::kNullTerminate));
        ESP_LOGI(TAG, "  | aes key:  %60s                               |", icd_aes_key_hex);
        LogErrorOnFailure(Encoding::BytesToHex(info.hmac_key_handle.As<Crypto::Symmetric128BitsKeyByteArray>(), Crypto::kHMAC_CCM128_Key_Length,
