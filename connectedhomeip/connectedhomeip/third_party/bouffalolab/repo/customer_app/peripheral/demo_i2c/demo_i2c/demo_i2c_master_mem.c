...
void demo_hosal_i2c_master_mem(void)
{
    int ret     = -1;
    int i       = 0;
    ...
    for (i = 0; i < 32; i++) {
        ...
        printf(" %X", i2c_data_rxbuf[i]);
    }
    printf("\r\n");
    for (i = 0; i < 32; i++) {
        if (i2c_data_txbuf[i] != i2c_data_rxbuf[i]) {
            hosal_i2c_finalize(&i2c0);
            blog_error("hosal i2c read write failed!\r\n");
            return;
        }
    }
