...
/*-----------------------------------------------------------------------*/
#if (JD_API_TEST == 1)
static float idct_test_us, huff_test_us, bit_test_us = 0;
static int bit_test_count, huff_test_count, idct_test_count = 0;
#elif (JD_API_TEST == 2)
static float mcu_test_us, out_test_us = 0;
static int mcu_test_count, out_test_count = 0;
#endif
...
#if (JD_API_TEST == 1)
	end_us = bl_timer_now_us();

	bit_test_us += end_us - start_us;
	bit_test_count++;
#endif
	return (int)v;
}
...
)
{
...
#if (JD_API_TEST == 1)
	printf("Ext huff data us: %02.4f us. count %d.\r\n", (float)(huff_test_us / huff_test_count), huff_test_count);
	printf("Inverse DCT us: %02.4f us. count %d.\r\n", (float)(idct_test_us / idct_test_count), idct_test_count);
	printf("Ext bit us: %02.4f us. count %d.\r\n", (float)(bit_test_us / bit_test_count), bit_test_count);
	...
	bit_test_count = 0;
#elif (JD_API_TEST == 2)
	printf("mcu_load us: %02.4f us. count %d.\r\n", (float)(mcu_test_us / mcu_test_count), mcu_test_count);
	printf("mcu_output us: %02.4f us. count %d.\r\n", (float)(out_test_us / out_test_count), out_test_count);
	mcu_test_us = 0;
	mcu_test_count = 0;
	out_test_us = 0;
	out_test_count = 0;
