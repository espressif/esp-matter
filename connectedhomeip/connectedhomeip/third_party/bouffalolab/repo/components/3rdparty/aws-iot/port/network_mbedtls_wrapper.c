...
#define BFL_LOGE(TAG, format, ...) printf("ERROR " format "\r\n", ##__VA_ARGS__)
#define BFL_LOGW(TAG, format, ...) printf("WARNN " format "\r\n", ##__VA_ARGS__)
...
static int _iot_tls_verify_cert(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags) 
{
    ...
    BFL_LOGD(TAG, "Verify requested for (Depth %d):", depth);
    mbedtls_x509_crt_info(buf, sizeof(buf) - 1, "", crt);
    BFL_LOGD(TAG, "%s", buf);

    if((*flags) == 0) {
        BFL_LOGD(TAG, "  This certificate has no flags");
    } else {
        BFL_LOGD(TAG, "Verify result:%s", buf);
    }
...
IoT_Error_t iot_tls_init(Network *pNetwork, const char *pRootCALocation, const char *pDeviceCertLocation,
    ...
    uint16_t destinationPort, uint32_t timeout_ms, bool ServerVerificationFlag) 
{
    _iot_tls_set_connect_params(pNetwork, pRootCALocation,
            pDeviceCertLocation, pDevicePrivateKeyLocation,
            pDestinationURL, destinationPort,
...
#if 0
static void aws_debug( void *ctx, int level, const char *file, int line, const char *str )
{
...
IoT_Error_t iot_tls_connect(Network *pNetwork, TLSConnectParams *params) 
{
    ...
    BFL_LOGD(TAG, "Seeding the random number generator...");
    ...
    if ((ret = mbedtls_ctr_drbg_seed(&(tlsDataParams->ctr_drbg), mbedtls_entropy_func, &(tlsDataParams->entropy), (const unsigned char *) TAG, strlen(TAG))) != 0) {
        BFL_LOGE(TAG, "failed! mbedtls_ctr_drbg_seed returned -0x%x", -ret);
    ...
    */
    if (pNetwork->tlsConnectParams.pRootCALocation[0] == '/') {
        BFL_LOGD(TAG, "Loading CA root certificate from file ...");
        ret = mbedtls_x509_crt_parse_file(&(tlsDataParams->cacert), pNetwork->tlsConnectParams.pRootCALocation);
    } else {
        BFL_LOGD(TAG, "Loading embedded CA root certificate ...");
    ...
    if (ret < 0) {
        BFL_LOGE(TAG, "failed!  mbedtls_x509_crt_parse returned -0x%x while parsing root cert", -ret);
        return NETWORK_X509_ROOT_CRT_PARSE_ERROR;
    }
    BFL_LOGD(TAG, "ok (%d skipped)", ret);

    /* Load client certificate... */
    if (pNetwork->tlsConnectParams.pDeviceCertLocation[0] == '/') {
        BFL_LOGD(TAG, "Loading client cert from file...");
        ret = mbedtls_x509_crt_parse_file(&(tlsDataParams->clicert), pNetwork->tlsConnectParams.pDeviceCertLocation);
    } else {
        BFL_LOGD(TAG, "Loading embedded client certificate...");
    ...
    if(ret != 0) {
        BFL_LOGE(TAG, "failed!  mbedtls_x509_crt_parse returned -0x%x while parsing device cert", -ret);
    ...
    BFL_LOGD(TAG, "Connecting to %s/%s...", pNetwork->tlsConnectParams.pDestinationURL, portBuffer);
    if ((ret = mbedtls_net_connect(&(tlsDataParams->server_fd), pNetwork->tlsConnectParams.pDestinationURL, portBuffer, MBEDTLS_NET_PROTO_TCP)) != 0) {
        BFL_LOGE(TAG, "failed! mbedtls_net_connect returned -0x%x", -ret);
        switch (ret) {
    ...
    if (ret != 0) {
        BFL_LOGE(TAG, "failed! net_set_(non)block() returned -0x%x", -ret);
        return SSL_CONNECTION_ERROR;
    }
    BFL_LOGD(TAG, "ok");
    BFL_LOGD(TAG, "Setting up the SSL/TLS structure...");
    if((ret = mbedtls_ssl_config_defaults(&(tlsDataParams->conf), MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT)) != 0) {
        BFL_LOGE(TAG, "failed! mbedtls_ssl_config_defaults returned -0x%x", -ret);
        return SSL_CONNECTION_ERROR;
    }
    ...
    if (ret != 0) {
        BFL_LOGE(TAG, "failed! mbedtls_ssl_conf_own_cert returned %d", ret);
        return SSL_CONNECTION_ERROR;
    }
    ...
    if (pNetwork->tlsConnectParams.DestinationPort == 443) {
        ...
        if ((ret = mbedtls_ssl_conf_alpn_protocols(&(tlsDataParams->conf), alpnProtocols)) != 0) {
            BFL_LOGE(TAG, "failed! mbedtls_ssl_conf_alpn_protocols returned -0x%x", -ret);
            return SSL_CONNECTION_ERROR;
        }
    }
    ...
    if ((ret = mbedtls_ssl_setup(&(tlsDataParams->ssl), &(tlsDataParams->conf))) != 0) {
        BFL_LOGE(TAG, "failed! mbedtls_ssl_setup returned -0x%x", -ret);
        return SSL_CONNECTION_ERROR;
    }
    ...
    while ((ret = mbedtls_ssl_handshake(&(tlsDataParams->ssl))) != 0) {
        if(ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
            BFL_LOGE(TAG, "failed! mbedtls_ssl_handshake returned -0x%x", -ret);
            if(ret == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED) {
                BFL_LOGE(TAG, "    Unable to verify the server's certificate. ");
            }
            return SSL_CONNECTION_ERROR;
        }
    }

    BFL_LOGD(TAG, "ok [ Protocol is %s ]    [ Ciphersuite is %s ]", mbedtls_ssl_get_version(&(tlsDataParams->ssl)), mbedtls_ssl_get_ciphersuite(&(tlsDataParams->ssl)));
    if ((ret = mbedtls_ssl_get_record_expansion(&(tlsDataParams->ssl))) >= 0) {
        BFL_LOGD(TAG, " [ Record expansion is %d ]", ret);
    } else {
        BFL_LOGD(TAG, " [ Record expansion is unknown (compression) ]");
    }

    BFL_LOGD(TAG, "Verifying peer X.509 certificate...");
    if (pNetwork->tlsConnectParams.ServerVerificationFlag == true) {
        if ((tlsDataParams->flags = mbedtls_ssl_get_verify_result(&(tlsDataParams->ssl))) != 0) {
            BFL_LOGE(TAG, "failed");
            mbedtls_x509_crt_verify_info(info_buf, sizeof(info_buf), "  ! ", tlsDataParams->flags);
            BFL_LOGE(TAG, "%s", info_buf);
...
IoT_Error_t iot_tls_write(Network *pNetwork, unsigned char *pMsg, size_t len, Timer *timer, size_t *written_len) 
{
    ...
    for (written_so_far = 0, frags = 0; written_so_far < len && !has_timer_expired(timer); written_so_far += ret, frags++) {
        while (!has_timer_expired(timer) && (ret = mbedtls_ssl_write(&(tlsDataParams->ssl), pMsg + written_so_far, len - written_so_far)) <= 0) {
            if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
                BFL_LOGE(TAG, "failed! mbedtls_ssl_write returned -0x%x", -ret);
