...
// TODO: perhaps sync with FM config
...
static void wpa_auth_add_sm(struct wpa_state_machine *sm)
{
    if (sm) {
        ...
        for (i=0; i<WPA_SM_MAX_INDEX; i++) {
            ...
            sm->index = i;
            wpa_printf( MSG_DEBUG, "add sm, index=%d bitmap=%lx\n", i, s_sm_valid_bitmap);
            return;
        }
    }
}
...
static void wpa_auth_del_sm(struct wpa_state_machine *sm)
{
    if (sm && (sm->index < WPA_SM_MAX_INDEX)) {
        ...
        wpa_printf( MSG_DEBUG, "del sm, index=%lu bitmap=%lx\n", sm->index, s_sm_valid_bitmap);
    }
}
...
static inline int
wpa_auth_send_eapol(struct wpa_authenticator *wpa_auth, uint8_t sta_idx, const u8 *addr,
            const u8 *data, size_t data_len, int encrypt)
{
    ...
    wpa_sendto_wrapper(true, buffer, sizeof(struct l2_ethhdr) + data_len, NULL);
    os_free(buffer);
    return 0;
}
              ...
              int keyidx, int encr, int force_version)
{
    ...
    wpa_auth_send_eapol(wpa_auth, sm->sta_idx, sm->addr, (u8 *) hdr, len, sm->pairwise_set);
    os_free(hdr);
}
                       ...
                       struct wpa_group *group)
{
    ...
    if (wpa_auth->conf.ieee80211w != NO_MGMT_FRAME_PROTECTION &&
        ...
        ret = -1;
#endif /* CONFIG_IEEE80211W */
#endif /* 11W not supported in AP mode */
...
bool wpa_ap_join(void **sm, uint8_t *mac, uint8_t *wpa_ie, uint8_t wpa_ie_len)
{
    ...
    if (hapd) {
        if (hapd->wpa_auth->conf.wpa) {
            ...
            if (wpa_validate_wpa_ie(hapd->wpa_auth, wpa_sm_new, wpa_ie, wpa_ie_len)) {
