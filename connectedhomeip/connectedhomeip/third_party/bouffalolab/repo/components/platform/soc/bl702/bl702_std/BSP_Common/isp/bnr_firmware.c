...
typedef struct
{
    uint16_t bnr_weight;
    uint16_t lpf_weight;
    ISP_BNR_THRESHOLD_TYPE bnr_thr;
    ISP_BNR_FALLBACK_TYPE bnr_fb;
} bnr_reg_internal_t;
...
{
    ISP_Set_BNR_Weight(bnr_reg.bnr_weight);
    ISP_Set_BNR_LPF_Weight(bnr_reg.lpf_weight);
    ISP_Set_BNR_Threshold(&(bnr_reg.bnr_thr));
    ISP_Set_BNR_Fallback(&(bnr_reg.bnr_fb));
}

void bnr_control_debug()
{
    ISP_Set_BNR_State(ENABLE);
