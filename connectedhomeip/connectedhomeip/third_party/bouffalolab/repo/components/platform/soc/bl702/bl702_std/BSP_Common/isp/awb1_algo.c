    ...
    for (i = 0, j = VERT_NUM - 1; i < VERT_NUM; j = i++) {
        if (((vert_array[i].verty >= testy) != (vert_array[j].verty >= testy)) &&
            (testx <= (vert_array[j].vertx - vert_array[i].vertx) * (testy - vert_array[i].verty) /
                              (vert_array[j].verty - vert_array[i].verty) +
                          vert_array[i].vertx)) {
            c = !c;
        }
    }
...
static int awb1_stats_enable(aaa_stats_cfg_t *stats)
{
    ...
    awb_stats_conf.roi_start_x = (awb_stats_conf.img_width - AWB_STATS_GRID_NUM_X * awb_stats_conf.grid_width) / 2;
    awb_stats_conf.roi_start_y = (awb_stats_conf.img_height - AWB_STATS_GRID_NUM_Y * awb_stats_conf.grid_height) / 2;
...
static int awb1_advanced(awb_obj_t *awb_obj)
{
    ...
    awb_obj->awb_info.color_temp = (int)(awb_obj->awb_speed * target_ct + (16 - awb_obj->awb_speed) * awb_obj->awb_info.color_temp) / 16;
