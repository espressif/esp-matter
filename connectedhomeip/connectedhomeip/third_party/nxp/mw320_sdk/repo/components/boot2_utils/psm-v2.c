...
#define offset_to_address(__psm__, __offset__) (__psm__->start_address + __offset__)

#define align_to_block_offset(__psm__, __offset__) (__offset__ & ~(__psm__->block_size - 1))

#define is_offset_block_aligned(__psm__, __offset__) (!(__offset__ & (__psm__->block_size - 1)))

#define object_size(__pobj__) (sizeof(psm_object_t) + (__pobj__)->name_len + (__pobj__)->data_len)

#define psm_is_data_len_sane(__psm__, __len__) \
    ((__len__ < OBJECT_TYPE_SMALL_BINARY_MAX_SIZE) && (__len__ < (part_size(psm) / 2)))

#define metadata_size(__pobj__) (sizeof(psm_object_t) + (__pobj__)->name_len)

#define is_object_active(__pobj__)    ((__pobj__)->flags & PSM_OBJ_INACTIVE)
#define is_object_indexable(__pobj__) (!((__pobj__)->flags & PSM_OBJ_INDEX_REQ))
#define is_object_cacheable(__pobj__) (!((__pobj__)->flags & PSM_OBJ_CACHE_REQ))

#ifdef CONFIG_SECURE_PSM
#define get_object_secure_bits(__pobj__) ((__pobj__)->flags & PSM_OBJ_SECURE_MASK)

#define set_object_secure_bits(__pobj__, bitval)                                     \
    do                                                                               \
    {                                                                                \
        (__pobj__)->flags = (((__pobj__)->flags & ~PSM_OBJ_SECURE_MASK) | (bitval)); \
    } while (0)
...
#define set_object_secure(__pobj__) set_object_secure_bits((__pobj__), PSM_OBJ_SECURE)

#define clear_object_secure(__pobj__) set_object_secure_bits((__pobj__), PSM_OBJ_UNSECURE)
...
void psm_dump_hex(const char *buf, uint32_t size)
{
    ...
    for (index = 0; index < size; index++)
    {
        if (isprint((unsigned char)buf[index]))
            wmprintf("%2x-(%c) ", (unsigned char)buf[index], buf[index]);
        else
...
static uint32_t psm_calc_metadata_crc32(psm_object_t *pobj, uint32_t partial_crc32)
{
    /* Backup */
...
static int psm_erase_flash(psm_t *psm, uint32_t offset, uint32_t size)
{
    ...
    int rv           = mflash_drv_erase(address, size);
    ...
    if (g_psm_event_cb)
        g_psm_event_cb(PSM_EVENT_ERASE, (void *)((unsigned long)address), (void *)((unsigned long)size));
...
int psm_read_flash(psm_t *psm, void *buffer, uint32_t size, uint32_t offset)
{
    ...
    int rv = mflash_drv_read(address, buffer, size);
    if (rv != kStatus_Success)
    {
    ...
    if (g_psm_event_cb)
        g_psm_event_cb(PSM_EVENT_READ, (void *)((unsigned long)address), (void *)((unsigned long)size));

    return rv;
}

static int psm_write_verify(psm_t *psm, const uint8_t *reference_buf, uint32_t size, uint32_t offset)
{
    psm_entry_i();
    int rv;
    uint32_t read_size;
    ...
    for (index = 0; index < size; index += read_size)
    {
        ...
        if (rv != WM_SUCCESS)
        {
            psm_d("%s: psm_read_flash %x: %d failed", __func__, offset + index, read_size);
            return rv;
        }

        if (memcmp(&reference_buf[index], buf, read_size))
        {
            psm_d("%s: verification failed @ offset: %x: %d", __func__, offset + index, read_size);
            return -WM_FAIL;
        }
    }
 ...
 * from the same flash.
 */
static int psm_write_flash_same_device_rw(psm_t *psm, const void *buffer, uint32_t size, uint32_t address)
{
    int rv                = -WM_FAIL;
    ...
    {
        int chunk_size = remaining_size > tmpbuf_size ? tmpbuf_size : remaining_size;
        memcpy(tmpbuf, buffer, chunk_size);
        rv = mflash_drv_write(address, tmpbuf, chunk_size);
        if (rv != kStatus_Success)
        {
            psm_d("Unable to write to flash: %x: %d", address, size);
            break;
        }
 ...
 * buffer.
 */
static int psm_write_flash(psm_t *psm, const void *buffer, uint32_t size, uint32_t offset, bool verify)
{
    psm_ll("%s: %x, %d", __func__, offset, size);
    ...
    rv = mflash_drv_write(address, (void *)buffer, size);
    if (rv != kStatus_Success)
    {
        if (rv == -WM_E_FAULT)
    ...
    if (g_psm_event_cb)
        g_psm_event_cb(PSM_EVENT_WRITE, (void *)((unsigned long)address), (void *)((unsigned long)size));
...
static int write_flag_bit(psm_t *psm, uint32_t flag_offset, uint8_t flag)
{
    ...
    return psm_write_flash(psm, &inv_flag, sizeof(uint8_t), flag_offset, PSM_WRITE_NO_VERIFY);
}
...
static int read_swap_descriptor(psm_t *psm, psm_swap_desc_t *swap_desc)
{
    ...
    int rv          = psm_read_flash(psm, swap_desc, sizeof(psm_swap_desc_t), offset);
    if (rv != WM_SUCCESS)
    {
...
static int write_swap_descriptor(psm_t *psm, const psm_swap_desc_t *swap_desc, bool write_swap_offset)
{
    uint32_t offset = part_size(psm) - sizeof(psm_swap_desc_t);
    if (write_swap_offset)
    {
        psm_entry_i("swap_offset: 0x%x", swap_desc->swap_start_offset);
        int rv = psm_write_flash(psm, &swap_desc->swap_start_offset, sizeof(swap_desc->swap_start_offset) * 2, offset,
                                 PSM_WRITE_VERIFY);
        if (rv != WM_SUCCESS)
        {
    ...
    else
    {
        ...
        int rv = psm_write_flash(psm, &swap_desc->checkpoint_marker, sizeof(swap_desc->checkpoint_marker) * 2, offset,
                                 PSM_WRITE_VERIFY);
        if (rv != WM_SUCCESS)
        {
...
static int invalidate_swap_descriptor(psm_t *psm)
{
    ...
    uint32_t offset = part_size(psm) - sizeof(psm_swap_desc_t);
    int rv          = psm_write_flash(psm, &swap_desc, sizeof(psm_swap_desc_t), offset, PSM_WRITE_VERIFY);
    if (rv != WM_SUCCESS)
    {
...
{
    return align_to_block_offset(psm, (part_size(psm) - sizeof(psm_swap_desc_t) - psm->total_active_object_size));
}
 ...
 */
static int psm_calc_name_hash(
    psm_t *psm, psm_object_t *pobj, uint32_t flash_offset, uint32_t *name_hash, uint8_t *name_buf)
{
...
static void psm_add_to_index(psm_t *psm, psm_object_t *pobj, uint32_t flash_offset)
{
    psm_i("Indexing obj_id: %d", pobj->obj_id);
    if (!psm->index)
    {
        psm_i("Allocating index");
        uint32_t index_size = sizeof(psm_index_t) * PSM_DEFAULT_INDEX_ELEMENTS;
    ...
    if (psm->index_elements == psm->index_max_elements)
    {
        ...
        uint32_t current_index_size = sizeof(psm_index_t) * psm->index_max_elements;
        uint32_t new_index_size     = current_index_size + (sizeof(psm_index_t) * PSM_DEFAULT_INDEX_ELEMENTS);
        psm_index_t *new_index      = os_mem_alloc(new_index_size);

        if (!new_index)
        {
            psm_d("New index allocation failed: %d", new_index_size);
            return;
        }
        memcpy(new_index, psm->index, current_index_size);
        os_mem_free(psm->index);

        /* Zero out new allocated area */
        memset(&new_index[psm->index_max_elements], 0x00, (sizeof(psm_index_t) * PSM_DEFAULT_INDEX_ELEMENTS));
        psm->index = new_index;
        psm->index_max_elements += PSM_DEFAULT_INDEX_ELEMENTS;
    }
    ...
    {
        uint32_t cache_size = sizeof(psm_cache_t) + pobj->name_len + pobj->data_len;
    ...
    int rv = psm_calc_name_hash(psm, pobj, flash_offset, &name_hash, name_buf);
    if (rv != WM_SUCCESS)
    {
    ...
    psm_i("Object %d is also cacheable. Caching name and value (size: %d)", pobj->obj_id, cache->data_len);
    /*
    ...
    if (cache->data_len != 0)
    {
        rv = psm_read_flash(psm, data_buf, cache->data_len, flash_offset + metadata_size(pobj));
        if (rv != WM_SUCCESS)
        {
        ...
        if (part_encrypted(psm))
        {
            ...
            rv = decrypt_buf(psm, name_buf, psm->psm_name_enc_buf, cache->name_len);
            if (rv != WM_SUCCESS)
            {
                os_mem_free(cache);
                memset(index, 0x00, sizeof(psm_index_t));
            ...
            rv = decrypt_buf(psm, data_buf, data_buf, cache->data_len);
            if (rv != WM_SUCCESS)
            {
                psm_d("dec-i fail");
                os_mem_free(cache);
 ...
 */
static void handle_double_active_object(psm_t *psm, uint32_t object_offset)
{
    /*
    ...
    (void)psm_write_flash(psm, &invalid_obj, sizeof(invalid_obj), object_offset, false);
}
...
static int psm_part_scan(psm_t *psm)
{
    ...
    while (offset < limit)
    {
        psm_isc("offset: %p (%p) limit: %p (%p) ", (void *)offset, (void *)offset_to_address(psm, offset),
                (void *)limit, (void *)offset_to_address(psm, limit));
        rv1 = psm_read_flash(psm, &pobj, sizeof(psm_object_t), offset);
        if (rv1 != WM_SUCCESS)
        {
        ...
        {
            psm_w(
                "scan: Invalid obj (%x) at the end of"
                " data area off: %p addr: %p",
                pobj.object_type, (void *)((long)offset), (void *)((long)offset_to_address(psm, offset)));
            rv = -WM_FAIL;
            break;
        }
         ...
         * status mismatch
         */
        if ((part_encrypted(psm) && !is_object_secure(&pobj)) || (!part_encrypted(psm) && is_object_secure(&pobj)))
        {
            /*
             ...
             */
            psm_d(
                "PSM secure status mismatch found during"
                " scan");
            rv = -WM_FAIL;
            offset += object_size(&pobj);
        ...
        psm_isc("Obj: Name Len: %d  Data Len: %d Active: %d", pobj.name_len, pobj.data_len, is_object_active(&pobj));
    ...
    psm_isc("Scan complete: Data Free Offset: %p Phy. Address: %p", offset, offset_to_address(psm, offset));
 ...
 *   populated.
 */
static int psm_copy_object(
    psm_t *psm, psm_object_t *pobj, uint32_t source_offset, uint32_t dest_offset, objectid_t new_objid)
{
    psm_entry_i("obj-id: 0x%x len: %d source: 0x%x dest: 0x%x", pobj->obj_id, pobj->data_len, source_offset,
                dest_offset);
    ...
    if (g_psm_event_cb)
        g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_10, (void *)PSM_COMPACTION_MAX);

    int rv = psm_write_flash(psm, pobj, sizeof(psm_object_t), dest_offset, PSM_WRITE_NO_VERIFY);
    if (rv != WM_SUCCESS)
    {
    ...
    for (index = 0; index < copy_size; index += read_size)
    {
        ...
        rv = psm_write_flash(psm, buf, read_size, dest_offset, PSM_WRITE_NO_VERIFY);
        if (rv != WM_SUCCESS)
        {
    ...
    psm_ed("src_ref_crc32: 0x%x src_crc32: 0x%x dest_crc32: 0x%x", src_ref_crc32, src_crc32, dest_crc32);
    /*
    ...
    if (src_crc32 == src_ref_crc32)
    {
         ...
         */
        uint32_t crc32_offset = dest_object_start_offset + offsetof(psm_object_t, crc32);
        rv = psm_write_flash(psm, &dest_crc32, sizeof(dest_crc32), crc32_offset, PSM_WRITE_NO_VERIFY);
        if (rv != WM_SUCCESS)
        {
     ...
     */
    psm_d("Object: 0x%x has failed CRC during copy. Marking inactive", pobj->obj_id);
    uint32_t flag_offset = dest_object_start_offset + offsetof(psm_object_t, flags);
    ...
    psm_t *psm, psm_object_t *pobj, uint32_t source_offset, uint32_t dest_offset, objectid_t new_objid)
{
     ...
     */
    psm_d("Object: 0x%x has failed CRC during upgrade. Marking inactive", pobj->obj_id);
    uint32_t flag_offset = dest_object_start_offset + offsetof(psm_object_t, flags);
    rv                   = write_flag_bit(psm, flag_offset, PSM_OBJ_INACTIVE);
    if (rv != WM_SUCCESS)
    {
                                ...
                                uint32_t dest_limit)
{
    ...
    while (source_offset < source_limit)
    {
        ...
        int rv = psm_read_flash(psm, &pobj, sizeof(psm_object_t), source_offset);
        if (rv != WM_SUCCESS)
        {
            psm_d("%s: could not read object @ : %x", __func__, source_offset);
            return -WM_FAIL;
        }
        ...
        if (!psm_is_data_len_sane(psm, pobj.data_len))
        {
            psm_d("%s: Invalid data length: %d", __func__, pobj.data_len);
            return -WM_FAIL;
        }
        ...
        {
            psm_d(
                "%s: Invalid obj at the end of data area. "
                "Ignoring",
                __func__);
            break;
        }
        ...
        if (!is_object_active(&pobj))
        {
            psm_ed("Inactive object with id: %x size: %d", pobj.obj_id, pobj.data_len);
        ...
        if (checkpoint == CHECKPOINT_1)
        {
             ...
             */
            rv = psm_upgrade_object(psm, &pobj, source_offset, dest_offset, ++objid_counter);
        }
        else if (checkpoint == CHECKPOINT_2)
        {
             ...
             */
            rv = psm_copy_object(psm, &pobj, source_offset, dest_offset, ++objid_counter);
        }
        else
        {
            rv = -WM_FAIL;
        ...
        if (dest_offset >= dest_limit)
        {
             ...
             */
            psm_d("%s: no space left in destination to copy", __func__);
            return -WM_FAIL;
        }
    }
...
static int handle_recovery(psm_t *psm, checkpoint_t checkpoint, psm_swap_desc_t *swap_desc)
{
    ...
    switch (checkpoint)
    {
        ...
        {
            if (g_psm_event_cb)
                g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_1, (void *)PSM_COMPACTION_MAX);
            /* Swap needs to be erased */
            uint32_t swap_start_offset = psm_get_swap_start_offset(psm);
            ...
            if (g_psm_event_cb)
                g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_2, (void *)PSM_COMPACTION_MAX);
            ...
            if (g_psm_event_cb)
                g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_3, (void *)PSM_COMPACTION_MAX);
        ...
        case CHECKPOINT_1:
        {
            ...
            uint32_t swap_data_offset = swap_desc->swap_start_offset + PSM_DATA_SWAP_DEMARCATION;
            uint32_t swap_limit       = part_size(psm) - sizeof(psm_swap_desc_t);

            rv = psm_copy_all_objects(psm, checkpoint, data_offset, data_limit, swap_data_offset, swap_limit);
            if (rv != WM_SUCCESS)
            {
            ...
            if (g_psm_event_cb)
                g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_5, (void *)PSM_COMPACTION_MAX);
            ...
            if (g_psm_event_cb)
                g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_6, (void *)PSM_COMPACTION_MAX);
        ...
        case CHECKPOINT_2:
        {
             ...
             * caller request.
             */
            rv = psm_erase_flash(psm, 0, swap_desc->swap_start_offset);
            if (rv != WM_SUCCESS)
            {
            ...
            if (g_psm_event_cb)
                g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_7, (void *)PSM_COMPACTION_MAX);
            ...
            rv = psm_copy_all_objects(psm, checkpoint, swap_data_offset, swap_limit, data_offset, data_limit);
            if (rv != WM_SUCCESS)
            {
            ...
            if (g_psm_event_cb)
                g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_8, (void *)PSM_COMPACTION_MAX);
            ...
            if (g_psm_event_cb)
                g_psm_event_cb(PSM_EVENT_COMPACTION, (void *)PSM_COMPACTION_9, (void *)PSM_COMPACTION_MAX);
        ...
        case CHECKPOINT_3:
        {
            ...
            rv = psm_erase_flash(psm, erase_start, part_size(psm) - erase_start);
            if (rv != WM_SUCCESS)
            {
 ...
 */
static int read_and_match_name(psm_t *psm, const psm_object_t *pobj, const char *name, uint32_t offset)
{
 ...
 * < 0: Error
 */
static int read_and_match_object(psm_t *psm,
                                 psm_object_t *pobj,
                                 objectid_t *obj_id,
                                 const char *name,
                                 uint32_t name_len,
                                 ...
                                 uint32_t limit)
{
    ...
    if (*obj_id == PSM_INVALID_OBJ_ID)
    {
        ...
        rv = read_and_match_name(psm, pobj, name, offset + sizeof(psm_object_t));
        if (rv < 0)
            return rv;
 ...
 */
static psm_index_t *psm_search_index(psm_t *psm, const char *name, uint32_t name_len, psm_object_t *pobj)
{
    ...
    {
        index = &psm->index[cnt];
        psm_i("Check: obj_id: %x H: %x: %x", index->obj_id, name_hash, index->name_hash);
        ...
        int rv = psm_read_flash(psm, pobj, sizeof(psm_object_t), index->flash_offset);
        if (rv != WM_SUCCESS)
        {
        ...
        rv = read_and_match_name(psm, pobj, name, index->flash_offset + sizeof(psm_object_t));
        if (rv < 0)
        {
...
static void psm_remove_index_entry(psm_t *psm, psm_index_t *index)
{
    ...
    else
    {
        ...
        memmove(index, index + 1, (sizeof(psm_index_t) * later_index_count));
        psm->index_elements--;
    }
}

static int search_active_object(psm_t *psm,
                                psm_object_t *pobj,
                                const char *name,
                                uint32_t name_len,
                                uint32_t *flash_offset,
                                ...
                                psm_index_t **index)
{
    ...
    *found = false;
    ...
    while (offset < limit)
    {
        int rv = read_and_match_object(psm, pobj, obj_id, name, name_len, offset, limit);
        if (rv < 0)
        {
...
static void psm_init_new_object(psm_t *psm,
...
static int manage_space_constraints(psm_t *psm, uint32_t new_obj_size, bool *compaction_reqd)
{
    ...
    uint32_t proposed_active_objects_size = psm->total_active_object_size + new_obj_size;
    uint32_t proposed_swap_start_offset   = align_to_block_offset(
        psm, (part_size(psm) - sizeof(psm_swap_desc_t) - PSM_DATA_SWAP_DEMARCATION - proposed_active_objects_size));
    uint32_t proposed_data_free_offset = psm->data_free_offset + new_obj_size;
...
static int _psm_object_open(psm_t *psm,
                            psm_objinfo_t *objinfo,
                            const char *name,
                            psm_mode_t mode,
                            uint32_t max_data_len,
                            psm_objattrib_t *attribs)
{
    psm_entry_i("name: %s mode: %d max_data_len = %d", name, mode, max_data_len);
    ...
    if (mode == PSM_MODE_WRITE)
    {
        ...
        rv = manage_space_constraints(psm, sizeof(psm_object_t) + name_len + max_data_len, &compaction_reqd);
        if (rv != WM_SUCCESS)
        {
            psm_d("Unable to alloc space for new obj size: %u", max_data_len);
            return rv;
        }
        ...
        if (compaction_reqd)
        {
            ...
            {
                psm_d(
                    "Compaction to alloc space for new "
                    "obj size: %u failed",
                    max_data_len);
                return rv;
            }
    ...
    rv = search_active_object(psm, &objinfo->pobj, name_to_search, name_len, &flash_offset, &found, &obj_id, &index);
    if (rv != WM_SUCCESS)
    {
    ...
    if (mode == PSM_MODE_READ)
    {
        ...
        if (part_encrypted(psm))
        {
            ...
            */
            rv = decrypt_buf(psm, psm->psm_name_enc_buf, psm->psm_name_enc_buf, name_len);
            if (rv)
                return rv;
        }
#endif /* CONFIG_SECURE_PSM */
    ...
    if (found)
    {
        ...
        psm_init_new_object(psm, objinfo, old_obj_id, name_len, attribs, mode, max_data_len);
    ...
    else
    {
         ...
         */
        objectid_t new_obj_id = (obj_id == PSM_INVALID_OBJ_ID) ? psm_get_unused_obj_id(psm) : obj_id;
        /* Brand new object */
        psm_init_new_object(psm, objinfo, new_obj_id, name_len, attribs, mode, max_data_len);
    }
    ...
    rv = psm_write_flash(psm, &objinfo->pobj, sizeof(psm_object_t), objinfo->curr_flash_offset, PSM_WRITE_NO_VERIFY);
    if (rv != WM_SUCCESS)
    {
        psm_d("%s: metadata write failed.", __func__);
        return rv;
    ...
    rv = psm_write_flash(psm, name_to_search, name_len, objinfo->curr_flash_offset, PSM_WRITE_NO_VERIFY);
    if (rv != WM_SUCCESS)
    {
        psm_d("%s: metadata write failed.", __func__);
        return rv;
...
static int _psm_object_write(psm_t *psm, psm_objinfo_t *objinfo, const uint8_t *data, uint32_t len)
{
    ...
    {
        psm_d("Write failed. Write more than requested: %d: %d", max_allowed_len, len);
        return -WM_E_NOSPC;
    }
    ...
    if (part_encrypted(psm))
    {
        ...
        {
            int enc_len = data_remaining > READ_BUFFER_SIZE ? READ_BUFFER_SIZE : data_remaining;

            rv = encrypt_buf(psm, data + offset, psm->psm_enc_buf, enc_len);
            if (rv)
            {
            ...
            rv = psm_write_flash(psm, psm->psm_enc_buf, enc_len, objinfo->curr_flash_offset, PSM_WRITE_NO_VERIFY);
            if (rv != WM_SUCCESS)
            {
                psm_d("%s: Write failed", __func__);
                return rv;
            }
            pobj->crc32 = soft_crc32(psm->psm_enc_buf, enc_len, pobj->crc32);
...
#endif
    rv = psm_write_flash(psm, data, len, objinfo->curr_flash_offset, PSM_WRITE_NO_VERIFY);
    if (rv != WM_SUCCESS)
    {
        psm_d("%s: Write failed", __func__);
        return rv;
...
static int _psm_object_read(psm_t *psm, psm_objinfo_t *objinfo, uint8_t *buf, uint32_t max_len)
{
    psm_object_t *pobj = &objinfo->pobj;
    psm_entry_i("obj-id: %x mode: %d max_len: %d", pobj->obj_id, objinfo->mode, max_len);
    int rv;
    ...
    const uint32_t data_start_offset = objinfo->object_flash_offset + metadata_size(pobj);
    uint32_t remaining               = pobj->data_len - (objinfo->curr_flash_offset - data_start_offset);
    if (remaining == 0)
    {
    ...
    if (remaining == read_size)
    {
         ...
         */
        objinfo->crc32 = psm_calc_metadata_crc32(&objinfo->pobj, objinfo->crc32);
        if (objinfo->crc32 != pobj->crc32)
        {
            psm_d(
                "Invalid CRC32 during read detected "
                "0x%x != 0x%x",
                objinfo->crc32, pobj->crc32);
            return -WM_E_CRC;
        }
    }
...
static int _psm_object_close(psm_t *psm, psm_objinfo_t *objinfo)
{
    ...
    {
        psm_d("Object %d (opened in read mode) closed", objinfo->pobj.obj_id);
        return WM_SUCCESS;
    }
    ...
    psm_d("Closing object: length: %d Crc32: 0x%x", pobj->data_len, pobj->crc32);
    /* Write the crc32 and data length fields to flash */
    uint32_t size   = sizeof(pobj->crc32) + sizeof(pobj->data_len);
    uint32_t offset = objinfo->object_flash_offset + offsetof(psm_object_t, crc32);
    int rv          = psm_write_flash(psm, &pobj->crc32, size, offset, PSM_WRITE_NO_VERIFY);
    if (rv != WM_SUCCESS)
    {
        psm_d(
            "Failed to update length and crc32 fields for "
            "object: %d",
            pobj->obj_id);
        return rv;
    }
    ...
    if (objinfo->old_active_object_present)
    {
        ...
        uint32_t flag_offset = objinfo->old_object_flash_offset + offsetof(psm_object_t, flags);
        rv                   = write_flag_bit(psm, flag_offset, PSM_OBJ_INACTIVE);
        if (rv != WM_SUCCESS)
        {
                    ...
                    psm_object_handle_t *ohandle)
{
    ...
    *ohandle   = 0;
    ...
    if (mode == PSM_MODE_WRITE)
    {
        ...
        {
            psm_d("Unsupported data length for new object: %u", max_data_len);
            return -WM_E_INVAL;
        }
    }
    ...
    int rv = _psm_object_open(psm, objinfo, name, mode, max_data_len, attribs);
    if (rv != WM_SUCCESS)
    {
        os_mem_free(objinfo);
        psm_rwunlock_part(psm, mode);
...
static int _psm_object_delete(psm_t *psm, const char *name, uint32_t name_len)
{
    ...
    int rv = search_active_object(psm, &pobj, name_to_search, name_len, &flash_offset, &found, &obj_id, &index);
    if (rv != WM_SUCCESS)
    {
...
static int _psm_init_verify_params(const flash_desc_t *fdesc, uint32_t block_size, psm_cfg_t *psm_cfg)
{
    if (!fdesc->fl_size)
    {
    ...
    {
        psm_d("Atleast %d block(s) are required for PSM operation", min_block_cnt_req);
        return -WM_E_PERM;
    }
...
static int deduce_checkpoint_at_init(psm_t *psm, psm_swap_desc_t *swap_desc, checkpoint_t *checkpoint)
{
    psm_entry_i();
...
static int psm_module_physical_init(psm_t *psm)
{
    ...
    if (checkpoint != CHECKPOINT_2)
    {
        ...
        if (rv != WM_SUCCESS)
        {
             ...
             */
            psm_d(
                "Partition scan failed. Will try compaction"
                "/upgrade");
...
static psm_t *_get_existing_psm_object(const flash_desc_t *fdesc)
{
    ...
    {
        psm = PSM_LIST_OBJ(psm_t *, node, list);
        if (psm->fdesc_fl_dev == fdesc->fl_dev && psm->start_address == fdesc->fl_start)
        {
            /*
...
int psm_module_init(flash_desc_t *fdesc, psm_hnd_t *phandle, psm_cfg_t *psm_cfg)
{
    ...
    psm_entry("start-address: 0x%x size: %d", fdesc->fl_start, fdesc->fl_size);

    uint32_t block_size = psm_get_block_size();
    int rv              = _psm_init_verify_params(fdesc, block_size, psm_cfg);
    if (rv != WM_SUCCESS)
        return rv;
    ...
    if (psm->psm_cfg.keystore_override == false)
    {
        ...
        else if (rv == -WM_E_NOENT)
        {
            ...
            rv = psm_security_init_with_key(&psm->psm_sec_hnd, &psm->psm_cfg);
            if (rv == WM_SUCCESS)
    ...
    if (!part_readonly(psm))
    {
        ...
        psm->object_offset_limit = part_size(psm) - block_size(psm) - sizeof(psm_object_t);
    }
    else
    {
        psm->object_offset_limit = part_size(psm) - sizeof(psm_object_t);
    }
    ...
    PSM_List_AddTail(&part_list, &psm->node);
    os_exit_critical_section(save);
...
int psm_module_init_ex(const psm_init_part_t *psm_init_part, psm_hnd_t *phandle)
{
    ...
    {
        p = part_get_layout_by_name(psm_init_part->part_info.name, NULL);
        if (p == NULL)
            return -WM_E_INVAL;
...
int psm_set_variable(psm_hnd_t phandle, const char *variable, const void *value, uint32_t len)
{
    ...
    psm_objinfo_t objinfo;
    int rv = _psm_object_open(psm, &objinfo, variable, mode, len, NULL);
    if (rv != WM_SUCCESS)
    {
...
int psm_get_variable(psm_hnd_t phandle, const char *variable, void *value, uint32_t max_len)
{
    ...
    psm_objinfo_t objinfo;
    int rv = _psm_object_open(psm, &objinfo, variable, mode, 0, NULL);
    if (rv != WM_SUCCESS)
    {
    ...
    {
        psm_d("%s: Variable value %d will not fit in given buffer %d", __func__, objinfo.pobj.data_len, max_len);
    ...
    read_len = _psm_object_read(psm, &objinfo, value, max_len);
...
static int list_object(psm_t *psm, psm_object_t *pobj, uint32_t object_offset, psm_list_cb list_cb)
{
