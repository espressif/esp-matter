...
unsigned int g_LargestBlockSize = 0;
...
#ifdef BestFit
    g_LargestBlockSize = 0;
    while (b != &freelist) {
        if( ((bufsize) b->bh.bsize) > g_LargestBlockSize )
		{
			g_LargestBlockSize = (bufsize) b->bh.bsize;
		}
        if (b->bh.bsize >= size) {
  ...
  void *buf)
{
    ...
    if (bs < 0) {
        ...
        bs = -bs;
        bn =  BFH(((char *) b) + bs);
        /* Suppressing "bh" unused warning due to undefined ASSERT(t)  */
        (void) bn;
        ASSERT(bn->bh.prevfree== 0);
    } else {
        //buffer free
        ASSERT(bs > 0);
        if (bs <= 0) {
        ...
        bn =  BFH(((char *) b) + bs);
        //verify next buffer prevfree have that size
        ASSERT(bn->bh.prevfree == b->bh.bsize);
