...
static void EnableSectorsForWrite(uint32_t ui32BankNo);
...
uint32_t FlashSectorErase(uint32_t ui32SectorAddress) {
    ...
    uint32_t ui32SectorNumber;
    uint32_t ui32BankNo;
    uint32_t ui32TopBankStartAddr = (HWREG(FLASH_BASE + FLASH_O_FCFG_B1_START) &
    ...
    // Find flash bank contaning sector to erase
    ui32BankNo = 0;
    if(ui32SectorAddress >= ui32TopBankStartAddr)
    {
        ui32BankNo = 1;
    }

    // Enable all sectors for erase.
    EnableSectorsForWrite(ui32BankNo);
    ...
    if(ui32SectorNumber <= 31)
    {
        ui32SectorBit = 1 << ui32SectorNumber;
...
uint32_t FlashBankErase(bool bForcePrecondition) {
    uint32_t ui32ErrorReturn = FAPI_STATUS_SUCCESS;
    ...
    HWREG(FLASH_BASE + FLASH_O_FADDR) = ADDR_OFFSET + (ui32BankNo * ui32TopBankStartAddr);
...
uint32_t FlashhOtpEngrErase(void) {
    ...
    HWREG(FLASH_BASE + FLASH_O_FBAC) = (HWREG(FLASH_BASE + FLASH_O_FBAC) & ~FLASH_FBAC_OTPPROTDIS_M)
                                       | ((1 << ui32BankNo) << FLASH_FBAC_OTPPROTDIS_S);
    ...
    if(ui32BankNo == 1)
    {
        HWREG(FLASH_BASE + FLASH_O_FADDR) = 0xF0002000;
    }
    else
    {
    ...
    HWREG(FLASH_BASE + FLASH_O_FBPROT) = FLASH_FBPROT_PROTL1DIS;
    HWREG(FLASH_BASE + FLASH_O_FBAC) &= ~FLASH_FBAC_OTPPROTDIS_M;
    HWREG(FLASH_BASE + FLASH_O_FBPROT) = 0;
...
uint32_t FlashProgram(uint8_t *pui8DataBuffer, uint32_t ui32Address, uint32_t ui32Count) {
    ...
    ui32BankNo = 0;
    if(ui32Address >= ui32TopBankStartAddr)
    {
        ui32BankNo = 1;
    }

    // Enable sectors for programming in target bank.
    EnableSectorsForWrite(ui32BankNo);
    ...
    while(ui32Count)
    {
        ...
        if((ui32BankNo == 0) && (ui32Address >= ui32TopBankStartAddr))
        {
            // Change target bank.
            FlashDisableSectorsForWrite();
            ui32BankNo = 1;
...
uint32_t FlashProgramNowait(uint32_t ui32StartAddress, uint8_t *pui8DataBuffer, uint8_t ui8NoOfBytes) {
    ...
    ui32BankNo = 0;
    if(ui32StartAddress >= ui32TopBankStartAddr)
    {
        ui32BankNo = 1;
    }
...
uint32_t FlashProgramEngr(uint8_t *pui8DataBuffer, uint32_t ui32AddressOffset, uint32_t ui32Count) {
    ...
    // Check the arguments.
    ASSERT((ui32AddressOffset + ui32Count) <= (2048 * 2));
    // Enable sectors for programming.
    ui32BankNo = 0;
    if(ui32AddressOffset >= 2048)
    {
        ui32BankNo = 1;
    }
    EnableSectorsForWrite(ui32BankNo);
    ...
    // Check the arguments.
    if(((ui32AddressOffset + ui32Count) > (2048 * 2)) ||
       ((ui32AddressOffset < 2048) && ((ui32AddressOffset + ui32Count) > 2048)))
    {
        // Invalid arguments. Exit function!
        FlashDisableSectorsForWrite();
        return (FAPI_STATUS_INCORRECT_DATABUFFER_LENGTH);
    ...
    while(ui32Count)
    {
        ...
        {
            HWREG(FLASH_BASE + FLASH_O_FADDR) = (ui32AddressOffset - 2048) + 0xF0082000;
        }
...
//*****************************************************************************
static void
EnableSectorsForWrite(uint32_t ui32BankNo)
{
...
SetWriteMode(void)
{
       ...
       AON_PMCTL_PWRCTL_EXT_REG_MODE)
    {
        ...
        ui32Value |= ((ui32TrimValue &
                       ...
                       FCFG1_FLASH_OTP_DATA4_DIS_STANDBY_EXT_WRT_S) <<
                       FLASH_CFG_DIS_STANDBY_S;
    ...
    else
    {
        ...
        ui32Value |= ((ui32TrimValue &
                       ...
                       FCFG1_FLASH_OTP_DATA4_DIS_STANDBY_INT_WRT_S) <<
                       FLASH_CFG_DIS_STANDBY_S;
