    ...
    dma_descriptor_t *desc, uint32_t xfercfg, void *srcStartAddr, void *dstStartAddr, void *nextDesc)
{
...
#if (defined(FSL_FEATURE_MEMORY_HAS_ADDRESS_OFFSET) && FSL_FEATURE_MEMORY_HAS_ADDRESS_OFFSET)
    srcStartAddr = (void *)MEMORY_ConvertMemoryMapAddress((uint32_t)srcStartAddr, kMEMORY_Local2DMA);
    dstStartAddr = (void *)MEMORY_ConvertMemoryMapAddress((uint32_t)dstStartAddr, kMEMORY_Local2DMA);
    nextDesc     = (void *)MEMORY_ConvertMemoryMapAddress((uint32_t)nextDesc, kMEMORY_Local2DMA);
#endif
     ...
     */
    if (((NULL != srcStartAddr) && (0 == (uint32_t)srcStartAddr % width)) &&
        ((NULL != dstStartAddr) && (0 == (uint32_t)dstStartAddr % width)))
    {
        if (srcInc == 3U)
        {
