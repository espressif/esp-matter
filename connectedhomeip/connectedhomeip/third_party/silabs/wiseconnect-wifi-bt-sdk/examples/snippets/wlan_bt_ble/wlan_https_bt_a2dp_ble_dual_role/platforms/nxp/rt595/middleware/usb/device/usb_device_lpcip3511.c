...
void USB_DeviceLpcIp3511IsrFunction(void *deviceHandle)
{
    ...
    if (interruptStatus & USB_LPC3511IP_INTSTAT_DEV_INT_MASK)
    {
        ...
        if((0U == lpc3511IpState->deviceState) &&
           (((lpc3511IpState->registerBase->DEVCMDSTAT & USB_LPC3511IP_DEVCMDSTAT_DCON_MASK) && (devState & USB_LPC3511IP_DEVCMDSTAT_VBUS_DEBOUNCED_MASK)) ||
            ((!(lpc3511IpState->registerBase->DEVCMDSTAT & USB_LPC3511IP_DEVCMDSTAT_DCON_MASK)) && (lpc3511IpState->registerBase->LPM & USB_LPC3511IP_USB_LPM_ADPPROBE_MASK))) &&
