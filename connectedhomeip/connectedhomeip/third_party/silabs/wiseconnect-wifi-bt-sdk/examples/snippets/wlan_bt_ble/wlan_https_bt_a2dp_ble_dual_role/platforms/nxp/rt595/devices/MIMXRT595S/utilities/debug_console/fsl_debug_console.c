...
#elif COEX_TEST_FRAMEWORK
#include <rsi_extract_user_input.h>
#endif
#ifdef RSI_WITH_OS
...
#endif
#endif
#if !(COEX_MAX_APP || COEX_TEST_FRAMEWORK)
...
void FLEXCOMM0_IRQHandler(void)
{
    ...
    #if (COEX_MAX_APP)
    //! disable the interrupts after receiving '\r' from serial terminal
...
#endif
    #if (COEX_TEST_FRAMEWORK)
    //! disable the interrupts after receiving '\r' from serial terminal
    if(demoRingBuffer[rxIndex - 1] == '\r')
    {
    	//! disable uart interrupt
    	DisableIRQ(FLEXCOMM0_IRQn);
    	//rxIndex = 0;
...
status_t DbgConsole_Init(uint8_t instance, uint32_t baudRate, serial_port_type_t device, uint32_t clkSrcFreq)
{
...

    {
        ...
        assert(kStatus_SerialManager_Success == status);
#if 0        
#if (BT_A2DP_SOURCE_WIFI_HTTP_S_RX_DYN_COEX || BLE_MULTI_SLAVE_MASTER || COEX_MAX_APP || COEX_TEST_FRAMEWORK)
#ifdef RSI_WITH_OS
            /* Enable RX interrupt. */
          USART_EnableInterrupts(USART0, kUSART_RxLevelInterruptEnable | kUSART_RxErrorInterruptEnable);
          NVIC_SetPriority(FLEXCOMM0_IRQn, BOARD_UART_IRQ_PRIORITY_5); 
