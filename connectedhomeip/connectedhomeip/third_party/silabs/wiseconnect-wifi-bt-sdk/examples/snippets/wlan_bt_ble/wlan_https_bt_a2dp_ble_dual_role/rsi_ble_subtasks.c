/*******************************************************************************
* @file  rsi_ble_subtasks_DEMO_57.c
* @brief 
*******************************************************************************
* # License
...
/*==============================================*/
/**
 * @fn         rsi_ble_check_bitmap_based_on_conn
 * @brief      checks weather the required bit available or not
 * @param[in]  none.
 * @return     int32_t
 *             > 1  = event available
 *               0  = event not available
 * @section description
 * This function returns the either 1 or 0
 */
int32_t rsi_ble_check_bitmap_based_on_conn(uint8_t conn_id, uint32_t event_num)
{
  if (event_num < 32) {
    if (ble_app_event_task_map[conn_id] & (1 << event_num)) {
      return 1;
    }
  } else {
    if (ble_app_event_task_map1[conn_id] & (1 << (event_num - 32))) {
      return 1;
    }
  }
  return 0;
...
void rsi_ble_task_on_conn(void *parameters)
{
  ...
  uint8_t smp_done = 0, mtu_exchange_done = 0, neg_rply = 0;
    ...
    switch (event_id) {
      ...
      case RSI_BLE_CONN_EVENT: {
        ...
        if (l_conn_id >= RSI_BLE_MAX_NBR_SLAVES) {
          ...
          if ((num_of_conn_slaves < RSI_BLE_MAX_NBR_SLAVES) & !CHK_BIT(rsi_ble_states_bitmap, RSI_SCAN_STATE)) {
          ...
          else if ((num_of_conn_slaves == RSI_BLE_MAX_NBR_SLAVES) & !CHK_BIT(rsi_ble_states_bitmap, RSI_SCAN_STATE)) {
            LOG_PRINT("\r\n Start scanning - conn%d\n", l_conn_id);
      ...
      case RSI_BLE_ENHC_CONN_EVENT: {
        ...
        if (l_conn_id >= RSI_BLE_MAX_NBR_SLAVES) {
          ...
          //! if max connections reached, advertise module with non connectable advertising interval of 211.25ms
          else if ((num_of_conn_masters == RSI_BLE_MAX_NBR_MASTERS) & !CHK_BIT(rsi_ble_states_bitmap, RSI_ADV_STATE)) {
            status = rsi_ble_start_advertising_with_values(&change_adv_param);
            if (status != RSI_SUCCESS) {
              LOG_PRINT("\r\n advertising with values failed with status = 0x%x -conn%d \n", status, l_conn_id);
              //continue ;
      ...
      case RSI_BLE_CONN_UPDATE_COMPLETE_EVENT: {
        ...
        if (ble_conn_conf->buff_mode_sel.buffer_mode == 1) {
          if ((rsi_ble_conn_info[l_conn_id].conn_update_resp.conn_interval) <= 12) {
            ...
            if (rsi_ble_conn_info[l_conn_id].remote_device_role == MASTER_ROLE) {
              ble_conn_conf->buff_mode_sel.buffer_cnt = 3;
            }
          } else if ((rsi_ble_conn_info[l_conn_id].conn_update_resp.conn_interval) <= 40) {
            //! Updated Feasible Buffer Count based on the Connection Interval
            if (rsi_ble_conn_info[l_conn_id].remote_device_role == MASTER_ROLE) {
              ble_conn_conf->buff_mode_sel.buffer_cnt = 5;
            }
          } else {
            //! Updated Feasible Buffer Count based on the Connection Interval
            if (rsi_ble_conn_info[l_conn_id].remote_device_role == MASTER_ROLE) {
              ble_conn_conf->buff_mode_sel.buffer_cnt = 7;
            }
          }

          if (rsi_ble_conn_info[l_conn_id].remote_device_role == MASTER_ROLE) {
            // take backup of the Data transfer bitmap
            if (rsi_ble_check_bitmap_based_on_conn(l_conn_id, RSI_DATA_RECEIVE_EVENT)) {
              rsi_rx_from_rem_dev = true;
              rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_DATA_RECEIVE_EVENT);
            }
            if (rsi_ble_check_bitmap_based_on_conn(l_conn_id, RSI_DATA_TRANSMIT_EVENT)) {
              rsi_tx_to_rem_dev = true;
              rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_DATA_TRANSMIT_EVENT);
            }
            buff_config_done = false;
      ...
      case RSI_DATA_TRANSMIT_EVENT: {
        ...
        //! Notify to remote device continuously
          ...
          status =
            rsi_ble_notify_value(rsi_connected_dev_addr, rsi_ble_att1_val_hndl, max_data_length, (uint8_t *)read_data1);
          if (status != RSI_SUCCESS) {
            if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
#if RSI_DEBUG_EN
      ...
      case RSI_BLE_SMP_FAILED_EVENT: {
        ...
        status = rsi_ble_disconnect(rsi_ble_conn_info[l_conn_id].rsi_ble_smp_failed.dev_addr);
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\ndisconnect command failed with reason %x\n", status);
        }
      }
