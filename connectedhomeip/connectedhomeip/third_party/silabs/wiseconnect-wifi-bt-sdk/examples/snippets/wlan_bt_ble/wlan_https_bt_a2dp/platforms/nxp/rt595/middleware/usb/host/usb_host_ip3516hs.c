        ...
        else
        {
            ...
            switch (pipe->pipeCommon.pipeType)
            {
                case USB_ENDPOINT_CONTROL:
                    ...
                    switch (trCurrent->setupStatus)
                    {
                        ...
                        case kStatus_UsbHostIp3516Hs_Data2:
                            ...
                            if (USB_IN == trCurrent->direction)
                            {
                                memcpy(&trCurrent->transferBuffer[trCurrent->transferSofar], &s_UsbHostIp3516HsBufferArray[pipe->bufferIndex][0], s_UsbHostIp3516HsPtd[usbHostState->controllerId].atl[pipe->tdIndex].stateUnion.stateBitField.NrBytesToTransfer);
                            }
                            trCurrent->transferSofar += s_UsbHostIp3516HsPtd[usbHostState->controllerId]
                                                            .atl[pipe->tdIndex]
                                                            .stateUnion.stateBitField.NrBytesToTransfer;
                ...
                case USB_ENDPOINT_INTERRUPT:
                    ...
                    if (USB_IN == trCurrent->direction)
                    {                      
                        memcpy(&trCurrent->transferBuffer[trCurrent->transferSofar], &s_UsbHostIp3516HsBufferArray[pipe->bufferIndex][0], s_UsbHostIp3516HsPtd[usbHostState->controllerId].interrupt[pipe->tdIndex].stateUnion.stateBitField.NrBytesToTransfer);
                    }
                    trCurrent->transferSofar += s_UsbHostIp3516HsPtd[usbHostState->controllerId]
                                                    .interrupt[pipe->tdIndex]
                                                    .stateUnion.stateBitField.NrBytesToTransfer;
                ...
                case USB_ENDPOINT_ISOCHRONOUS:
                    ...
                    if (USB_IN == trCurrent->direction)
                    {   
                        memcpy(&trCurrent->transferBuffer[trCurrent->transferSofar], &s_UsbHostIp3516HsBufferArray[pipe->bufferIndex][0], s_UsbHostIp3516HsPtd[usbHostState->controllerId].iso[indexLength.state.tdIndex].stateUnion.stateBitField.NrBytesToTransfer);
                    }
                    trCurrent->transferSofar += s_UsbHostIp3516HsPtd[usbHostState->controllerId]
                                                    .iso[indexLength.state.tdIndex]
                                                    .stateUnion.stateBitField.NrBytesToTransfer;
                                      ...
                                      usb_host_pipe_init_t *pipeInit)
{
    ...
    if (USB_ENDPOINT_ISOCHRONOUS == pipe->pipeCommon.pipeType)
    {
        ...
        pipe->pipeCommon.interval =
            (1U << (pipe->pipeCommon.interval - 1)); /* iso interval is the power of 2 */
#else
        pipe->pipeCommon.interval = 1U;
#endif
    }
