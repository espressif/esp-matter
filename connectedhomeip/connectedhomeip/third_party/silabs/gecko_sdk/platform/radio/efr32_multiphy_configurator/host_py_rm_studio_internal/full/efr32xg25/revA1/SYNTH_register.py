...
class RM_Register_SYNTH_SFMIF(Base_RM_Register):
    def __init__(self, rmio, label):
        self.__dict__['zz_frozen'] = False
        super(RM_Register_SYNTH_SFMIF, self).__init__(rmio, label,
            0xa8018000, 0x0C0,
            'SFMIF', 'SYNTH.SFMIF', 'read-write',
            u"",
            0x00000000, 0x00000237,
            0x00001000, 0x00002000,
            0x00003000)

        self.LOCKEDSFMIF = RM_Field_SYNTH_SFMIF_LOCKEDSFMIF(self)
        self.zz_fdict['LOCKEDSFMIF'] = self.LOCKEDSFMIF
        self.UNLOCKEDSFMIF = RM_Field_SYNTH_SFMIF_UNLOCKEDSFMIF(self)
        self.zz_fdict['UNLOCKEDSFMIF'] = self.UNLOCKEDSFMIF
        self.SYRDYSFMIF = RM_Field_SYNTH_SFMIF_SYRDYSFMIF(self)
        self.zz_fdict['SYRDYSFMIF'] = self.SYRDYSFMIF
        self.VCOHIGHSFMIF = RM_Field_SYNTH_SFMIF_VCOHIGHSFMIF(self)
        self.zz_fdict['VCOHIGHSFMIF'] = self.VCOHIGHSFMIF
        self.VCOLOWSFMIF = RM_Field_SYNTH_SFMIF_VCOLOWSFMIF(self)
        self.zz_fdict['VCOLOWSFMIF'] = self.VCOLOWSFMIF
        self.LOCNTDONESFMIF = RM_Field_SYNTH_SFMIF_LOCNTDONESFMIF(self)
        self.zz_fdict['LOCNTDONESFMIF'] = self.LOCNTDONESFMIF
        self.__dict__['zz_frozen'] = True


class RM_Register_SYNTH_SFMIEN(Base_RM_Register):
    def __init__(self, rmio, label):
        self.__dict__['zz_frozen'] = False
        super(RM_Register_SYNTH_SFMIEN, self).__init__(rmio, label,
            0xa8018000, 0x0C4,
            'SFMIEN', 'SYNTH.SFMIEN', 'read-write',
            u"",
            0x00000000, 0x00000237,
            0x00001000, 0x00002000,
            0x00003000)

        self.LOCKEDSFMIEN = RM_Field_SYNTH_SFMIEN_LOCKEDSFMIEN(self)
        self.zz_fdict['LOCKEDSFMIEN'] = self.LOCKEDSFMIEN
        self.UNLOCKEDSFMIEN = RM_Field_SYNTH_SFMIEN_UNLOCKEDSFMIEN(self)
        self.zz_fdict['UNLOCKEDSFMIEN'] = self.UNLOCKEDSFMIEN
        self.SYRDYSFMIEN = RM_Field_SYNTH_SFMIEN_SYRDYSFMIEN(self)
        self.zz_fdict['SYRDYSFMIEN'] = self.SYRDYSFMIEN
        self.VCOHIGHSFMIEN = RM_Field_SYNTH_SFMIEN_VCOHIGHSFMIEN(self)
        self.zz_fdict['VCOHIGHSFMIEN'] = self.VCOHIGHSFMIEN
        self.VCOLOWSFMIEN = RM_Field_SYNTH_SFMIEN_VCOLOWSFMIEN(self)
        self.zz_fdict['VCOLOWSFMIEN'] = self.VCOLOWSFMIEN
        self.LOCNTDONESFMIEN = RM_Field_SYNTH_SFMIEN_LOCNTDONESFMIEN(self)
