...
/// String for example
static uint8_t example_string[] = "SE Manager Key Provisioning Example (TrustZone)";
  ...
  switch (app_state) {
...
#if !defined(CRYPTOACC_PRESENT)
    case READ_SE_OTP_VERSION:
      app_state = SE_MANAGER_EXIT;
      printf("\n  . Get current SE OTP version... ");
      if (get_se_otp_version() == SL_STATUS_OK) {
        printf("  + Current SE OTP version (MSB..LSB): %08lX\n",
               get_version());
        app_state = GET_OTP_CONF;
      }
      break;
    ...
    case RD_CERT_SIZE:
      ...
      if (read_cert_size() == SL_STATUS_OK) {
        printf("  + The size of on-chip device certificate is %lu\n",
               get_cert_size(SL_SE_CERT_DEVICE_HOST));
        printf("  + The size of on-chip batch certificate is %lu\n",
    ...
    case PRE_INIT_AES_KEY:
      ...
      if (space_press) {
        space_press = false;
        app_state = PRE_INIT_PUB_SIGN_KEY;
        printf("\n  . Press ENTER to program public sign key in SE OTP or "
               "press SPACE to skip.\n");
      }
      break;
