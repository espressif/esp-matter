...
uint32_t WsfBufInit(uint32_t bufMemLen, uint8_t *pBufMem, uint8_t numPools, wsfBufPoolDesc_t *pDesc)
{
  ...
  while (TRUE)
  {
    ...
    pPool->maxAlloc = 0;
    pPool->maxReqLen = 0;
    pPool->cumulativeLen = 0;
    pPool->cumulativeCount = 0;
#endif
...
/*************************************************************************************************/
/*!
 *  \brief  Verify whether a buffer with required length is available.
 *
 ...
 */
/*************************************************************************************************/
bool_t CheckWsfBufAlloc(uint16_t len)
{
  wsfBufPool_t  *pPool;
  uint8_t       i;
  bool_t        bufferAvailable = FALSE;
  ...
  for (i = wsfBufNumPools; i > 0; i--, pPool++)
  {
    ...
    if (len <= pPool->desc.len)
    {
      ...
      if (pPool->pFree != NULL)
      {
        ...
        pPool->cumulativeLen += len;
        pPool->cumulativeCount++;
#endif
        /* Exit critical section. */
        WSF_CS_EXIT(cs);
...
void WsfBufGetPoolStats(WsfBufPoolStat_t *pStat, uint8_t poolId)
{
  ...
  pStat->cumulativeLen = pPool[poolId].cumulativeLen;
  pStat->cumulativeCount = pPool[poolId].cumulativeCount;
#else
  pStat->numAlloc = 0;
  pStat->maxAlloc = 0;
  pStat->maxReqLen = 0;
  pStat->cumulativeLen = 0;
  pStat->cumulativeCount = 0;
#endif

#if WSF_BUF_STATS_HIST == TRUE
  pStat->overflows = wsfPoolOverFlowCount[poolId];
#else
  pStat->overflows = 0;
