                            ...
                            const int is_wrap )
{
    ...
    if( cipher_info->MBEDTLS_PRIVATE(block_size) != 16 )
        return( MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA );
    ...
    mbedtls_cipher_free( &ctx->MBEDTLS_PRIVATE(cipher_ctx) );

    if( ( ret = mbedtls_cipher_setup( &ctx->MBEDTLS_PRIVATE(cipher_ctx), cipher_info ) ) != 0 )
        return( ret );

    if( ( ret = mbedtls_cipher_setkey( &ctx->MBEDTLS_PRIVATE(cipher_ctx), key, keybits,
                          ...
                          unsigned char *output, size_t *out_len, size_t out_size )
{
    ...
    *out_len = 0;
        ...
        && in_len <= KW_SEMIBLOCK_LENGTH )
    {
        ...
        ret = mbedtls_cipher_update( &ctx->MBEDTLS_PRIVATE(cipher_ctx),
