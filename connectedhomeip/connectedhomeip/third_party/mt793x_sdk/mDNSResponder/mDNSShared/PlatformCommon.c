...
static void (*mLogFunc)(const char *, const char *) = NULL;

void mDNSPlatformWriteLogRedirect(void (*LogFunc)(const char *, const char *))
{
    mLogFunc = LogFunc;
}
...
mDNSexport void mDNSPlatformWriteLogMsg(const char * ident, const char * buffer, mDNSLogLevel_t loglevel)
{
    ...
    else if (mLogFunc)
    {
        switch (loglevel)
        {
        case MDNS_LOG_MSG:       mLogFunc("LOG", buffer); break;
        case MDNS_LOG_OPERATION: mLogFunc("OPR", buffer); break;
        case MDNS_LOG_SPS:       mLogFunc("SPS", buffer); break;
        case MDNS_LOG_INFO:      mLogFunc("INF", buffer); break;
        case MDNS_LOG_DEBUG:     mLogFunc("DBG", buffer); break;
        default:
