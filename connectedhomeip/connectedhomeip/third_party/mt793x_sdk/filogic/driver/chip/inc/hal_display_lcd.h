 ...
 * |\b DSI                      | The Display Serial Interface (DSI) is a specification by the Mobile Industry Processor Interface (MIPI). For more information, please refer to <a href="https://en.wikipedia.org/wiki/Display_Serial_Interface"> Display Serial Interface </a> in Wikipedia.
 ...
 *
 * - Use LCD in DMA mode. \n
 *  - Step 1. Call #hal_display_lcd_init() to initialize the LCD engine.
 *  - Step 2. Call #hal_display_lcd_config_roi() to set the ROI information.
 *  - Step 3. Call #hal_display_lcd_config_layer() to set the layer data.
 *  - Step 4. Call blockwrite funtion in LCM driver function table to start the transfer.
 *  - sample code:
 *    @code
 ...
 *
 * - Use LCD in CPU mode. \n
 *  - Step 1. Write a for loop to transfer pixel data in CPU mode using the blockwrite function of the LCM driver.
 *  - Step 2. Call #hal_display_lcd_init() to initialize the LCD engine.
 *  - Step 3. Call #hal_display_lcd_config_roi() to set the ROI information.
 *  - Step 4. Call #hal_display_lcd_config_layer() to set the layer data.
 *  - Step 5. Call blockwrite funtion in LCM driver function table to start transfer.
 *  - sample code:
 *    @code
 *      hal_display_lcd_roi_output_t roi_para;
 *      hal_display_lcd_layer_input_t layer_para;
