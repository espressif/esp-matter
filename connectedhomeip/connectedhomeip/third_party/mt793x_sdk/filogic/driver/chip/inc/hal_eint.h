 ...
 *
 * - \b Support \b software \b trigger \b interrupt. \n
 *   Can Call #hal_eint_set_software_trigger() function to trigger an interrupt immediately after the EINT is unmasked.
 *   \n
 * @}
 * @}
 */
 ...
 *  - Step 4: Register a callback function.
 *            Call #hal_eint_register_callback() to register a user callback function.
 *  - Step 5: Initialize the selected EINT source.
 ...
 *            \b Note: if trigger mode is \b HAL_EINT_EDGE_FALLING/RISING or \b HAL_EINT_EDGE_FALLING_AND_RISING, \b can't \b enable \b debounce, otherwise the error is produced.
 *  - Step 6: Unmask the EINT.
 *            Call #hal_eint_unmask() to unmask the EINT.
 *  - Sample code:
 *   \include eint_pinmux.dox
 *    @code
 ...
 *    @endcode
 */
#else /* #ifdef HAL_EINT_FEATURE_MASK */
/**
 * @addtogroup HAL
 * @{
 * @addtogroup EINT
 ...
 *  - Step 1: Call #hal_gpio_init() to init the pins, if EPT tool hasn't been used to configure the related pinmux. If EPT tool has been used, please skip step 1 and step 2.
 *  - Step 2: Call #hal_pinmux_set_function() to set the @ref GPIO pinmux. For more information about #hal_pinmux_set_function(), please refer to GPIO API reference.
 *  - Step 3: register a callback function. Call #hal_eint_register_callback() to register a user callback function.
...
typedef enum {
    ...
    HAL_EINT_STATUS_ERROR              = -1,     /**< EINT operation is fail. */
...
#endif /* #ifdef HAL_EINT_FEATURE_FIRQ */
} hal_eint_config_t;
 ...
 *  @param [in] user_data is parameter used by users.
 */
 ...
 *            If the return value is #HAL_EINT_STATUS_ERROR, the operation is fail;
 ...
 * @return    To indicate whether this function call is successful.
 *            If the return value is #HAL_EINT_STATUS_OK, the operation completed successfully;
 *            If the return value is #HAL_EINT_STATUS_ERROR_EINT_NUMBER, a wrong eint number is given, the number must be verified.
 * @sa  hal_eint_deinit()
 * @par       Example
 * Sample code please refer to @ref HAL_EINT_Driver_Usage_Chapter
*/
...
#endif /* #ifdef HAL_EINT_FEATURE_SW_TRIGGER_EINT */
