 ...
 *  local variable with "static" in cache APIs to prevent system hang up.
 */
...
/**
 * @brief  This function get hit count and access count according region group (0: Region 0~7, 1: Region 8~15
 * @param[in] countIdx is the region group index.
 * @param[in] hitCnt_h is the hit count high upper part (bit:0~15, CHIT_CNTx[47:32])
 * @param[in] hitCnt_l is the hit count low upper part  (bit:0~31, CHIT_CNTx[0:31])
 * @param[in] accCnt_h is the access count high upper part (bit:0~15, CACHE_CCNTx[47:32])
 * @param[in] accCnt_l is the access count low upper part  (bit:0~31, CACHE_CCNTx[0:31])
 * @return
 * return true, success. \n
 * return false, fail.
 */
hal_cache_status_t hal_cache_get_hit_count(uint32_t countIdx, uint32_t *hitCnt_h, uint32_t *hitCnt_l, uint32_t *accCnt_h, uint32_t *accCnt_l);


/**
 * @brief   This function get the number of enabled regions
 * @return
 * return Number of Regions Enabled
 */
...
#define HAL_CACHE_REMAP_ENABLE      1  /* Enable CACHE setting with remap feature. */
