 ...
 * \if MT7933
 * The MPU is an optional component in ARMv8-M architecture, that provides control of access rights on physical addresses. The processor supports a standard ARMv8 Protected Memory System Architecture model. The MPU divides the memory map into a number of regions, and defines the location, size, access permissions and memory attributes of each region. The Cortex-M33 MPU defines sixteen separate memory regions as well as a background region. The Cortex-M33 MPU memory map is unified. This means instruction accesses and data
 ...
 * |------------------------------|------------------------------------------------------------------------|
 * \if MT7933
 * |\b MPU                        | Memory Protection Unit. For more information, please refer to <a href="https://developer.arm.com/documentation/ddi0553/am/">ARMv8-M Architecture Reference Manual</a>.|
 * \else
...
typedef enum {
    HAL_MPU_REGION_IVALID = -1,                 /**< MPU region invalid */
    ...
    HAL_MPU_REGION_MAX = 8,                     /**< Max MPU region number \if MT7933 (invalid), For back-compatible for Cortex-M4 \endif */
    ...
    HAL_CM33_MPU_REGION_MAX = 16,               /**< Max MPU region number for Cortex-M33 */
...
/** @brief MPU region config structure contains the start address, the size and access rights of the region */
typedef struct {
    uint32_t mpu_region_address;                /**< MPU region start address */
    uint32_t mpu_region_size;                   /**< MPU region size */
    hal_mpu_access_permission_t mpu_region_access_permission; /**< MPU region access permission */
    bool mpu_xn;                                /**< XN attribute of MPU, if set TRUE, execution of an instruction fetched from the corresponding region is not permitted */
} hal_mpu_region_ex_config_t;
