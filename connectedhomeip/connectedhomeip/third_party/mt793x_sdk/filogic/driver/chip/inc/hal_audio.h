...
#endif /* #ifndef HAL_AUDIO_SUPPORT_MULTIPLE_STREAM_OUT */
...
#define MAXIMUM_VOLUME_LEVEL 16
...
//Please make sure Lc3FrameDuration is in accord with definition in dsp when amending.
typedef enum Lc3FrameDuration_e {
    DURATION_10000US = 0x00,
    DURATION_7500US = 0x01,
} Lc3FrameDuration;

//Please make sure SampleRate_e is in accord with definition in dsp when amending.
typedef enum SampleRate_e {
...
//Please make sure BitsPerSample_e is in accord with definition in dsp when amending.
typedef enum BitsPerSample_e {
...
/**
 * @brief     init audio driver and dsp.
 * @return    #HAL_AUDIO_STATUS_OK, if OK.
 * @param[in] sample_rate is the sampling rate of the stream.
 * @param[in] channels is the channel counts of the stream.
 * @param[in] bit_type is the bit depth of the stream.
 * @code      hal_audio_stream_out_open(48000, 2, 16);
 * @endcode
 */
hal_audio_status_t hal_audio_stream_out_open(uint32_t sample_rate, uint32_t channels, uint32_t bit_type);

/**
 * @brief     deinit dsp and aduio driver.
 * @return    #HAL_AUDIO_STATUS_OK, if OK.
 * @par       Example
 * @code      hal_audio_stream_out_close();
 * @endcode
 */
 ...
 * @return    #HAL_AUDIO_STATUS_OK, if OK.
 * @par       Example
 * @code      hal_audio_stream_out_start();
 * @endcode
 */
 ...
 * @return    #HAL_AUDIO_STATUS_OK, if OK.
 * @par       Example
 * @code      hal_audio_stream_out_stop();
 * @endcode
 */
hal_audio_status_t hal_audio_stream_out_stop(void);

/**
 * @brief     suspend processing stream.
 * @return    #HAL_AUDIO_STATUS_OK, if OK.
 * @par       Example
 * @code      hal_audio_stream_out_suspend();
 * @endcode
 */
hal_audio_status_t hal_audio_stream_out_suspend(void);

/**
 * @brief     resume processing stream.
 * @return    #HAL_AUDIO_STATUS_OK, if OK.
 * @par       Example
 * @code      hal_audio_stream_out_resume();
 * @endcode
 */
hal_audio_status_t hal_audio_stream_out_resume(void);

/**
 * @brief     set processing stream volume.
 * @return    #HAL_AUDIO_STATUS_OK, if OK.
 * @param[in] level of volume to be set.
 * @code      hal_audio_stream_out_set_volume(volume);
 * @endcode
 */
hal_audio_status_t hal_audio_stream_out_set_volume(uint16_t volume_level);

/**
 * @brief     write processing stream data.
 * @return    counts of data been written
 * @param[in] pointer of data to be written.
 * @param[in] length of data to be written.
 * @code      hal_audio_stream_out_write(buffer, datasize);
 * @endcode
 */
