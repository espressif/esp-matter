...
static uint8_t numAccepted = 0;
static uint8_t numUpdated = 0;
...
/**
 * Remove While Running In FreeRTOS
...
int shadow_sample_main(void) {
	...
	IOT_INFO("\nShadow Sample AWS IoT SDK Version %d.%d.%d-%s\n", VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH, VERSION_TAG);
   ...
   if (mqttClient == NULL) {     
    ...
    IOT_DEBUG("Shadow Sample Accepted : %d, Publish = %d", numAccepted, numPubs);

    vTaskDelay(2 * 1000);

    vPortFree(mqttClient);

    if (numAccepted == numPubs) {
        return SUCCESS;
    }
