 ...
 */
uint32_t sub_pub_sample_publishCount = 10;

void iot_subscribe_callback_handler_for_sub_pub_sample(AWS_IoT_Client *pClient, char *topicName, uint16_t topicNameLen,
...
void disconnectCallbackHandler_for_sub_pub_sample(AWS_IoT_Client *pClient, void *data) {
...
int subscribe_publish_sample_main(void) {
...
//	parseInputArgsForConnectParams(argc, argv);
   ...
   if (client == NULL) {     
	...
	IOT_INFO("\nSubscribe Publish Sample .... AWS IoT SDK Version %d.%d.%d-%s\n", VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH, VERSION_TAG);
	...
	mqttInitParams.disconnectHandler = disconnectCallbackHandler_for_sub_pub_sample;
	...
	rc = aws_iot_mqtt_subscribe(client, "sdkTest/sub", 11, QOS0,
                                iot_subscribe_callback_handler_for_sub_pub_sample, NULL);
	if(SUCCESS != rc) {
	...
	if(sub_pub_sample_publishCount != 0) {
		infinitePublishFlag = false;
	}

	while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)
		  && (sub_pub_sample_publishCount > 0 || infinitePublishFlag)) {
