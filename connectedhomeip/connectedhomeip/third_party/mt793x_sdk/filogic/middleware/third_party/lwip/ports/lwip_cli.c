...
static int8_t _cli_ip_show(char *ifname)
{
    ...
    for (int i = 0; i < LWIP_IPV6_NUM_ADDRESSES; i++) {
        if (NULL != inet6_ntoa(iface->ip6_addr[i])) {
            printf("IPv6 addr[%d]: %s%%%d (state=%x)\n",
                    i,
                    inet6_ntoa(iface->ip6_addr[i]),
                    ip6_addr_zone(ip_2_ip6(&iface->ip6_addr[i])),
                    netif_ip6_addr_state(iface, i));
        }
    }
#endif /* #if LWIP_IPV6 */
    return 0;
