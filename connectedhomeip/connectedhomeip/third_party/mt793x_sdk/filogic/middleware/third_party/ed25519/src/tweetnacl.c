...
static void randombytes(void *inBuffer, size_t inByteCount){
	uint32_t *buf = inBuffer;
    hal_trng_status_t result;

    result = hal_trng_init();

	while(inByteCount/4 > 0) {
        result = hal_trng_get_generated_random_number(buf);
        // require_action( result == HAL_TRNG_STATUS_OK, exit, err = kUnexpectedErr);
        if (result != HAL_TRNG_STATUS_OK)
            printf("WARNING: HAL TRNG Glitch \n");
		buf++;
		inByteCount -= 4;
	}
	...
	if (inByteCount > 0) {
        ...
        result = hal_trng_get_generated_random_number(&fourRandBytes);
        if(result != HAL_TRNG_STATUS_OK){
        	printf("WARNING: HAL TRNG Glitch \n");
        	return;
        }
    ...
    result = hal_trng_deinit();
    if(result != HAL_TRNG_STATUS_OK){
    	printf("WARNING: hal_trng_deinit error \n");
    	return;
    }
}
...
int crypto_sign(u8 *sm,u64 *smlen,const u8 *m,u64 n,const u8 *sk)
{
  ...
  p_gf = (gf *)os_malloc(4* sizeof(gf));
  if (p_gf == NULL){
    return 0;
  }
  os_memset(p_gf,0,4*sizeof(gf));
...
int crypto_sign_open(u8 *m,u64 *mlen,const u8 *sm,u64 n,const u8 *pk)
{
  ...
  p = (gf *)os_malloc(4* sizeof(gf));
  if (p == NULL){
    return 0;
  }
  os_memset(p,0,4*sizeof(gf));

  q = (gf *)os_malloc(4* sizeof(gf));
  if (q == NULL){
    if (p != NULL){
      os_free(p);
      p = NULL;
