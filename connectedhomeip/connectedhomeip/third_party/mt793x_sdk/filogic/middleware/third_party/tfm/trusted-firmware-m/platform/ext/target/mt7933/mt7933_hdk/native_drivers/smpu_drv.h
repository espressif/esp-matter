...
enum smpu_sec_attr_t {
    SMPU_SEC_ATTR_SRW_NSRW = 0,     /*!< Non-secure attribute */
    SMPU_SEC_ATTR_SRW_NSB,  /*!< Secure attribute */
    SMPU_SEC_ATTR_SRW_NSRO
};

/* What can happen when trying to do an illegal memory access */
enum smpu_sec_resp_t {
...
struct smpu_dev_cfg_t {
    const uint32_t base;  /*!< MPC base address */
};

/* ARM MPC SIE 200 device data structure */
struct smpu_dev_data_t {
    const struct smpu_memory_range_t** range_list;  /*!< Array of pointers
                                                               to memory ranges
                                                               controlled by
                                                               the MPC */
    ...
    uint16_t cur_region;      /*!< 32 bits alignment */
};

/* ARM MPC SIE 200 device structure */
struct smpu_dev_t {
    const struct smpu_dev_cfg_t* const cfg;  /*!< MPC configuration */
    struct smpu_dev_data_t* const data;      /*!< MPC data */
};

/**
 * \brief Initializes a MPC device.
 *
 * \param[in] dev            MPC device \ref smpu_dev_t
 * \param[in] range_list     List of memory ranges controller by the MPC
 *                           (\ref smpu_memory_range_t). This list can not
 *                           freed after the initializations.
 * \param[in] nbr_of_ranges  Number of memory ranges
 *
 * \return Returns error code as specified in \ref smpu_error_t
 *
 ...
 *
 * \param[in]  dev      MPC device \ref smpu_dev_t
 * \param[out] sec_rep  Configured secure response (\ref smpu_sec_resp_t).
 *
 * \return Returns error code as specified in \ref smpu_error_t
 *
 * \note This function doesn't check if dev is NULL.
