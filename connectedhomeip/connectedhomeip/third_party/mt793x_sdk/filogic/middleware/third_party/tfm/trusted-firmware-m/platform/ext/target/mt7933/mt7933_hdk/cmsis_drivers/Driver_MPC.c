 ...
 *
 * \param[in] err  Error code to translate (\ref smpu_error_t).
 *
 * \return Returns CMSIS error code.
 */
static int32_t error_trans(enum smpu_error_t err)
{
    switch(err) {
...
static struct smpu_memory_range_t MPC_ISRAM0_RANGE0_NS = {
    .base  = MPC_ISRAM0_RANGE0_BASE_NS,
    .limit = MPC_ISRAM0_RANGE0_LIMIT_NS,
    .attr  = SMPU_SEC_ATTR_SRW_NSRW
};

#define MPC_ISRAM0_RANGE_LIST_LEN  1u
static const struct smpu_memory_range_t* MPC_ISRAM0_RANGE_LIST[MPC_ISRAM0_RANGE_LIST_LEN]=
    {&MPC_ISRAM0_RANGE0_NS};

/* ISRAM0_MPC Driver wrapper functions */
static int32_t ISRAM0_MPC_Initialize(void)
{
    enum smpu_error_t ret;

    ret = smpu_init(&MPC_ISRAM0_DEV_S,
...
static int32_t ISRAM0_MPC_GetBlockSize(uint32_t* blk_size)
{
    ...
    ret = smpu_get_block_size(&MPC_ISRAM0_DEV_S, blk_size);
...
static int32_t ISRAM0_MPC_GetCtrlConfig(uint32_t* ctrl_val)
{
    ...
    ret = smpu_get_ctrl(&MPC_ISRAM0_DEV_S, ctrl_val);
...
static int32_t ISRAM0_MPC_SetCtrlConfig(uint32_t ctrl)
{
    ...
    ret = smpu_set_ctrl(&MPC_ISRAM0_DEV_S, ctrl);
                                          ...
                                          ARM_MPC_SEC_ATTR* attr)
{
    ...
    ret = smpu_get_region_config(&MPC_ISRAM0_DEV_S, base, limit,
                                       ...
                                       ARM_MPC_SEC_ATTR attr)
{
    ...
    ret = smpu_config_region(&MPC_ISRAM0_DEV_S, base, limit,
                                   (enum smpu_sec_attr_t)attr);
...
static int32_t ISRAM0_MPC_EnableInterrupt(void)
{
    ...
    ret = smpu_irq_enable(&MPC_ISRAM0_DEV_S);
...
{
    smpu_irq_disable(&MPC_ISRAM0_DEV_S);
}


static void ISRAM0_MPC_ClearInterrupt(void)
{
    smpu_clear_irq(&MPC_ISRAM0_DEV_S);
}

static uint32_t ISRAM0_MPC_InterruptState(void)
{
    return smpu_irq_state(&MPC_ISRAM0_DEV_S);
}

static int32_t ISRAM0_MPC_LockDown(void)
{
    return smpu_lock_down(&MPC_ISRAM0_DEV_S);
}
...
static struct smpu_memory_range_t MPC_ISRAM1_RANGE_S = {
    .base  = MPC_ISRAM1_RANGE_BASE_S,
    .limit = MPC_ISRAM1_RANGE_LIMIT_S,
    .attr  = MPC_SIE200_SEC_ATTR_SECURE
};

static struct smpu_memory_range_t MPC_ISRAM1_RANGE_NS = {
...
static const struct smpu_memory_range_t* MPC_ISRAM1_RANGE_LIST[MPC_ISRAM1_RANGE_LIST_LEN]=
...
static int32_t ISRAM1_MPC_Initialize(void)
{
    ...
    ret = smpu_init(&MPC_ISRAM1_DEV_S,
...
static int32_t ISRAM1_MPC_GetBlockSize(uint32_t* blk_size)
{
    ...
    ret = smpu_get_block_size(&MPC_ISRAM1_DEV_S, blk_size);
...
static int32_t ISRAM1_MPC_GetCtrlConfig(uint32_t* ctrl_val)
{
    ...
    ret = smpu_get_ctrl(&MPC_ISRAM1_DEV_S, ctrl_val);
...
static int32_t ISRAM1_MPC_SetCtrlConfig(uint32_t ctrl)
{
    ...
    ret = smpu_set_ctrl(&MPC_ISRAM1_DEV_S, ctrl);
                                          ...
                                          ARM_MPC_SEC_ATTR* attr)
{
    ...
    ret = smpu_get_region_config(&MPC_ISRAM1_DEV_S, base, limit,
                                       (enum smpu_sec_attr_t*)attr);
                                       ...
                                       ARM_MPC_SEC_ATTR attr)
{
    ...
    ret = smpu_config_region(&MPC_ISRAM1_DEV_S, base, limit,
                                   (enum smpu_sec_attr_t)attr);
...
static int32_t ISRAM1_MPC_EnableInterrupt(void)
{
    ...
    ret = smpu_irq_enable(&MPC_ISRAM1_DEV_S);
...
{
    smpu_irq_disable(&MPC_ISRAM1_DEV_S);
}


static void ISRAM1_MPC_ClearInterrupt(void)
{
    smpu_clear_irq(&MPC_ISRAM1_DEV_S);
}

static uint32_t ISRAM1_MPC_InterruptState(void)
{
    return smpu_irq_state(&MPC_ISRAM1_DEV_S);
}

static int32_t ISRAM1_MPC_LockDown(void)
{
    return smpu_lock_down(&MPC_ISRAM1_DEV_S);
}
...
static struct smpu_memory_range_t MPC_ISRAM2_RANGE_S = {
    .base  = MPC_ISRAM2_RANGE_BASE_S,
    .limit = MPC_ISRAM2_RANGE_LIMIT_S,
    .attr  = MPC_SIE200_SEC_ATTR_SECURE
};

static struct smpu_memory_range_t MPC_ISRAM2_RANGE_NS = {
...
static const struct smpu_memory_range_t* MPC_ISRAM2_RANGE_LIST[MPC_ISRAM2_RANGE_LIST_LEN]=
...
static int32_t ISRAM2_MPC_Initialize(void)
{
    ...
    ret = smpu_init(&MPC_ISRAM2_DEV_S,
...
static int32_t ISRAM2_MPC_GetBlockSize(uint32_t* blk_size)
{
    ...
    ret = smpu_get_block_size(&MPC_ISRAM2_DEV_S, blk_size);
...
static int32_t ISRAM2_MPC_GetCtrlConfig(uint32_t* ctrl_val)
{
    ...
    ret = smpu_get_ctrl(&MPC_ISRAM2_DEV_S, ctrl_val);
...
static int32_t ISRAM2_MPC_SetCtrlConfig(uint32_t ctrl)
{
    ...
    ret = smpu_set_ctrl(&MPC_ISRAM2_DEV_S, ctrl);
                                          ...
                                          ARM_MPC_SEC_ATTR* attr)
{
    ...
    ret = smpu_get_region_config(&MPC_ISRAM2_DEV_S, base, limit,
                                       (enum smpu_sec_attr_t*)attr);
                                       ...
                                       ARM_MPC_SEC_ATTR attr)
{
    ...
    ret = smpu_config_region(&MPC_ISRAM2_DEV_S, base, limit,
                                   (enum smpu_sec_attr_t)attr);
...
static int32_t ISRAM2_MPC_EnableInterrupt(void)
{
    ...
    ret = smpu_irq_enable(&MPC_ISRAM2_DEV_S);
...
{
    smpu_irq_disable(&MPC_ISRAM2_DEV_S);
}


static void ISRAM2_MPC_ClearInterrupt(void)
{
    smpu_clear_irq(&MPC_ISRAM2_DEV_S);
}

static uint32_t ISRAM2_MPC_InterruptState(void)
{
    return smpu_irq_state(&MPC_ISRAM2_DEV_S);
}

static int32_t ISRAM2_MPC_LockDown(void)
{
    return smpu_lock_down(&MPC_ISRAM2_DEV_S);
}
...
static struct smpu_memory_range_t MPC_ISRAM3_RANGE_S = {
    .base  = MPC_ISRAM3_RANGE_BASE_S,
    .limit = MPC_ISRAM3_RANGE_LIMIT_S,
    .attr  = MPC_SIE200_SEC_ATTR_SECURE
};

static struct smpu_memory_range_t MPC_ISRAM3_RANGE_NS = {
...
static const struct smpu_memory_range_t* MPC_ISRAM3_RANGE_LIST[MPC_ISRAM3_RANGE_LIST_LEN]=
...
static int32_t ISRAM3_MPC_Initialize(void)
{
    ...
    ret = smpu_init(&MPC_ISRAM3_DEV_S,
...
static int32_t ISRAM3_MPC_GetBlockSize(uint32_t* blk_size)
{
    ...
    ret = smpu_get_block_size(&MPC_ISRAM3_DEV_S, blk_size);
...
static int32_t ISRAM3_MPC_GetCtrlConfig(uint32_t* ctrl_val)
{
    ...
    ret = smpu_get_ctrl(&MPC_ISRAM3_DEV_S, ctrl_val);
...
static int32_t ISRAM3_MPC_SetCtrlConfig(uint32_t ctrl)
{
    ...
    ret = smpu_set_ctrl(&MPC_ISRAM3_DEV_S, ctrl);
                                          ...
                                          ARM_MPC_SEC_ATTR* attr)
{
    ...
    ret = smpu_get_region_config(&MPC_ISRAM3_DEV_S, base, limit,
                                       (enum smpu_sec_attr_t*)attr);
                                       ...
                                       ARM_MPC_SEC_ATTR attr)
{
    ...
    ret = smpu_config_region(&MPC_ISRAM3_DEV_S, base, limit,
                                   (enum smpu_sec_attr_t)attr);
...
static int32_t ISRAM3_MPC_EnableInterrupt(void)
{
    ...
    ret = smpu_irq_enable(&MPC_ISRAM3_DEV_S);
...
{
    smpu_irq_disable(&MPC_ISRAM3_DEV_S);
}


static void ISRAM3_MPC_ClearInterrupt(void)
{
    smpu_clear_irq(&MPC_ISRAM3_DEV_S);
}

static uint32_t ISRAM3_MPC_InterruptState(void)
{
    return smpu_irq_state(&MPC_ISRAM3_DEV_S);
}

static int32_t ISRAM3_MPC_LockDown(void)
{
    return smpu_lock_down(&MPC_ISRAM3_DEV_S);
}
...
static struct smpu_memory_range_t MPC_CODE_SRAM1_RANGE0_NS = {
    .base  = MPC_CODE_SRAM1_RANGE0_BASE_NS,
    .limit = MPC_CODE_SRAM1_RANGE0_LIMIT_NS,
    .attr  = SMPU_SEC_ATTR_SRW_NSRW
};

static struct smpu_memory_range_t MPC_CODE_SRAM1_RANGE1_NS = {
    .base  = MPC_CODE_SRAM1_RANGE1_BASE_NS,
    .limit = MPC_CODE_SRAM1_RANGE1_LIMIT_NS,
    .attr  = SMPU_SEC_ATTR_SRW_NSRW
};

#define MPC_CODE_SRAM1_RANGE_LIST_LEN  2u
static const struct  smpu_memory_range_t* MPC_CODE_SRAM1_RANGE_LIST[MPC_CODE_SRAM1_RANGE_LIST_LEN]=
    {&MPC_CODE_SRAM1_RANGE0_NS, &MPC_CODE_SRAM1_RANGE1_NS};
...
static int32_t SRAM1_MPC_Initialize(void)
{
    ...
    ret = smpu_init(&MPC_CODE_SRAM1_DEV_S,
...
static int32_t SRAM1_MPC_GetBlockSize(uint32_t* blk_size)
{
    ...
    ret = smpu_get_block_size(&MPC_CODE_SRAM1_DEV_S, blk_size);
...
static int32_t SRAM1_MPC_GetCtrlConfig(uint32_t* ctrl_val)
{
    ...
    ret = smpu_get_ctrl(&MPC_CODE_SRAM1_DEV_S, ctrl_val);
...
static int32_t SRAM1_MPC_SetCtrlConfig(uint32_t ctrl)
{
    ...
    ret = smpu_set_ctrl(&MPC_CODE_SRAM1_DEV_S, ctrl);
                                         ...
                                         ARM_MPC_SEC_ATTR* attr)
{
    ...
    ret = smpu_get_region_config(&MPC_CODE_SRAM1_DEV_S, base, limit,
                                       (enum smpu_sec_attr_t*)attr);
                                      ...
                                      ARM_MPC_SEC_ATTR attr)
{
    ...
    ret = smpu_config_region(&MPC_CODE_SRAM1_DEV_S, base, limit,
                                   (enum smpu_sec_attr_t)attr);
...
static int32_t SRAM1_MPC_EnableInterrupt(void)
{
    ...
    ret = smpu_irq_enable(&MPC_CODE_SRAM1_DEV_S);
...
{
    smpu_irq_disable(&MPC_CODE_SRAM1_DEV_S);
}


static void SRAM1_MPC_ClearInterrupt(void)
{
    smpu_clear_irq(&MPC_CODE_SRAM1_DEV_S);
}

static uint32_t SRAM1_MPC_InterruptState(void)
{
    return smpu_irq_state(&MPC_CODE_SRAM1_DEV_S);
}

static int32_t SRAM1_MPC_LockDown(void)
{
    return smpu_lock_down(&MPC_CODE_SRAM1_DEV_S);
}
...
static struct smpu_memory_range_t MPC_CODE_SRAM2_RANGE1_NS = {
    .base  = MPC_CODE_SRAM1_RANGE0_BASE_NS,
    .limit = MPC_CODE_SRAM1_RANGE0_LIMIT_NS,
    .attr  = SMPU_SEC_ATTR_SRW_NSRW
};

#define MPC_CODE_SRAM2_RANGE_LIST_LEN  1u
static const struct  smpu_memory_range_t* MPC_CODE_SRAM2_RANGE_LIST[MPC_CODE_SRAM2_RANGE_LIST_LEN]=
    {&MPC_CODE_SRAM2_RANGE1_NS};
...
static int32_t SRAM2_MPC_Initialize(void)
{
    ...
    ret = smpu_init(&MPC_CODE_SRAM2_DEV_S,
...
static int32_t SRAM2_MPC_GetBlockSize(uint32_t* blk_size)
{
    ...
    ret = smpu_get_block_size(&MPC_CODE_SRAM2_DEV_S, blk_size);
...
static int32_t SRAM2_MPC_GetCtrlConfig(uint32_t* ctrl_val)
{
    ...
    ret = smpu_get_ctrl(&MPC_CODE_SRAM2_DEV_S, ctrl_val);
...
static int32_t SRAM2_MPC_SetCtrlConfig(uint32_t ctrl)
{
    ...
    ret = smpu_set_ctrl(&MPC_CODE_SRAM2_DEV_S, ctrl);
                                         ...
                                         ARM_MPC_SEC_ATTR* attr)
{
    ...
    ret = smpu_get_region_config(&MPC_CODE_SRAM2_DEV_S, base, limit,
                                       (enum smpu_sec_attr_t*)attr);
                                      ...
                                      ARM_MPC_SEC_ATTR attr)
{
    ...
    ret = smpu_config_region(&MPC_CODE_SRAM2_DEV_S, base, limit,
                                   (enum smpu_sec_attr_t)attr);
...
static int32_t SRAM2_MPC_EnableInterrupt(void)
{
    ...
    ret = smpu_irq_enable(&MPC_CODE_SRAM2_DEV_S);
...
{
    smpu_irq_disable(&MPC_CODE_SRAM2_DEV_S);
}


static void SRAM2_MPC_ClearInterrupt(void)
{
    smpu_clear_irq(&MPC_CODE_SRAM2_DEV_S);
}

static uint32_t SRAM2_MPC_InterruptState(void)
{
    return smpu_irq_state(&MPC_CODE_SRAM2_DEV_S);
}

static int32_t SRAM2_MPC_LockDown(void)
{
    return smpu_lock_down(&MPC_CODE_SRAM2_DEV_S);
}
...
static struct smpu_memory_range_t MPC_CODE_SRAM3_RANGE_S = {
    .base  = MPC_CODE_SRAM3_RANGE_BASE_S,
    .limit = MPC_CODE_SRAM3_RANGE_LIMIT_S,
    .attr  = MPC_SIE200_SEC_ATTR_SECURE
};

static struct smpu_memory_range_t MPC_CODE_SRAM3_RANGE_NS = {
...
static const struct  smpu_memory_range_t* MPC_CODE_SRAM3_RANGE_LIST[MPC_CODE_SRAM3_RANGE_LIST_LEN]=
...
static int32_t SRAM3_MPC_Initialize(void)
{
    ...
    ret = smpu_init(&MPC_CODE_SRAM3_DEV_S,
...
static int32_t SRAM3_MPC_GetBlockSize(uint32_t* blk_size)
{
    ...
    ret = smpu_get_block_size(&MPC_CODE_SRAM3_DEV_S, blk_size);
...
static int32_t SRAM3_MPC_GetCtrlConfig(uint32_t* ctrl_val)
{
    ...
    ret = smpu_get_ctrl(&MPC_CODE_SRAM3_DEV_S, ctrl_val);
...
static int32_t SRAM3_MPC_SetCtrlConfig(uint32_t ctrl)
{
    ...
    ret = smpu_set_ctrl(&MPC_CODE_SRAM3_DEV_S, ctrl);
                                         ...
                                         ARM_MPC_SEC_ATTR* attr)
{
    ...
    ret = smpu_get_region_config(&MPC_CODE_SRAM3_DEV_S, base, limit,
                                       (enum smpu_sec_attr_t*)attr);
                                      ...
                                      ARM_MPC_SEC_ATTR attr)
{
    ...
    ret = smpu_config_region(&MPC_CODE_SRAM3_DEV_S, base, limit,
                                   (enum smpu_sec_attr_t)attr);
...
static int32_t SRAM3_MPC_EnableInterrupt(void)
{
    ...
    ret = smpu_irq_enable(&MPC_CODE_SRAM3_DEV_S);
...
{
    smpu_irq_disable(&MPC_CODE_SRAM3_DEV_S);
}


static void SRAM3_MPC_ClearInterrupt(void)
{
    smpu_clear_irq(&MPC_CODE_SRAM3_DEV_S);
}

static uint32_t SRAM3_MPC_InterruptState(void)
{
    return smpu_irq_state(&MPC_CODE_SRAM3_DEV_S);
}

static int32_t SRAM3_MPC_LockDown(void)
{
    return smpu_lock_down(&MPC_CODE_SRAM3_DEV_S);
}
