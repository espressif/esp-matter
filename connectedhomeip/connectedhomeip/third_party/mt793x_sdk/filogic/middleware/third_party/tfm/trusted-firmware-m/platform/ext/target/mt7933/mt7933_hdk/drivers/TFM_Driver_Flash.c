                              ...
                              uint32_t offset)
{
    ...
    flash_limit = (flash_dev->data->sector_count * flash_dev->data->sector_size) + flash_dev->memory_base -1;
...
static struct arm_flash_dev_t TFM_FLASH0_DEV = {
    //use non-cached flash base addr to compatible with hal_flash internal
#if (__DOMAIN_NS == 1)
    .memory_base = NS_ROM_BASE,
#else
    .memory_base = S_ROM_BASE,
#endif /* __DOMAIN_NS == 1 */
    .data        = &(ARM_FLASH0_DEV_DATA)};

struct arm_flash_dev_t *FLASH0_DEV = &TFM_FLASH0_DEV;
...
static int32_t Flash_ReadData(uint32_t addr, void *data, uint32_t cnt)
{
    ...
    rc = hal_flash_read(addr, (uint8_t*)data, cnt);
    if (rc != HAL_FLASH_STATUS_OK) {
                                     ...
                                     uint32_t cnt)
{
    ...
    rc = hal_flash_write(addr, data, cnt);
    if (rc != HAL_FLASH_STATUS_OK) {
...
static int32_t Flash_EraseSector(uint32_t addr)
{
    ...
    */
    rc = hal_flash_erase(addr, HAL_FLASH_BLOCK_4K);
    if (rc != HAL_FLASH_STATUS_OK) {
        printf("hal_flash_erase fail: %d\n", rc);
        return ARM_DRIVER_ERROR_SPECIFIC;
    }
    return ARM_DRIVER_OK;
}
