...
#if (RTE_MRAM_MPC)
/* Ranges controlled by this MRAM_MPC */
static const struct mpc_sie200_memory_range_t MPC_MRAM_RANGE_S = {
    .base         = MPC_MRAM_RANGE_BASE_S,
    .limit        = MPC_MRAM_RANGE_LIMIT_S,
    .range_offset = 0,
    .attr         = MPC_SIE200_SEC_ATTR_SECURE
};

static const struct mpc_sie200_memory_range_t MPC_MRAM_RANGE_NS = {
    .base         = MPC_MRAM_RANGE_BASE_NS,
    .limit        = MPC_MRAM_RANGE_LIMIT_NS,
    .range_offset = 0,
    .attr         = MPC_SIE200_SEC_ATTR_NONSECURE
};

#define MPC_MRAM_RANGE_LIST_LEN  2u
static const struct  mpc_sie200_memory_range_t* MPC_MRAM_RANGE_LIST[MPC_MRAM_RANGE_LIST_LEN]=
    {&MPC_MRAM_RANGE_S, &MPC_MRAM_RANGE_NS};

/* MRAM_MPC Driver wrapper functions */
static int32_t MRAM_MPC_Initialize(void)
{
    enum mpc_sie200_error_t ret;

    ret = mpc_sie200_init(&MPC_MRAM_DEV,
                          MPC_MRAM_RANGE_LIST,
                          MPC_MRAM_RANGE_LIST_LEN);

    return error_trans(ret);
}

static int32_t MRAM_MPC_Uninitialize(void)
{
    /* Nothing to be done */
    return ARM_DRIVER_OK;
}

static int32_t MRAM_MPC_GetBlockSize(uint32_t* blk_size)
{
    enum mpc_sie200_error_t ret;

    ret = mpc_sie200_get_block_size(&MPC_MRAM_DEV, blk_size);

    return error_trans(ret);
}

static int32_t MRAM_MPC_GetCtrlConfig(uint32_t* ctrl_val)
{
    enum mpc_sie200_error_t ret;

    ret = mpc_sie200_get_ctrl(&MPC_MRAM_DEV, ctrl_val);

    return error_trans(ret);
}

static int32_t MRAM_MPC_SetCtrlConfig(uint32_t ctrl)
{
    enum mpc_sie200_error_t ret;

    ret = mpc_sie200_set_ctrl(&MPC_MRAM_DEV, ctrl);

    return error_trans(ret);
}

static int32_t MRAM_MPC_GetRegionConfig(uintptr_t base,
                                         uintptr_t limit,
                                         ARM_MPC_SEC_ATTR* attr)
{
    enum mpc_sie200_error_t ret;

    ret = mpc_sie200_get_region_config(&MPC_MRAM_DEV, base, limit,
                                       (enum mpc_sie200_sec_attr_t*)attr);

    return error_trans(ret);
}

static int32_t MRAM_MPC_ConfigRegion(uintptr_t base,
                                      uintptr_t limit,
                                      ARM_MPC_SEC_ATTR attr)
{
    enum mpc_sie200_error_t ret;

    ret = mpc_sie200_config_region(&MPC_MRAM_DEV, base, limit,
                                   (enum mpc_sie200_sec_attr_t)attr);

    return error_trans(ret);
}

static int32_t MRAM_MPC_EnableInterrupt(void)
{
    enum mpc_sie200_error_t ret;

    ret = mpc_sie200_irq_enable(&MPC_MRAM_DEV);

    return error_trans(ret);
}

static void MRAM_MPC_DisableInterrupt(void)
{
    mpc_sie200_irq_disable(&MPC_MRAM_DEV);
}


static void MRAM_MPC_ClearInterrupt(void)
{
    mpc_sie200_clear_irq(&MPC_MRAM_DEV);
}

static uint32_t MRAM_MPC_InterruptState(void)
{
    return mpc_sie200_irq_state(&MPC_MRAM_DEV);
}

static int32_t MRAM_MPC_LockDown(void)
{
    return mpc_sie200_lock_down(&MPC_MRAM_DEV);
}

/* MRAM_MPC Driver CMSIS access structure */
extern ARM_DRIVER_MPC Driver_MRAM_MPC;
ARM_DRIVER_MPC Driver_MRAM_MPC = {
    .GetVersion       = ARM_MPC_GetVersion,
    .Initialize       = MRAM_MPC_Initialize,
    .Uninitialize     = MRAM_MPC_Uninitialize,
    .GetBlockSize     = MRAM_MPC_GetBlockSize,
    .GetCtrlConfig    = MRAM_MPC_GetCtrlConfig,
    .SetCtrlConfig    = MRAM_MPC_SetCtrlConfig,
    .ConfigRegion     = MRAM_MPC_ConfigRegion,
    .GetRegionConfig  = MRAM_MPC_GetRegionConfig,
    .EnableInterrupt  = MRAM_MPC_EnableInterrupt,
    .DisableInterrupt = MRAM_MPC_DisableInterrupt,
    .ClearInterrupt   = MRAM_MPC_ClearInterrupt,
    .InterruptState   = MRAM_MPC_InterruptState,
    .LockDown         = MRAM_MPC_LockDown,
};
#endif /* RTE_MRAM_MPC */
