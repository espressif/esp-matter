...
{
    #define CDC_LOCAL_BUF_SIZE (64)
    uint32_t len_data = 0;
    uint8_t data[CDC_LOCAL_BUF_SIZE];

    for(int i = 0; i < usbd_cdc_acm_num; ++i) {
        len_data = USBD_CDC_ACM_DataFree(i);
        ...
        if (len_data) {
            len_data = uart_read_data(i, data, len_data);
        } else {
            // Eason +
            // Drop mulitple data here to revant uart isr drop data byte by byte..
            uart_read_data(i, data, CDC_LOCAL_BUF_SIZE);
        }

        if (len_data) {
            if (USBD_CDC_ACM_DataSend(i, data , len_data)) {
                main_blink_cdc_led(MAIN_LED_FLASH);
            }
        }

        len_data = uart_write_free(i);
        ...
        if (len_data) {
            len_data = USBD_CDC_ACM_DataRead(i, data, len_data);
        }

        if (len_data) {
            if (uart_write_data(i, data, len_data)) {
                main_blink_cdc_led(MAIN_LED_FLASH);
            }
        }
    } // for
