...
#define USBD_CDC_ACM_ENABLE  0
    ...
    U8 usbd_cdc_acm_cif_num[USBD_CDC_ACM_NUM] = {0}; //assigned during runtime init
    U8 usbd_cdc_acm_dif_num[USBD_CDC_ACM_NUM] = {0}; //assigned during runtime init
    const U8 usbd_cdc_acm_ep_intin[USBD_CDC_ACM_NUM] = {USBD_CDC_ACM_EP_INTIN};
    const U8 usbd_cdc_acm_ep_bulkin[USBD_CDC_ACM_NUM] = {USBD_CDC_ACM_EP_BULKIN};
    const U8 usbd_cdc_acm_ep_bulkout[USBD_CDC_ACM_NUM] = {USBD_CDC_ACM_EP_BULKOUT};
    const U16 usbd_cdc_acm_num = USBD_CDC_ACM_NUM;
...
const U16 usbd_cdc_acm_maxpacketsize1[2] = {USBD_CDC_ACM_WMAXPACKETSIZE1, USBD_CDC_ACM_HS_WMAXPACKETSIZE1};
    U8 USBD_CDC_ACM_SendBufPool[USBD_CDC_ACM_NUM * USBD_CDC_ACM_SENDBUF_SIZE];
    U8 USBD_CDC_ACM_ReceiveBufPool[USBD_CDC_ACM_NUM * USBD_CDC_ACM_RECEIVEBUF_SIZE];
    U8 *USBD_CDC_ACM_SendBuf[USBD_CDC_ACM_NUM];
    U8 *USBD_CDC_ACM_ReceiveBuf[USBD_CDC_ACM_NUM];
#endif
...
#if    (USBD_CDC_ACM_ENABLE)
void USBD_EndPoint3(U32 event) { USBD_CDC_ACM_EP_INTIN_Event(0, event); }
void USBD_EndPoint5(U32 event) { USBD_CDC_ACM_EP_INTIN_Event(1, event); }
void USBD_EndPoint7(U32 event) { USBD_CDC_ACM_EP_INTIN_Event(2, event); }
void USBD_EndPoint4(U32 event) { USBD_CDC_ACM_EP_BULK_Event(0, event); }
void USBD_EndPoint6(U32 event) { USBD_CDC_ACM_EP_BULK_Event(1, event); }
void USBD_EndPoint8(U32 event) { USBD_CDC_ACM_EP_BULK_Event(2, event); }
#else
...
#define USBD_HID_DESC_OFS                 (USB_CONFIGUARTION_DESC_SIZE + USB_INTERFACE_DESC_SIZE                                                + \
                                           USBD_MSC_ENABLE * USBD_MSC_DESC_LEN + \
                                           USBD_CDC_ACM_ENABLE * (USBD_CDC_ACM_DESC_LEN * USBD_CDC_ACM_NUM))

#define USBD_WTOTALLENGTH_MAX              (USB_CONFIGUARTION_DESC_SIZE +                 \
                                           (USBD_CDC_ACM_DESC_LEN * USBD_CDC_ACM_NUM) *  USBD_CDC_ACM_ENABLE + \
...
#define CDC_ACM_DESC_IAD(first,num_of_ifs,fd)  /* CDC: Interface Association Descriptor */                     \
  USB_INTERFACE_ASSOC_DESC_SIZE,        /* bLength */                                                       \
  USB_INTERFACE_ASSOCIATION_DESCRIPTOR_TYPE,  /* bDescriptorType */                                         \
 (first),                               /* bFirstInterface */                                               \
 (num_of_ifs),                          /* bInterfaceCount */                                               \
  ...
  USBD_CDC_ACM_CIF_STR_NUM+(fd*2),             /* iFunction */                                                     \

#define CDC_ACM_DESC_IF0(fd)                                                                                        \
/* Interface, Alternate Setting 0, CDC Class */                                                             \
  ...
  USBD_CDC_ACM_CIF_STR_NUM+(fd*2),             /* iInterface: */                                                   \
                                                                                                            \
...
#define CDC_ACM_EP_IF0(fd)                  /* CDC Endpoints for Interface 0 for Low-speed/Full-speed */        \
/* Endpoint, EP Interrupt IN */         /* event notification (optional) */                                 \
  USB_ENDPOINT_DESC_SIZE,               /* bLength */                                                       \
  USB_ENDPOINT_DESCRIPTOR_TYPE,         /* bDescriptorType */                                               \
  USB_ENDPOINT_IN(usbd_cdc_acm_ep_intin[fd]),/* bEndpointAddress */                                             \
  USB_ENDPOINT_TYPE_INTERRUPT,          /* bmAttributes */                                                  \
  WBVAL(USBD_CDC_ACM_WMAXPACKETSIZE),   /* wMaxPacketSize */                                                \
  USBD_CDC_ACM_BINTERVAL,               /* bInterval */

#define CDC_ACM_EP_IF0_HS(fd)               /* CDC Endpoints for Interface 0 for High-speed */                  \
/* Endpoint, EP Interrupt IN */         /* event notification (optional) */                                 \
  USB_ENDPOINT_DESC_SIZE,               /* bLength */                                                       \
  USB_ENDPOINT_DESCRIPTOR_TYPE,         /* bDescriptorType */                                               \
  USB_ENDPOINT_IN(usbd_cdc_acm_ep_intin[fd]),/* bEndpointAddress */                                             \
...
/* Interface, Alternate Setting 0, Data class interface descriptor*/                                        \
  ...
  USBD_CDC_ACM_DIF_STR_NUM+(fd*2),             /* iInterface */

#define CDC_ACM_EP_IF1(fd)                  /* CDC Endpoints for Interface 1 for Low-speed/Full-speed */        \
/* Endpoint, EP Bulk OUT */                                                                                 \
  USB_ENDPOINT_DESC_SIZE,               /* bLength */                                                       \
  USB_ENDPOINT_DESCRIPTOR_TYPE,         /* bDescriptorType */                                               \
  USB_ENDPOINT_OUT(usbd_cdc_acm_ep_bulkout[fd]),/* bEndpointAddress */                                          \
  USB_ENDPOINT_TYPE_BULK,               /* bmAttributes */                                                  \
...
/* Endpoint, EP Bulk IN */                                                                                  \
  USB_ENDPOINT_DESC_SIZE,               /* bLength */                                                       \
  USB_ENDPOINT_DESCRIPTOR_TYPE,         /* bDescriptorType */                                               \
  USB_ENDPOINT_IN(usbd_cdc_acm_ep_bulkin[fd]),/* bEndpointAddress */                                            \
  USB_ENDPOINT_TYPE_BULK,               /* bmAttributes */                                                  \
  WBVAL(USBD_CDC_ACM_WMAXPACKETSIZE1),  /* wMaxPacketSize */                                                \
  0x00,                                 /* bInterval: ignore for Bulk transfer */

#define CDC_ACM_EP_IF1_HS(fd)               /* CDC Endpoints for Interface 1 for High-speed */                  \
/* Endpoint, EP Bulk OUT */                                                                                 \
  USB_ENDPOINT_DESC_SIZE,               /* bLength */                                                       \
  USB_ENDPOINT_DESCRIPTOR_TYPE,         /* bDescriptorType */                                               \
  USB_ENDPOINT_OUT(usbd_cdc_acm_ep_bulkout[fd]),/* bEndpointAddress */                                          \
...
__WEAK \
U8 USBD_ConfigDescriptor[512] = { 0 };
...
/*   All Descriptors (Configuration, Interface, Endpoint, Class, Vendor) */
__WEAK \
U8 USBD_ConfigDescriptor_HS[512] = { 0 };
...
const struct {
  ...
  #if USBD_CDC_ACM_NUM >= 2
    USBD_STR_DEF(CDC_ACM_CIF_STRDESC1);
    USBD_STR_DEF(CDC_ACM_DIF_STRDESC1);
  #endif
  #if USBD_CDC_ACM_NUM >= 3
    USBD_STR_DEF(CDC_ACM_CIF_STRDESC2);
    USBD_STR_DEF(CDC_ACM_DIF_STRDESC2);
  #endif
  #if USBD_CDC_ACM_NUM >= 4
...
= {
  ...
  #if USBD_CDC_ACM_NUM >= 2
    USBD_STR_VAL(CDC_ACM_CIF_STRDESC1),
    USBD_STR_VAL(CDC_ACM_DIF_STRDESC1),
  #endif
  #if USBD_CDC_ACM_NUM >= 3
...
static U16 acm_cdc_desc_fill(U8 fd, U8 * config_desc, U8 * config_desc_hs, U8 if_num) {
    U8 * pD = 0;
    const U8 cdc_desc[] = {
    #if (USBD_MULTI_IF)
        CDC_ACM_DESC_IAD(0, 2, fd)
    #endif
        CDC_ACM_DESC_IF0(fd)
        CDC_ACM_EP_IF0(fd)
        CDC_ACM_DESC_IF1(fd)
    ...
    #endif
        CDC_ACM_DESC_IF0(fd)
        CDC_ACM_EP_IF0_HS(fd)
        CDC_ACM_DESC_IF1(fd)
        CDC_ACM_EP_IF1_HS(fd)
    };
     pD = config_desc_hs;
...
#if (USBD_CDC_ACM_ENABLE)
    usbd_cdc_acm_cif_num[0] = if_num++;
    usbd_cdc_acm_dif_num[0] = if_num++;
    desc_ptr += acm_cdc_desc_fill(0, &USBD_ConfigDescriptor[desc_ptr], &USBD_ConfigDescriptor_HS[desc_ptr], usbd_cdc_acm_cif_num[0]);
  #if USBD_CDC_ACM_NUM >= 2
    usbd_cdc_acm_cif_num[1] = if_num++;
    usbd_cdc_acm_dif_num[1] = if_num++;
    desc_ptr += acm_cdc_desc_fill(1, &USBD_ConfigDescriptor[desc_ptr], &USBD_ConfigDescriptor_HS[desc_ptr], usbd_cdc_acm_cif_num[1]);
  #endif
  #if USBD_CDC_ACM_NUM >= 3
    usbd_cdc_acm_cif_num[2] = if_num++;
    usbd_cdc_acm_dif_num[2] = if_num++;
    desc_ptr += acm_cdc_desc_fill(2, &USBD_ConfigDescriptor[desc_ptr], &USBD_ConfigDescriptor_HS[desc_ptr], usbd_cdc_acm_cif_num[2]);
  #endif
  #if USBD_CDC_ACM_NUM >= 4
    usbd_cdc_acm_cif_num[3] = if_num++;
    usbd_cdc_acm_dif_num[3] = if_num++;
    desc_ptr += acm_cdc_desc_fill(3, &USBD_ConfigDescriptor[desc_ptr], &USBD_ConfigDescriptor_HS[desc_ptr], usbd_cdc_acm_cif_num[3]);
  #endif
