...
mxmlDelete(mxml_node_t *node)		/* I - Node to delete */
{
...
#if DEBUG > 1
   fprintf(stderr, "delete parent(parent node=%p)\n", (node->parent)?(node->parent):(NULL));
#endif
  mxmlRemove(node);
...
#if DEBUG > 1
   fprintf(stderr, "delete child(child node=%p)\n", (node->child)?(node->child):(NULL));
#endif
  while (node->child)
  ...
  */
...
#if DEBUG > 1
   fprintf(stderr, "delete data(node type=%d)\n", node->type);
#endif
  ...
  switch (node->type)
  {
    case MXML_ELEMENT :
        ...
        {
#if DEBUG > 1
          fprintf(stderr, "free element name(element.name=%s)\n", node->value.element.name);
#endif
	...
	{
#if DEBUG > 1
          fprintf(stderr, "free element attrs(num_attrs=%d)\n", node->value.element.num_attrs);
#endif
	  for (i = 0; i < node->value.element.num_attrs; i ++)
	  {
#if DEBUG > 1
            fprintf(stderr, "free element attrs(attrs[%d].name=%s, value= %s)\n", i, 
...
#if DEBUG > 1
            fprintf(stderr, "free element attrs(attrs=%p)\n", node->value.element.attrs);
#endif
  ...
  */
  ...
  #if DEBUG > 1
   fprintf(stderr, "free this node(node = %p)\n", node);
  #endif
  free(node);
}
