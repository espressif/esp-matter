			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	if (handler != NULL) {
		if (sta != NULL) {
			wpa_printf(MSG_DEBUG, "hostapd_notif_assoc: CONNECTED STA MAC - %02x:%02x:%02x:%02x:%02x:%02x\n",
			sta->addr[0], sta->addr[1], sta->addr[2], sta->addr[3], sta->addr[4], sta->addr[5]);
			u8 packet[WIFI_MAC_ADDRESS_LENGTH + 1] = {0};
			os_memcpy(packet, sta->addr, WIFI_MAC_ADDRESS_LENGTH);
			packet[WIFI_MAC_ADDRESS_LENGTH] = WIFI_PORT_AP;
...
#if defined(CONFIG_MBO) && defined(NEED_AP_MLME)
...
void hostapd_event_ch_switch(struct hostapd_data *hapd, int freq, int ht,
			     ...
			     int finished)
{
#if defined(NEED_AP_MLME) || defined(CONFIG_PARTIAL_AP)
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_INTERFACE_ENABLED:
		...
		if (hapd->disabled && hapd->started) {
			...
			ieee802_11_set_beacon(hapd);
#if defined(NEED_AP_MLME) && defined(CFG_AP_DFS)
