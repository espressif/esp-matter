...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->last_scan_req != MANUAL_SCAN_REQ &&
		...
		while (ssid) {
			if (!wpas_network_disabled(wpa_s, ssid) &&
			    ssid->scan_ssid && !wpa_s->manual_scan_passive) {
				wpa_hexdump_ascii(MSG_DEBUG, "Scan SSID",
						  ssid->ssid, ssid->ssid_len);
		...
		for (tssid = wpa_s->conf->ssid; tssid;
		     tssid = tssid->next) {
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	...
	if (wpa_s->conf->ssid->mode == WPAS_MODE_AP) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Ignore scan request in AP mode");
		return;
	}
#endif
	...
	if (res == 1) {
		...
		/* Set this level to INFO for parsing by test tool */
		wpa_dbg(wpa_s, MSG_INFO, "Setting scan request: %d.%06d sec",
			sec, usec);
		eloop_register_timeout(sec, usec, wpa_supplicant_scan, wpa_s, NULL);
		/* force to wakeup eloop to re-schedule the next wakeup time */
...
#ifdef CONFIG_SUPPORT_6G
		 || is_6ghz_freq(wa->freq) || is_6ghz_freq(wb->freq)
#endif /* CONFIG_SUPPORT_6G */
		)) {
...
#if (CFG_MINISUPP_DBG_ENABLE_CLASS == DBG_MINISUPP_LOG_LEVEL_EXTREME)
	size_t i;
				...
				struct scan_info *info, int new_scan)
{
	...
	if (scan_res->res) {
#ifdef CONFIG_BAND_PREFERENCE
		g_bss_compar = wpa_s->band_preference;
#endif
