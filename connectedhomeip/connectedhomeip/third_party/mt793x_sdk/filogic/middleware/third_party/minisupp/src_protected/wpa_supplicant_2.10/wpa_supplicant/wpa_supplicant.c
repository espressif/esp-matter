...
/* to store params for the next connection*/
struct wpa_config *new_conf_sta;
struct wpa_config *new_conf_ap;
...
int wpa_set_wep_keys(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	for (i = 0; i < NUM_WEP_KEYS; i++) {
		...
		set = 1;
		wpa_drv_set_key(wpa_s, WPA_ALG_WEP, wpa_s->wpa->bssid,
			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
...
#ifdef CONFIG_SME
		if (wpa_auth_alg_fils(wpa_s->sme.auth_alg))
			fils_hlp_sent = 1;
#endif /* CONFIG_SME */
#else
		if (wpa_auth_alg_fils(wpa_s->auth_alg))
			fils_hlp_sent = 1;
#endif /* CONFIG_SUPP_SME */
		...
		if (wpa_s->current_ssid->mode == WPAS_MODE_INFRA) {
			...
			os_memcpy(packet, wpa_s->bssid, WIFI_MAC_ADDRESS_LENGTH);
			packet[WIFI_MAC_ADDRESS_LENGTH] = WIFI_PORT_STA;
			wifi_api_event_trigger(WIFI_PORT_STA,
					       WIFI_EVENT_IOT_PORT_SECURE,
					       packet, WIFI_MAC_ADDRESS_LENGTH);
			/* FreeRTOS LWIP porting */
			netif_set_link_up(&sta_netif);
		}
#ifdef CONFIG_OBSS_SCAN
		sme_sched_obss_scan(wpa_s, 1);
#endif /* CONFIG_OBSS_SCAN */
		...
		if (state == WPA_DISCONNECTED) {
			if (lwip_get_ipmode() == REQ_IP_MODE_DHCP) {
				netif_set_addr(&sta_netif,
						IP4_ADDR_ANY, IP4_ADDR_ANY, IP4_ADDR_ANY);
			}
			netif_set_link_down(&sta_netif);
		}
		else {
			wpa_dbg(wpa_s, MSG_INFO, "No link down and clear ip, State: %s -> %s",
					wpa_supplicant_state_txt(wpa_s->wpa_state),
					wpa_supplicant_state_txt(state));
		}
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	struct wpa_config *new_conf;
	int reconf_ctrl;
#ifdef CONFIG_CTRL_IFACE_DBUS_NEW
	int old_ap_scan;
#endif

	new_conf = (wpa_s->conf->ssid->mode == WPAS_MODE_AP) ? new_conf_ap : new_conf_sta;
	wpa_printf(MSG_DEBUG,
		   "wpa_supplicant_reload_configuration (mode = %s) --->\n",
		   (wpa_s->conf->ssid->mode == WPAS_MODE_AP) ? "AP" : "STA");

	conf = new_conf;

	if (conf->ssid == NULL) {
		wpa_printf(MSG_DEBUG, "Please set SSID");
		return -1;
	}
	if (conf->ssid->disabled == 2) {
		wpa_printf(MSG_DEBUG,
			   "CTRL_IFACE: Cannot use ENABLE_NETWORK with persistent P2P group");
		return -1;
	}

	conf->ssid->mode = wpa_s->conf->ssid->mode;

	/* Update PSK key */
	if (!conf->ssid->psk_set
		&&(conf->ssid->passphrase_len >= 8)
		&& (conf->ssid->passphrase_len <= 63)
		&&  wpa_key_mgmt_wpa_psk(conf->ssid->key_mgmt))
		wpa_config_update_psk(conf->ssid);
...
#endif
	conf->ssid->frequency = wpa_s->conf->ssid->frequency;
	...
	mtk_freertos_wpa_set_favor_ssid(wpa_s, wpa_s->conf->ssid->ssid,
					wpa_s->conf->ssid->ssid_len);
	wpa_s->scan_min_time.sec = 0;
	wpa_s->scan_min_time.usec = 0;
	wpa_supplicant_enable_network(wpa_s, wpa_s->conf->ssid);
	...
	wpa_bssid_ignore_clear(wpa_s);
#ifdef CONFIG_AP
	if (conf->ssid->mode == WPAS_MODE_AP) {
		//wpa_config_update_psk( conf->ssid ); /*Unify to update AP PMK*/
		if (conf->ssid->pairwise_cipher == 0)
			conf->ssid->pairwise_cipher = WPA_CIPHER_NONE;
		wpa_drv_deinit_ap(wpa_s);
		wpa_supplicant_create_ap(wpa_s, conf->ssid);
	}
#endif /* CONFIG_AP  */
	conf->ssid->scan_ssid = 1;
	conf->ssid->disabled = 0;
	wpa_config_debug_dump_networks(conf);
	new_conf = wpa_config_alloc_new_conf(conf->ssid);
	if (new_conf == NULL)
		wpa_printf(MSG_ERROR,
			   "%s: Failed to pre-alloc new_conf buffer.",
			   __FUNCTION__);
	if (wpa_s->conf->ssid->mode == WPAS_MODE_AP)
		new_conf_ap = new_conf;
	else
		new_conf_sta = new_conf;
...
#ifndef CONFIG_SUPP_SME
	if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_UPDATE_FT_IES))
		sel &= ~WPA_KEY_MGMT_FT;
#endif /* CONFIG_SUPP_SME */
...
#else
	if (wpa_s->drv_flags2 & WPA_DRIVER_FLAGS2_OCV)
		wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_OCV, ssid->ocv);
#endif /* CONFIG_SUPP_SME */
#endif /* CONFIG_OCV */
...
{
#ifdef CONFIG_SUPP_SME
	return wpa_s->drv_flags & WPA_DRIVER_FLAGS_SUPPORT_FILS;
#else
	return wpa_s->drv_flags & WPA_DRIVER_FLAGS_FILS_SK_OFFLOAD;
#endif /* CONFIG_SUPP_SME */
}
...
#if defined(CONFIG_IBSS) || defined(CONFIG_MESH)
				     ...
				     const struct wpa_interface *iface)
{
	...
	if (iface->confname) {
		...
		if (wpa_s->confname == NULL) {
			wpa_printf(MSG_ERROR, "Failed to malloc confname "
				   "for configuration file '%s'.",
				   iface->confname);
			return -1;
		}
		...
		if (wpa_s->conf->ssid->mode == WPAS_MODE_INFRA) {
			new_conf_sta = wpa_config_alloc_new_conf(wpa_s->conf->ssid);
			if (new_conf_sta == NULL)
				wpa_printf(MSG_ERROR,
					   "%s: Failed to pre-alloc new_conf_sta buffer.",
					   __FUNCTION__);
			if (wpa_s->conf->ssid->ssid_len != 0)
				mtk_freertos_wpa_set_favor_ssid(wpa_s,
					wpa_s->conf->ssid->ssid,
					wpa_s->conf->ssid->ssid_len);
		}

		if (wpa_s->conf->ssid->mode == WPAS_MODE_AP) {
			new_conf_ap = wpa_config_alloc_new_conf(wpa_s->conf->ssid);
			if (new_conf_ap == NULL)
				wpa_printf(MSG_ERROR,
					   "%s: Failed to pre-alloc new_conf_ap buffer.",
					   __FUNCTION__);
		}
					...
					int notify, int terminate)
{
	...
	if (new_conf_sta != NULL) {
		wpa_config_free(new_conf_sta);
		new_conf_sta = NULL;
	}

	if (new_conf_ap != NULL) {
		wpa_config_free(new_conf_ap);
		new_conf_ap = NULL;
	}
...
#ifndef CONFIG_OS_FRERRTOS
	/* this should be free in main task. */
	os_free(wpa_s);
#endif
}
						 ...
						 struct wpa_supplicant *parent)
{
	...
	struct wpa_ssid *ssid;
#endif
#if (defined(MTK_NVDM_ENABLE) && defined(CONFIG_FAST_CONNECTION))
...
#if defined(MTK_NVDM_ENABLE) && defined(CONFIG_FAST_CONNECTION)
	/* Load config from nvdm */
	len = sizeof(buff);
	if (nvdm_read_data_item("wifi", "EnableFastConnect", buff, &len) ==
	    NVDM_STATUS_OK)
		wpa_s->fast_connection = (uint8_t)atoi((char *)buff);
...
void wpas_periodic(void *eloop_ctx, void *timeout_ctx)
{
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	wpa_supplicant_entry_op_mode_get(&op_mode);
