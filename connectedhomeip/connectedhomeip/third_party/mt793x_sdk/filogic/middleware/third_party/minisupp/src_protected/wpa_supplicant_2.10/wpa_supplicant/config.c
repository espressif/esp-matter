				  ...
				  const char *value)
{
	...
	while (*start != '\0') {
		...
		*end = '\0';
		...
		if (os_strncmp(start, "RSN", 3) == 0 ||
		    os_strncmp(start, "WPA2", 4) == 0)
			val |= WPA_PROTO_RSN;
		else if (os_strncmp(start, "WPA", 3) == 0)
			val |= WPA_PROTO_WPA;
		else if (os_strncmp(start, "OSEN", 4) == 0)
			val |= WPA_PROTO_OSEN;
		else {
...
void wpa_config_update_psk(struct wpa_ssid *ssid)
{
	...
	os_get_time(&now);
	printf("%s start sec.usec: %ld.%ld\r\n", __func__, now.sec, now.usec);
#ifndef CONFIG_NO_PBKDF2
...
#undef ecw2cw
	...
	config->sae_pwe = DEFAULT_SAE_PWE_CONFIG;
