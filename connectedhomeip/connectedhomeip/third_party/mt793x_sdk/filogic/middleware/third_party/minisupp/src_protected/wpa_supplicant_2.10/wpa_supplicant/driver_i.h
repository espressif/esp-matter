...
static inline int wpa_drv_inband_cmd(struct wpa_supplicant *wpa_s, char *cmd,
                                     const u8 *buf, uint8_t buf_len)
{
	if (wpa_s->driver->inband_driver_cmd) {

		return wpa_s->driver->inband_driver_cmd(wpa_s->drv_priv, cmd,
							buf, buf_len);
	}
	return -1;
}
#endif
static inline struct l2_packet_data *wpa_drv_get_socket(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->driver->get_sock) {
		return wpa_s->driver->get_sock(wpa_s->drv_priv);
	}
	return NULL;
}
