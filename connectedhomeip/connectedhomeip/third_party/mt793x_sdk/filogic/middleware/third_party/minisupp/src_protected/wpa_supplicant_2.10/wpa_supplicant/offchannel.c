...
static void wpas_send_action_cb(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->off_channel_freq != wpa_s->pending_action_freq &&
		...
		if (without_roc && wpa_s->off_channel_freq == 0) {
			...
			// fosmod_wifi_logging begin
			if ((res = wpa_drv_remain_on_channel(wpa_s,
					wpa_s->pending_action_freq,
					duration)) < 0) {
				wpa_printf(MSG_INFO, "Off-channel: Failed to "
					   "request driver to remain on "
					   "channel (%u MHz) for Action Frame "
					   "TX: err=%d (%s)", wpa_s->pending_action_freq,
					   -res, strerror(-res));
			} else {
	...
	if (res) {
		...
		wpa_printf(MSG_DEBUG, "Off-channel: Failed to send the "
			   "pending Action frame: err=%d (%s)", -res,
			   strerror(-res));
		// fosmod_wifi_logging end
		/*
...
int offchannel_send_action(struct wpa_supplicant *wpa_s, unsigned int freq,
			   ...
			   int no_cck)
{
	int err; // fosmod_wifi_logging oneline
...
#endif /* CONFIG_TESTING_OPTIONS */
	// fosmod_wifi_logging begin
	if ((err = wpa_drv_remain_on_channel(wpa_s, freq, wait_time)) < 0) {
		wpa_printf(MSG_INFO, "Off-channel: Failed to request driver "
			   "to remain on channel (%u MHz) for Action "
			   "Frame TX: err=%d (%s)", freq, -err, strerror(-err));
		return -1;
	}
	// fosmod_wifi_logging end
	wpa_s->off_channel_freq = 0;
