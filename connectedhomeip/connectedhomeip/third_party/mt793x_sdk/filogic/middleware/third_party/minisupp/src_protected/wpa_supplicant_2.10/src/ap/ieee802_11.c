					     ...
					     int status_code)
{
	...
	if (sta->sae->tmp) {
		...
		use_pt = sta->sae->h2e;
#if defined(CONFIG_SAE_PK) && defined(NEED_AP_MLME)
...
#if (defined(CONFIG_SAE) && defined(NEED_AP_MLME)) || defined(CONFIG_PASN)
...
#if defined(NEED_AP_MLME) || defined(CONFIG_FILS)
