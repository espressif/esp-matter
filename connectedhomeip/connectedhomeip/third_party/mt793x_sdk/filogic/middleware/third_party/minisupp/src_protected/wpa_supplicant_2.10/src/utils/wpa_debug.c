...
static char wpa_printf_buf[2048];
...
void wpa_debug_print_timestamp(void)
{
	...
	if (!out_file && !wpa_debug_syslog) {
		LOG_I(minisupp, "%ld.%06u: ", (long) tv.sec,
		      (unsigned int) tv.usec);
	}
#endif /* CONFIG_ANDROID_LOG */
}
...
static void print_log(int level, char *buf)
{
	if (level == MSG_ERROR) {
		LOG_E(minisupp, buf);
 ...
 */
void minisupp_wpa_printf(int level, const char *fmt, ...)
{
	if (level <= wpa_debug_level) {
		...
		if (!wpa_debug_syslog && !out_file) {
			...
			int len;
			os_memset(wpa_printf_buf, 0, sizeof(wpa_printf_buf));
			buf = &wpa_printf_buf[0];
			va_start(ap, fmt);
...
static void _wpa_hexdump_ascii_syslog(int level, const char *title, const u8 *buf,
				      size_t len, int show)
{
	if (buf == NULL) {
		wpa_printf(level, "%s - hexdump_ascii(len=%lu): [NULL]",
			   title, (unsigned long) len);
	} else if (len == 0) {
		wpa_printf(level, "%s - hexdump_ascii(len=%lu): ",
			   title, (unsigned long) len);
	} else if (show && len) {
		size_t i, llen;
		...
		while (len) {
			...
			for (i = 0; i < llen; i++) {
				if (os_snprintf(&strbuf[i * 3], 4, " %02x",
				    pos[i]) < 0) {
					error = 1;
					goto done;
				}
			...
			for (i = 0; i < llen; i++) {
				if (isprint(pos[i])) {
					if (os_snprintf(&strbuf[line_len * 3 + i],
					    2, "%c", pos[i]) < 0) {
						error = 1;
						goto done;
					}
				} else {
					if (os_snprintf(&strbuf[line_len * 3 + i], 2,
					    "_") < 0) {
						error = 1;
						goto done;
					}
		...
		if (error)
			wpa_printf(MSG_ERROR,
				   "wpa_hexdump: Failed to snprintf");
		bin_clear_free(strbuf, 1 + 4 * line_len);
	} else {
		wpa_printf(level, "%s - hexdump_ascii(len=%lu): [REMOVED]",
			   title, (unsigned long) len);
	}
}

static void _wpa_hexdump_syslog(int level, const char *title, const u8 *buf,
				size_t len, int show)
{
	size_t i;
	const char *display;
	...
	if (buf == NULL) {
		...
		for (i = 0; i < len; i++) {
			if (os_snprintf(&strbuf[i * 3], 4, " %02x",
			    buf[i]) < 0) {
				wpa_printf(MSG_ERROR, "wpa_hexdump: Failed"
					   "to snprintf");
	...
	wpa_printf(level, "%s - hexdump(len=%lu):%s", title,
...
{
#if defined(CONFIG_DEBUG_LINUX_TRACING) || defined(CONFIG_ANDROID_LOG)
	size_t i;
#endif
...
#ifdef CONFIG_DEBUG_SYSLOG
	if (wpa_debug_syslog) {
		_wpa_hexdump_syslog(level, title, buf, len, show);
		if (only_syslog)
			return;
	}
#endif /* CONFIG_DEBUG_SYSLOG */
...
void minisupp_wpa_hexdump(int level, const char *title, const void *buf, size_t len)
{
	_wpa_hexdump(level, title, buf, len, 1, 0);
}


void minisupp_wpa_hexdump_key(int level, const char *title, const void *buf, size_t len)
{
...
#endif /* CONFIG_DEBUG_FILE */
	if (!wpa_debug_syslog && !out_file) {
		_wpa_hexdump_ascii_syslog(level, title, buf, len, show);
	}
#endif /* CONFIG_ANDROID_LOG */
}
...
void minisupp_wpa_hexdump_ascii_key(int level, const char *title, const void *buf,
			   size_t len)
{
...
void minisupp_wpa_msg(void *ctx, int level, const char *fmt, ...)
{
	va_list ap;
	char *buf;
	const int buflen = 2048;
