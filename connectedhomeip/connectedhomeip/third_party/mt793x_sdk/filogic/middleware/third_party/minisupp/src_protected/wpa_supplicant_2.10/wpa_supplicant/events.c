...
void wpa_supplicant_mark_disassoc(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->wpa_state <= WPA_INTERFACE_DISABLED) {
		wpa_dbg(wpa_s, MSG_DEBUG, "No need to mark_disassoc again");
		return;
	}
...
#ifndef CONFIG_NO_UNUSED_EVENTS
					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	if ((ssid->key_mgmt & WPA_KEY_MGMT_OSEN) &&
		...
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   allow in OSEN");
		return 1;
	}

	if (!wpa_key_mgmt_wpa(ssid->key_mgmt)) {
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   allow in non-WPA/WPA2");
		return 1;
	}
			    ...
			    bool debug_print)
{
	...
	if (check_ssid &&
		...
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   skip - SSID mismatch");
		return false;
	}
	...
	if (ssid->bssid_set &&
	    os_memcmp(bss->bssid, ssid->bssid, ETH_ALEN) != 0) {
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   skip - BSSID mismatch");
		return false;
	}
	...
	if (ssid->p2p_group &&
		...
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   skip - no P2P IE seen");
		return false;
	}
	...
	/* TODO: FIXEME KOKO */
	if (0) { //os_reltime_before(&bss->last_update, &wpa_s->scan_min_time))
				     ...
				     int only_first_ssid, int debug_print)
{
	...
	if (debug_print) {
		wpa_dbg(wpa_s, MSG_INFO, "%d: " MACSTR
	...
	if (match_ssid_len == 0) {
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   skip - SSID not known");
		return NULL;
	}

	if (disallowed_bssid(wpa_s, bss->bssid)) {
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   skip - BSSID disallowed");
		return NULL;
	}

	if (disallowed_ssid(wpa_s, match_ssid, match_ssid_len)) {
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   skip - SSID disallowed");
		return NULL;
	}

	if (disabled_freq(wpa_s, bss->freq)) {
		if (debug_print)
			wpa_dbg(wpa_s, MSG_INFO, "   skip - channel disabled");
		return NULL;
	}
...
#ifdef CONFIG_ROAMING_CUSTOMIZED_RSSI_DELTA
	status = wifi_config_get_roam_delta(WIFI_PORT_STA, &rssi_delta);

	if (current_bss->level < 0 &&
	    selected->level <= current_bss->level + rssi_delta) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Skip roam - The difference of "
			"current BSS's and selected BSS's signal level is lower than delta");
			return 0;
	}
	ret = 1;
#else
					      ...
					      int new_scan, int own_request)
{
	...
	if (selected) {
		...
		/*
		 * remove scan_freq from active ssid after no available network found
		 * for not finding the network means
		 * that the channel information is not valid
		 */
		for (tssid = wpa_s->conf->ssid; tssid; tssid = tssid->next) {
			if (wpas_network_disabled(wpa_s, tssid))
				continue;
			if (tssid->scan_freq) {
				os_free(tssid->scan_freq);
				tssid->scan_freq = NULL;
			}
		}
...
static int wpa_supplicant_event_associnfo(struct wpa_supplicant *wpa_s,
					  union wpa_event_data *data)
{
	int l, len, found = 0, found_x = 0, wpa_found, rsn_found, rsnxe_found;
	const u8 *p;
	u8 bssid[ETH_ALEN];
	bool bssid_known;
...
no_pfs:
	...
	if (wpa_ft_is_completed(wpa_s->wpa)) {
		if (!bssid_known ||
		    wpa_ft_validate_reassoc_resp(wpa_s->wpa,
						 data->assoc_info.resp_ies,
						 data->assoc_info.resp_ies_len,
	 ...
	 */
	wpa_found = rsn_found = rsnxe_found = 0;
	while (p && l >= 2) {
		len = p[1] + 2;
		if (len > l) {
	...
	if (!rsnxe_found && data->assoc_info.beacon_ies)
...
#ifndef CONFIG_SUPP_SME
	if (data && wpa_supplicant_use_own_rsne_params(wpa_s, data) < 0)
		return;
#endif /* CONFIG_SUPP_SME */
...
#if defined(CONFIG_WNM) && !defined(CONFIG_NO_UNUSED_EVENTS)
...
#endif /* CONFIG_WNM && CONFIG_NO_UNUSED_EVENTS*/
...
static void ft_rx_action(struct wpa_supplicant *wpa_s, const u8 *data,
			 size_t len)
{
	/* only SME case supported for now */
#ifdef CONFIG_SUPP_SME
				...
				struct disassoc_info *info)
{
	...
	if (wpa_s->wpa_state < WPA_AUTHENTICATING) {
		wpa_dbg(wpa_s, MSG_DEBUG, "No need to do Disassociation again");
		return;
	}
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_SCAN_RESULTS:
		...
		wifi_api_event_trigger(WIFI_PORT_STA,
				       WIFI_EVENT_IOT_SCAN_COMPLETE, NULL, 0);
		if (!(data && data->scan_info.external_scan))
			wpa_s->own_scan_running = 0;
...
#ifdef CONFIG_AP
#if defined(CONFIG_ACS_OFFLOAD) || defined(CONFIG_ACS)
		if (!wpa_s->ap_iface)
			break;
