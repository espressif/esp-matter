			       ...
			       enum wpa_states old_state)
{
	...
	if (op_mode == WIFI_MODE_STA_ONLY) {
		if (new_state == WPA_COMPLETED ||
			new_state == WPA_INACTIVE ||
			new_state == WPA_INTERFACE_DISABLED) {
			eloop_cancel_timeout(wpas_periodic, wpa_s->global, NULL);
		} else {
			if (!eloop_is_timeout_registered(wpas_periodic, wpa_s->global, NULL))
				wpas_periodic(wpa_s->global, NULL);
		}
	}
	...
	wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_STATE_CHANGE
