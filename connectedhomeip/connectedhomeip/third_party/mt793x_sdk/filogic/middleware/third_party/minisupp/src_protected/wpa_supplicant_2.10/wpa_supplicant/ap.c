...
#if defined(CONFIG_P2P) && defined(CONFIG_SUPPORT_6G)
...
#endif /* CONFIG_P2P && CONFIG_SUPPORT_6G */
			      ...
			      struct hostapd_config *conf)
{
	...
	if (conf->hw_mode == NUM_HOSTAPD_MODES) {
		wpa_printf(MSG_ERROR, "Unsupported AP mode frequency: %d MHz",
			   ssid->frequency);
		/* in case conf->channel as 0/ssid->frequency is zero
		 * then would go bellow acs flow
		 */
		/* return -1; */
	}
...
extern int hostapd_config_fill(struct hostapd_config *conf,
			     ...
			     struct wpa_ssid *ssid)
{
	...
	bss = conf->bss[0];
	hostapd_config_fill(conf, bss, ssid);
