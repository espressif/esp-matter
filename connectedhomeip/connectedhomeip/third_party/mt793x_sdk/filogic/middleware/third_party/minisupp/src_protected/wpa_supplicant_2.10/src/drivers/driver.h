...
/* rtos porting */
#include "common/minisupp_common.h"
#ifdef CONFIG_MACSEC
...
#endif
#define DISABLE_INACTIVITY_TIMEOUT 1
#define PROPERTY_VALUE_MAX 64
#define PASS_BSSID_FREQ 1
...
struct wpa_scan_res {
	...
	/* rtos_porting */
	u8 auth_mode;    //Pengfei add, for connect without security mode
	u8 encrypt_type; //Pengfei add, for connect without security mode
};
...
struct wpa_driver_ops {
	...
	/* rtos_porting */
#if defined(CONFIG_DRIVER_INBAND) || defined(CONFIG_DRIVER_GEN4M)
	int (*inband_driver_cmd)(void *priv,
				 char *cmd, const u8 *buf, uint8_t buf_len);
#endif
	struct l2_packet_data *(*get_sock)(void *priv);
	/**
...
int mtk_supplicant_set_ssid(uint8_t port, uint8_t *ssid,
			    uint8_t ssid_length);

int mtk_supplicant_get_ssid(uint8_t port, uint8_t *ssid,
			    uint8_t *ssid_length);

int mtk_supplicant_set_bssid(uint8_t *bssid);

int mtk_supplicant_set_security(uint8_t port, uint8_t auth_mode,
...
int mtk_supplicant_get_security(uint8_t port, uint8_t *auth_mode,
				uint8_t *encrypt_type);

int mtk_supplicant_set_wpa_psk_key(uint8_t port, uint8_t *passphrase,
				   uint8_t passphrase_length);

int mtk_supplicant_get_wpa_psk_key(uint8_t port, uint8_t *passphrase,
				   uint8_t *passphrase_length);
...
void mtk_supplicant_start_scan(void);

void mtk_supplicant_stop_scan(void);

void mtk_supplicant_ap_onoff(uint8_t radio_on);

void wpa_driver_gen4m_password_error(void);

int process_global_event(void *priv, unsigned char evt_id,
...
/* NULL termina ted array of linked in driver wrappers */
extern const struct wpa_driver_ops *const wpa_drivers[];
//rtos_porting
extern struct wpa_driver_ops wpa_driver_inband_ops; /* driver_inband.c */
#ifdef CONFIG_DRIVER_GEN4M
extern struct wpa_driver_ops wpa_driver_gen4m_ops; /* driver_gen4m.c */
#endif /* CONFIG_DRIVER_GEN4M */
...
extern unsigned char g_rx_handler_mode;
extern unsigned char g_sched_scan_enabled;
extern os_semaphore_t opmode_switch_mutex;
extern os_semaphore_t g_wait_wpa_ready;
