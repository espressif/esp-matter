...
#ifdef CONFIG_OS_FRERRTOS
	if (ssid)
		mtk_freertos_wpa_set_favor_ssid(wpa_s, ssid->ssid,
						ssid->ssid_len);
#endif
	wpa_s->scan_min_time.sec = 0;
	wpa_s->scan_min_time.usec = 0;
					 ...
					 char *buf, size_t *resp_len)
{
	...
	//const int reply_size = 256; // it's not enough for scan results
	int reply_len;

	wpa_dbg(wpa_s, MSG_DEBUG,
		"wpa_supplicant_ctrl_iface_process ==> '%s'", buf);
