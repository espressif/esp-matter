...
int gnum_channels = 0;
static int hostapd_get_hw_mode_any_channels(struct hostapd_data *hapd,
...
int hostapd_drv_do_acs(struct hostapd_data *hapd)
{
	...
	if (mode) {
		...
		for (i = 0; i < hapd->iface->num_hw_features; i++) {
			...
			num_channels = hostapd_get_hw_mode_any_channels(hapd, mode,
							 acs_ch_list_all,
							 &freq_list);
		}

		channels = os_malloc(num_channels);
		if (channels == NULL)
			return -1;

		for (i = 0; i< num_channels; i++)
			ieee80211_freq_to_chan(freq_list[i], &channels[i]);
	}
