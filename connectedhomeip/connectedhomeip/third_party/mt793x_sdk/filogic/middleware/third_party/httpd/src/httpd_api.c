...
static QueueHandle_t httpd_proc_mbox = NULL;
static TaskHandle_t httpd_proc_task_handle = NULL;
static HTTPD_STATUS httpd_status = HTTPD_STATUS_UNINIT;
static QueueHandle_t httpd_fb_mbox = NULL;
...
int httpd_fb_status(HTTPD_STATUS status, HTTPD_STOP_REASON reason)
{
    ...
    if(httpd_fb_mbox)
    {
        if(xQueueSend(httpd_fb_mbox, (void *)&fb, portMAX_DELAY) != pdPASS)
        {
...
void httpd_proc_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (xQueueReceive(httpd_proc_mbox, (void *)parameter, portMAX_DELAY) == pdPASS)
        {
...
HTTPD_RESULT httpd_init(void)
{
    ...
    {
        httpd_proc_mbox = xQueueCreate(HTTPD_QUEUE_LENGTH, sizeof(httpd_para));
        if(httpd_proc_mbox == NULL)
        {
    ...
    if (httpd_proc_task_handle == NULL)
    {
        ...
        xTaskCreate(
            httpd_proc_thread,
            HTTPD_TASK_NAME,
            HTTPD_TASK_STACKSIZE/sizeof(portSTACK_TYPE),
            NULL,
            HTTPD_TASK_PRIO,
        ...
        if(httpd_proc_task_handle == NULL)
        {
            //HTTPD_DEBUGF((httpd, "[HTTPD] httpd_init(), xTaskCreate fail, return vaule = %ld\n", xReturn));
            return HTTPD_RESULT_UNINIT;
        }
...
HTTPD_RESULT httpd_start(httpd_para *parameter)
{
    ...
    if(xQueueSend(httpd_proc_mbox, (void *)&para, portMAX_DELAY) != pdPASS)
    {
...
HTTPD_RESULT httpd_deinit(void)
{
    ...
    if(httpd_proc_task_handle != NULL)
    {
        vTaskDelete(httpd_proc_task_handle);
