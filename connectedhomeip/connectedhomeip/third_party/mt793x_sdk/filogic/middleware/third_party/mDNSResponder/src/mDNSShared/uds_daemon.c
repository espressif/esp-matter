...
mDNSlocal void send_all(dnssd_sock_t s, const char *ptr, int len)
{
    ...
    if (n < len)
        mdnslogInfo("ERROR: send_all(%d) wrote %d of %d errno %d (%s)",
...
mDNSlocal void regrecord_callback(mDNS *const m, AuthRecord *rr, mStatus result)
{
    ...
    if (!rr->RecordContext)     // parent struct already freed by termination callback
    {
        ...
        else
        {
            ...
            if (rr->resrec.rdata != &rr->rdatastorage) free(rr->resrec.rdata);
            free(rr);
        }
    }
    else
...
mDNSlocal mStatus update_record(AuthRecord *rr, mDNSu16 rdlen, const char *rdata, mDNSu32 ttl, const mDNSBool *const external_advertise)
{
    ...
    if (result) { LogMsg("update_record: Error %d for %s", (int)result, ARDisplayString(&mDNSStorage, rr)); free(newrd); }
    return result;
}
...
mDNSlocal mStatus handle_update_request(request_state *request)
{
    ...
    {
        if (request->u.servicereg.txtdata)
        { free(request->u.servicereg.txtdata); request->u.servicereg.txtdata = NULL; }
        if (rdlen > 0)
        {
...
mDNSlocal mStatus handle_regservice_request(request_state *request)
{
    ...
    { LogMsg("ERROR: handle_regservice_request - Couldn't find interfaceIndex %d", interfaceIndex); goto bad_param; }
    ...
    if (!request->msgptr) { LogMsg("%3d: DNSServiceRegister(unreadable parameters)", request->sd); goto bad_param; }
...
mDNSlocal mStatus handle_queryrecord_request(request_state *request)
{
    ...
    if (get_string(&request->msgptr, request->msgend, name, 64) < 0) return(mStatus_BadParamErr);
...
mDNSlocal mStatus handle_addrinfo_request(request_state *request)
{
    ...
    if (get_string(&request->msgptr, request->msgend, hostname, 64) < 0) return(mStatus_BadParamErr);
...
rerror:
    ...
    if (socket_errno == dnssd_EWOULDBLOCK || socket_errno == dnssd_EINTR) return;
    req->ts = t_error;
}
...
mDNSlocal void request_callback(int fd, short filter, void *info)
{
    ...
    for (;;)
    {
        ...
        if (req->hdr.op != cancel_request && req->hdr.op != getproperty_request && req->hdr.op != send_bpf && req->hdr.op != getpid_request)
        {
            ...
            if (req->errsd != req->sd)
            {
                mdnslogInfo("%3d: not socket %d closed  %08X %08X (%d)",
