...
void HandleEvent(void)
{
    ...
    while (!stopnow) {
        ...
        if (result > 0) {
            ...
            if (error) {
                mdnslogInfo(stderr, "DNSServiceProcessResult returned %d\n", error);
                stopnow = 1;
            }
        } else {
...
int RegisterBonjourService(char* servicename, char* type)
{
    ...
    nvdm_read_data_item("STA", "MacAddr", (uint8_t *) mac_addr, &sz);
    ...
    err = TXTRecordSetValue(&txtRecord, "srcver", os_strlen(sourceVersionString),
            sourceVersionString);
    require_noerr( err, exit);
    ...
    */
    ...
    mdnslogInfo(
            "Register Bonjour Service: %s type: %s domain: %s port: %d\n", servicename, ServiceType, ServiceDomain, ServicePort);
...
void rename_service_cmd(char **argv)
{
    ...
    if (g_txtRecord != NULL) {
      int ret = TXTRecordContainsKey(TXTRecordGetLength(g_txtRecord), TXTRecordGetBytesPtr(g_txtRecord), "seed");
        ...
        DNSServiceErrorType err = DNSServiceAddRecord(client, &hinfo_recRef, 0, kDNSServiceType_HINFO, sizeof(hinfoW), &hinfoW[0], 0);
        mdnslogInfo("~~~~~~~~~~DNSServiceAddRecord hinfo err=%d RecordRef=%d", err, hinfo_recRef);
        err = DNSServiceAddRecord(client, &testTXT_recRef, 0, kDNSServiceType_TXT, sizeof(testTXT), &testTXT[0], 0);
        mdnslogInfo("~~~~~~~~~~DNSServiceAddRecord chip err=%d RecordRef=%d", err, testTXT_recRef);
        g_txtRecord = NULL;
