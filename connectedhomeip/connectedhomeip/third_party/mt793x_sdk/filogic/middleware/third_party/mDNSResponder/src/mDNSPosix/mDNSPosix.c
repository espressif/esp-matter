...
mDNSlocal void SocketDataReady(mDNS *const m, PosixNetworkInterface *intf, int skt)
{
    ...
    if(intf->multicastSocket4 == skt)
        packetLen = recvfrom_flags(skt, &packet, sizeof(packet), &flags, (struct sockaddr *) &from, &fromLen, &packetInfo, &ttl, mDNSfalse);
#if HAVE_IPV6
    else if (intf->multicastSocket6 == skt)
        packetLen = recvfrom_flags(skt, &packet, sizeof(packet), &flags, (struct sockaddr *) &from, &fromLen, &packetInfo, &ttl, mDNStrue);
#endif
    else
...
mDNSlocal void ClearInterfaceList(mDNS *const m)
{
    ...
    while (m->HostInterfaces)
    {
        ...
        if (gMDNSPlatformPosixVerboseLevel > 0) fdebug_printf(stderr, "Deregistered interface %s\n", intf->intfName);
...
mDNSlocal int SetupSocket(struct sockaddr *intfAddr, mDNSIPPort port, int interfaceIndex, int *sktPtr)
{
    ...
    if (*sktPtr < 0) { err = errno; debug_perror((intfAddr->sa_family == AF_INET) ? "socket AF_INET" : "socket AF_INET6"); }
    ...
    if (err == 0 && port.NotAnInteger)
    {
        ...
        #endif
	if (err < 0) { err = errno; debug_perror("setsockopt - SO_REUSExxxx"); debug_printf("\n setsockopt failed err is %d\n ", err); }
   ...
   //             #endif
   //         #else
   //		debug_printf("warning This platform has no way to get the destination interface information -- will only work for single-homed hosts\n");
   ...
   //    }
        ...
        if (err == 0 && JoinMulticastGroup)
        {
			...
			if (err < 0) { err = errno; mdnslogInfo("setsockopt - IP_ADD_MEMBERSHIP"); mdnslogInfo("SetupSocket: setsockopt IP_ADD_MEMBERSHIP err < 0 \n ");}
        }
        ...
        if (err == 0 && JoinMulticastGroup)
        {
			...
			if (err < 0) { err = errno; mdnslogInfo("setsockopt - IP_MULTICAST_IF");  mdnslogInfo("SetupSocket: setsockopt IP_MULTICAST_IF err < 0 \n ");}
        }
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; mdnslogInfo("setsockopt - IP_TTL");  mdnslogInfo("SetupSocket: setsockopt IP_TTL err < 0 \n ");}
        }
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; mdnslogInfo("setsockopt - IP_MULTICAST_TTL"); mdnslogInfo("SetupSocket: setupsockopt IP_MULTICAST_TTL\n");}
        }
        if (err == 0)
        {
            err = setsockopt(*sktPtr, IPPROTO_IP, IP_PKTINFO, &kOn, sizeof(kOn));
            if (err < 0) { err = errno; mdnslogInfo("setsockopt - v4 IP_PKTINFO");  mdnslogInfo("SetupSocket: setsockopt -v4 IP_PKTINFO error\n");}
        }
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; mdnslogInfo("bind"); fflush(stderr); mdnslogInfo("SetupSocket: bind listening socket error \n");}
        }
    ...
    else if (intfAddr->sa_family == AF_INET6)
    {
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; mdnslogInfo("setsockopt - IPV6_PKTINFO"); mdnslogInfo("SetupSocket: setsockopt - IPV6_PKTINFO error\n");}
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; mdnslogInfo("setsockopt - IPV6_HOPLIMIT");  mdnslogInfo("SetupSocket: setsockopt - IPV6_HOPLIMIT error\n");}
        }
    #endif
       if (err == 0 && JoinMulticastGroup)
       {
        ...
        if (err == 0 && JoinMulticastGroup)
        {
           err = setsockopt(*sktPtr, IPPROTO_IPV6, IPV6_MULTICAST_IF,  &((struct sockaddr_in6*)intfAddr)->sin6_addr, sizeof(struct in6_addr));
           if (err < 0) { err = errno; mdnslogInfo("setsockopt - IPV6_MULTICAST_IF");  mdnslogInfo("SetupSocket: setsockopt - IPV6_MULTICAST_IF error\n");}
	}
        ...
        if (err == 0)
        {
            ...
            if (err < 0) { err = errno; mdnslogInfo("setsockopt - IPV6_V6ONLY");  mdnslogInfo("SetupSocket: setsockopt - IPV6_V6ONLY error\n");}
        }
        ...
        {
            err = setsockopt(*sktPtr, IPPROTO_IP, IP_TTL, &kIntTwoFiveFive, sizeof(kIntTwoFiveFive));
            if (err < 0) { err = errno; mdnslogInfo("setsockopt - IPV6_UNICAST_HOPS"); }
        }
        ...
        if (err == 0)
        {
			...
			if (err < 0) { err = errno; mdnslogInfo("setsockopt - IPV6_MULTICAST_HOPS");mdnslogInfo("SetupSocket: setsockopt - IPV6_MULTICAST_HOPS error\n"); }
        }
        if (err == 0)
        {
            err = setsockopt(*sktPtr, IPPROTO_IP, IP_PKTINFO, &kOn, sizeof(kOn));
            if (err < 0) { err = errno; mdnslogInfo("setsockopt - v6 IP_PKTINFO");  mdnslogInfo("SetupSocket: setsockopt -v6 IP_PKTINFO error\n");}
        }
        // And start listening for packets
        if (err == 0)
        {
	    ...
	    if (err < 0) { err = errno; mdnslogInfo("bind"); fflush(stderr);mdnslogInfo("SetupSocket: bind listening packet Addr6 error %d \n ", err); }
        }
...
mDNSlocal int SetupOneInterface(mDNS *const m, struct sockaddr *intfAddr, struct sockaddr *intfMask, const char *intfName, int intfIndex)
{
    ...
    if (intf_setup == NULL) { assert(0); err = ENOMEM; }
    ...
    if (err == 0)
    {
		...
		if (intf_setup->intfName == NULL) { debug_printf("intf.infName is NULL \n"); err = ENOMEM; }
    }
    ...
    if (err == 0)
    {
		...
		if (alias == NULL) alias   = intf_setup;
...
mDNSlocal int SetupInterfaceList(mDNS *const m)
{
    ...
    os_memcpy(intfAddr_temp6->sin6_addr.un.u32_addr, netif_var->ip6_addr[lwip_ipv6_addr_index].u_addr.ip6.addr, sizeof(netif_var->ip6_addr[lwip_ipv6_addr_index].u_addr.ip6.addr));
...
mStatus mDNSPosixRunEventLoopOnce(mDNS *m, const struct timeval *pTimeout,
									void *pSignalsReceived, mDNSBool *pDataDispatched)
