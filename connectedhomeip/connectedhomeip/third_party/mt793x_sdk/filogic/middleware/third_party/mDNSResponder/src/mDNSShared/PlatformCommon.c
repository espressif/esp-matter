...
mDNSexport void ReadDDNSSettingsFromConfFile(mDNS *const m, const char *const filename, domainname *const hostname, domainname *const domain, mDNSBool *DomainDiscoveryDisabled)
{
    ...
    if (f)
    {
        if (DomainDiscoveryDisabled && GetConfigOption(buf, "DomainDiscoveryDisabled", f) && !os_strcasecmp(buf, "true")) *DomainDiscoveryDisabled = mDNStrue;
...
mDNSexport void mDNSPlatformWriteLogMsg(const char *ident, const char *buffer, mDNSLogLevel_t loglevel)
{
    ...
    else                // else, in production mode, we write to syslog
    {
        ...
        switch (loglevel)
        {
			...
			case MDNS_LOG_MSG:       syslog_level = 1;     break;
			//case MDNS_LOG_OPERATION: syslog_level = LOG_WARNING; break;
			case MDNS_LOG_OPERATION: syslog_level = 2; break;
			case MDNS_LOG_SPS:       syslog_level = 3;  break;
			//case MDNS_LOG_SPS:       syslog_level = LOG_NOTICE;  break;
			case MDNS_LOG_INFO:      syslog_level = 4;    break;
			//case MDNS_LOG_INFO:      syslog_level = LOG_INFO;    break;
			case MDNS_LOG_DEBUG:     syslog_level = 5;   break;
        ...
        //syslog(syslog_level, "%s", buffer);
			switch(syslog_level)
			{
				...
				case 1:printf("LOG_ERR: %s\n", buffer);
					break;
				case 2:printf("LOG_WARNING: %s\n", buffer);
					break;
				case 3:printf("LOG_NOTICE: %s\n", buffer);
					break;
				case 4:printf("LOG_INFO: %s\n", buffer);
					break;
				case 5: printf("LOG_DEBUG: %s\n", buffer);
					break;
				default: printf("LOG_OTHER: %s\n", buffer); 
					break; 
			}
#endif
    }
