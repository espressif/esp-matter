...
*/
    ...
    (void) snprintf(temp, sizeof(temp), "%s.%ld.%06ld.%08lx%08lx.%s", hostname, (unsigned long int) tv.tv_sec, (unsigned long int) tv.tv_usec, (unsigned long int) 0, (unsigned long int) 0, "1234567890123456789012345678901234567890");
...
cpu_util(double pcpu[3])
{
    ...
    double timediff = 1.0;
    double userdiff = 1.0;
    double systemdiff = 1.0;

    if (pcpu == NULL) {
        iperf_gettimeofday(&last);
        //clast = clock();
        return;
    }

    iperf_gettimeofday(&temp);
    //ctemp = clock();

    timediff = ((temp.tv_sec * 1000000.0) - (last.tv_sec * 1000000.0));
    userdiff = ((temp.tv_sec * 1000000.0) - (last.tv_sec * 1000000.0));
    systemdiff = ((temp.tv_sec * 1000000.0) - (last.tv_sec * 1000000.0));


    //pcpu[0] = (double)((((double)ctemp - (double)clast) * (double)1000000.0 / CLOCKS_PER_SEC) / timediff) * 100;
    pcpu[1] = (double)(userdiff / timediff) * 100;
    pcpu[2] = (double)(systemdiff / timediff) * 100;
    pcpu[0] = pcpu[1];
}
...
iperf_json_printf(const char *format, ...)
{
	...
	switch (*cp) {
	    ...
	    *np = '\0';
		...
		break;
		case 'd':
		j = cJSON_CreateBool(va_arg(argp, int64_t));
		break;
		case 'f':
