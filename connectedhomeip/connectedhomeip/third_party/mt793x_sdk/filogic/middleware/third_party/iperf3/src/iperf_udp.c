...
iperf_udp_recv(struct iperf_stream *sp)
{
...
#if IPERF3_DEBUG
	iperf_err(sp->test, "OUT OF ORDER - incoming packet = %d and received packet = %d AND SP = %d", (int)pcount, (int)sp->packet_count, (int)sp->socket);
#endif
    }
    ...
    sp->jitter += (d - sp->jitter) / (double)16.0;
...
#if IPERF3_DEBUG
        printf("[iperf_test][iperf_udp_accept]netannounce error, domain = %d, Pudp = %d, bind_address = %s, port = %d, prot_listener = %d \n", (int)test->settings->domain, (int)Pudp, test->bind_address, (int)test->server_port, (int)test->prot_listener);
#endif
...
#if IPERF3_DEBUG
        printf("[iperf_test][iperf_udp_listen]netannounce error, domain = %d, Pudp = %d, bind_address = %s, port = %d, s = %d \n", (int)test->settings->domain, (int)Pudp, test->bind_address, (int)test->server_port, (int)s);
#endif
        i_errno = IESTREAMLISTEN;
        return -1;
    }
