...
// mtk54101 modify: decrease the array
// JLJoinPacket_t _g_jps[JL_MAX_JOIN_PKG_ARRARY];
JLJoinPacket_t _g_jps[1];
...
joylink_join_pkg_get_index_by_name_id(char *name, int id)
{
    ...
    // mtk54101 modify: when assert happened, customer need to increase the array
    assert(ret != E_RET_ERROR_NO_EXIST);
...
joylink_join_pkg_join_data(char *name, int id, int *out_len)
{
    ...
    if(ret >= 0){
        jp = &_g_jps[ret];
        p_data = (char *)joylink_malloc(jp->total_len);
