        ...
        int plainLength, UINT8 *pEncOut, int maxOutLen)
{
    ...
    char *tdata  = (char *)joylink_malloc(plainLength + 16);
        ...
        int encLength, UINT8 *pPlainOut, int maxOutLen)
{
    ...
    char *tdata  = joylink_malloc(encLength + 16);
    int dlen = encLength;
    int ret;
...
// mtk54101 modify : slim, use mbedtls rom aes
...
int joylinkEnc2Crypt(UINT8 *key, UINT32 keyLen, UINT8 *iv, UINT8 *data, UINT32 *len, UINT32 maxLen, int isPKCS5, int type)
{
    ...
    if (type == JOYLINK_ENC2_DECRYPT)
    {
        ...
        ret = mbedtls_aes_crypt_cbc(&ctx, type, (size_t)(*len), iv, data, data);
        dl = joylinkUnPadding(data, (UINT32)(*len), isPKCS5);
        if (dl < 0)
            return dl;
    }
    else
    {
        ...
        ret = mbedtls_aes_crypt_cbc(&ctx, type, (size_t)dl, iv, data, data);
    }
