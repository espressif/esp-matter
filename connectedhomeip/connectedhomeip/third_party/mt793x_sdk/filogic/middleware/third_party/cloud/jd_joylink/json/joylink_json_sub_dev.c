...
joylink_parse_sub_add(const uint8_t* pMsg, int* out_num)
{
    ...
    JLDevInfo_t * devs = (JLDevInfo_t*)joylink_malloc(sizeof(JLDevInfo_t) * JL_SUB_DEV_MAX);
    if(NULL == devs){
        return ret;
    }
...
joylink_package_subdev(JLDevInfo_t *sdev, int count)
{
    ...
    cJSON **js_devs = (cJSON **)joylink_malloc(sizeof(cJSON *) * count); 
    if(js_devs == NULL){
        goto RET;
    }
    if(NULL == (arrary = cJSON_CreateArray())){
        joylink_free(js_devs);
        goto RET;
    }
