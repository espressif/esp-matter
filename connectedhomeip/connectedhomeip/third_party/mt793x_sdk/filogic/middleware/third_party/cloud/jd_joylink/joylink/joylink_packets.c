...
joylink_packet_lan_scan_rsp(DevScan_t *scan)
{
    ...
    if(scan_rsp_data != NULL){
        ...
        if(_g_pdev->payload_total < JL_MAX_PAYLOAD_LEN){
            ...
            _g_pdev->send_p = (char *)joylink_malloc(_g_pdev->payload_total + JL_MAX_AES_EXTERN);
        ...
        if(NULL != _g_pdev->send_p)
            joylink_free(_g_pdev->send_p);
    }

RET:
    if(scan_rsp_data != NULL){
        joylink_free(scan_rsp_data);
    }
...
// mtk54101 modify : slim, heap instead
/*
int
...
{    
    ...
    if(ota_upload_rsp_data != NULL){
        joylink_free(ota_upload_rsp_data);
...
joylink_packet_server_ota_order_rsp(JLOtaOrder_t *otaOrder)
{
    ...
    uint8_t *dev_detail;    
    dev_detail = (char *)joylink_malloc(JL_MAX_PACKET_LEN);  
    bzero(dev_detail, JL_MAX_PACKET_LEN);

    memcpy((char*)dev_detail, &tt, 4);
    
    sprintf(data, "{\"code\":0,\"serial\":%d,\"msg\":\"success\"}", otaOrder->serial);
    memcpy((char*)dev_detail + 4, data, strlen(data));

    len = joylink_encypt_server_rsp(_g_pdev->send_buff, JL_MAX_PACKET_LEN,
            PT_OTA_ORDER, (uint8_t*)_g_pdev->jlp.sessionKey, dev_detail, strlen(data) + 4);

    joylink_free(dev_detail);
...
{    
    ...
    memcpy((char *)dev_detail, &tt, 4);
    if(ota_upload_rsp_data != NULL){
        memcpy((char *)dev_detail + 4, ota_upload_rsp_data, strlen(ota_upload_rsp_data));
        log_debug("new ota upload rsp data:%s", (char *)dev_detail + 4);
    }

    int len = joylink_encypt_server_rsp(_g_pdev->send_buff, JL_MAX_PACKET_LEN, 
        PT_OTA_UPLOAD, (uint8_t*)_g_pdev->jlp.sessionKey, 
        dev_detail, strlen(ota_upload_rsp_data) + 4);
