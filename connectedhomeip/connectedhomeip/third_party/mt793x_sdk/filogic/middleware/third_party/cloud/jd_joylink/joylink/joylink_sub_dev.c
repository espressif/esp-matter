...
joylink_packet_server_sub_hb_req(void)
{
    ...
    rsp_data = (char *)joylink_malloc(len);
    if(NULL == rsp_data){
        goto RET;
    }
...
joylink_subdev_script_ctrl(uint8_t* recPainText, JLSubContrl_t *ctr, unsigned short payloadlen)
{
    ...
    switch(ctr->biz_code){
        ...
        case JL_BZCODE_CTRL:
            ...
            if(ret > 0){
                ctr->cmd = (unsigned char*)joylink_malloc(sizeof(unsigned char) * (ret + 1));
                bzero(ctr->cmd, (ret + 1));
...
joylink_subdev_unbind(uint8_t* recPainText, JLSubUnbind_t* unbind)
{
    ...
    if(unbind->biz_code == JL_BZCODE_UNBIND){
        unbind->info = (JLSubDevInfo_t*)joylink_malloc(sizeof(JLSubDevInfo_t) * (unbind->num));
