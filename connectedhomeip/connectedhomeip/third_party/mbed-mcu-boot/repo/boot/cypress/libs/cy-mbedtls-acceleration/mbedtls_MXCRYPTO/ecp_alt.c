                         ...
                         mbedtls_ecp_restart_ctx *rs_ctx )
{
    ...
    cy_en_crypto_ecc_curve_id_t curveId = cy_get_dp_idx(grp->id);
    ...
    MBEDTLS_MPI_CHK( mbedtls_mpi_grow(&Pi.X, data_size) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_grow(&Pi.Y, data_size) );
