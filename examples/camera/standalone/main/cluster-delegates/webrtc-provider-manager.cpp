...
CHIP_ERROR
WebRTCProviderManager::HandleSolicitOffer(const OfferRequestArgs &args, WebRTCSessionStruct &outSession,
                                          bool &outDeferredOffer)
{
    ...
    if (mWebrtcTransportMap.size() > kMaxConcurrentWebRTCSessions) {
        ChipLogProgress(Camera, "Resource exhaustion detected: maximum WebRTC sessions (%u)",
...
WebRTCProviderManager::HandleProvideOffer(const ProvideOfferRequestArgs &args, WebRTCSessionStruct &outSession)
{
    ...
    WebrtcTransport *transport = GetTransport(args.sessionId);
    if (transport == nullptr) {
        // New session. Check resource availability before allocating; if we
        // cannot allocate resources, respond with a response status of
        // RESOURCE_EXHAUSTED without creating a session entry.
        if (mWebrtcTransportMap.size() >= kMaxConcurrentWebRTCSessions) {
            ChipLogProgress(Camera,
                            "Resource exhaustion detected in ProvideOffer: maximum "
                            "WebRTC sessions (%u) reached",
                            kMaxConcurrentWebRTCSessions);
...
CHIP_ERROR
WebRTCProviderManager::HandleProvideAnswer(uint16_t sessionId, const std::string &sdpAnswer)
{
    ...
    if (transport->GetState() != WebrtcTransport::State::SendingOffer &&
        transport->GetState() != WebrtcTransport::State::Idle) {
...
CHIP_ERROR WebRTCProviderManager::HandleProvideICECandidates(uint16_t sessionId,
...
CHIP_ERROR WebRTCProviderManager::HandleEndSession(uint16_t sessionId, WebRTCEndReasonEnum reasonCode)
{
    ...
    VerifyOrReturnError(transport != nullptr, CHIP_ERROR_INCORRECT_STATE,
                        ChipLogError(Camera, "Session ID %u does not match the current sessions", sessionId));
...
CHIP_ERROR WebRTCProviderManager::ValidateSFrameConfig(uint16_t cipherSuite, size_t baseKeyLength)
{
    ...
    if (baseKeyLength != expectedKeyLength) {
        ChipLogError(
            Camera, "SFrame base key length mismatch - expected %u bytes for cipher suite 0x%04X, got %u bytes",
...
CHIP_ERROR WebRTCProviderManager::IsUTCTimeNull(bool &isNull)
{
    ...
    //  - Read the UTCTime attribute from the Time Synchronization cluster
    //  (0x0038)
...
void WebRTCProviderManager::ScheduleICECandidatesSend(uint16_t sessionId)
{
    ...
    DeviceLayer::SystemLayer().ScheduleLambda([this, sessionId]() {
        ...
        WebrtcTransport::RequestArgs requestArgs = transport->GetRequestArgs();
        chip::ScopedNodeId peerId = requestArgs.peerId;
        ChipLogProgress(Camera, "Sending ICECandidates command to node " ChipLogFormatX64,
                        ChipLogValueX64(peerId.GetNodeId()));
...
CHIP_ERROR
WebRTCProviderManager::SendOfferCommand(Messaging::ExchangeManager &exchangeMgr, const SessionHandle &sessionHandle,
                                        uint16_t sessionId)
{
    ...
    return Controller::InvokeCommandRequest(&exchangeMgr, sessionHandle, args.originatingEndpointId, command, onSuccess,
                                            onFailure,
...
{
    ChipLogProgress(Camera, "Connection state changed for session %u: %s", sessionId,
...
CHIP_ERROR WebRTCProviderManager::SendAnswerCommand(Messaging::ExchangeManager &exchangeMgr,
                                                    const SessionHandle &sessionHandle, uint16_t sessionId)
{
    ...
    WebrtcTransport::RequestArgs requestArgs = transport->GetRequestArgs();
    // Now invoke the command using the found session handle
    return Controller::InvokeCommandRequest(&exchangeMgr, sessionHandle, requestArgs.originatingEndpointId, command,
                                            onSuccess, onFailure,
                                            /* timedInvokeTimeoutMs = */ NullOptional,
                                            /* responseTimeout = */ NullOptional,
                                            /* outCancelFn = */ nullptr, /*allowLargePayload = */ true);
...
CHIP_ERROR WebRTCProviderManager::SendEndCommand(Messaging::ExchangeManager &exchangeMgr,
                                                 const SessionHandle &sessionHandle, uint16_t sessionId,
                                                 WebRTCEndReasonEnum endReason)
{
    auto onSuccess = [](const ConcreteCommandPath &commandPath, const StatusIB &status, const auto &dataResponse) {
