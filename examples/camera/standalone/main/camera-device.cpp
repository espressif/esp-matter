...
// Default JPEG quality (1-100) used for CaptureSnapshot requests.
static constexpr uint8_t kSnapshotQuality = 50;
// Maximum time to wait for the media layer to produce a snapshot frame.
static constexpr uint32_t kSnapshotTimeoutMs = 5000;
...
// closest possible
bool CameraDevice::MatchClosestSnapshotParams(const VideoResolutionStruct &requested,
                                          ...
                                          const VideoResolutionStruct &resolution, ImageSnapshot &outImageSnapshot)
{
    ...
    ESP_LOGI("CameraDevice::CaptureSnapshot", "Capturing JPEG snapshot %ux%u (quality=%u)...", matchedRes.width,
             matchedRes.height, kSnapshotQuality);

    // Snapshot streams are always JPEG on this device.
    if (matchedCodec != ImageCodecEnum::kJpeg) {
        ChipLogError(Camera, "Unsupported snapshot codec %d; only JPEG is supported", static_cast<int>(matchedCodec));
        return CameraError::ERROR_CAPTURE_SNAPSHOT_FAILED;
    }

    // Grab a JPEG frame directly from the local camera pipeline. This handles all
    // camera states internally (H264 streaming active, idle-but-initialized, or
    // uninitialized) and downscales to the matched resolution when possible.
    uint8_t *jpeg_buf = nullptr;
    size_t jpeg_len = 0;
    esp_err_t err = video_capture_get_snapshot_scaled(&jpeg_buf, &jpeg_len, kSnapshotQuality, matchedRes.width,
                                                      matchedRes.height, kSnapshotTimeoutMs);

    if (err != ESP_OK || jpeg_buf == nullptr || jpeg_len == 0) {
        ESP_LOGE("CameraDevice::CaptureSnapshot", "Snapshot failed: status=%s, buf=%p, len=%zu", esp_err_to_name(err),
                 jpeg_buf, jpeg_len);
        if (jpeg_buf) {
    ...
    // Copy the JPEG data into the output, then release the source buffer.
    outImageSnapshot.data.assign(jpeg_buf, jpeg_buf + jpeg_len);
    video_capture_snapshot_free(jpeg_buf);

    outImageSnapshot.imageRes = matchedRes;
    outImageSnapshot.imageCodec = matchedCodec;

    ESP_LOGI("CameraDevice::CaptureSnapshot", "Snapshot captured successfully: %zu bytes", jpeg_len);
    return CameraError::SUCCESS;
}

// Allocate snapshot stream
CameraError CameraDevice::AllocateSnapshotStream(
    const CameraAVStreamManagementDelegate::SnapshotStreamAllocateArgs &args, uint16_t &outStreamID)
{
    if (AddSnapshotStream(args, outStreamID)) {
        auto it =
            std::find_if(mSnapshotStreams.begin(), mSnapshotStreams.end(), [outStreamID](const SnapshotStream &s) {
...
{
    static VideoSensorParamsStruct videoSensorParams = {kVideoSensorWidthPixels, kVideoSensorHeightPixels,
                                                        kMaxVideoFrameRate,
...
AudioCapabilitiesStruct &CameraDevice::GetMicrophoneCapabilities()
{
    ...
    static AudioCapabilitiesStruct audioCapabilities = {
        kMicrophoneMaxChannelCount, chip::Span<AudioCodecEnum>(audioCodecs), chip::Span<uint32_t>(sampleRates),
        chip::Span<uint8_t>(bitDepths)};
...
AudioCapabilitiesStruct &CameraDevice::GetSpeakerCapabilities()
{
    ...
    static AudioCapabilitiesStruct speakerCapabilities = {
        kSpeakerMaxChannelCount, chip::Span<AudioCodecEnum>(audioCodecs), chip::Span<uint32_t>(sampleRates),
        chip::Span<uint8_t>(bitDepths)};
...
{
    static std::vector<StreamUsageEnum> supportedStreamUsage = {StreamUsageEnum::kLiveView,
                                                                StreamUsageEnum::kRecording};
                                      ...
                                      const chip::app::Clusters::Globals::Structs::ViewportStruct::Type &viewport)
{
    ...
    ChipLogDetail(Camera, "New viewport. x1=%d, x2=%d, y1=%d, y2=%d.", viewport.x1, viewport.x2, viewport.y1,
                  viewport.y2);
...
void CameraDevice::InitializeAudioStreams()
{
    ...
    AudioStream monoStream = {{
                                  ...
                                  1 /* ChannelCount: Mono */, 48000 /* SampleRate */, 20000 /* BitRate */,
    ...
    AudioStream stereoStream = {{
                                    ...
                                    2 /* ChannelCount: Stereo */, 48000 /* SampleRate */, 32000 /* BitRate */,
                                    24 /* BitDepth */, 0 /* RefCount */
                                },
                                false,
                                nullptr};
...
bool CameraDevice::AddSnapshotStream(
    const CameraAVStreamManagementDelegate::SnapshotStreamAllocateArgs &snapshotStreamAllocateArgs,
    uint16_t &outStreamID)
{
    ...
    if (outStreamID == kInvalidStreamID) {
        ...
        for (uint16_t attempts = 0; attempts < kMaxSnapshotStreams; ++attempts) {
            auto found =
                std::find_if(mSnapshotStreams.begin(), mSnapshotStreams.end(), [streamId](const SnapshotStream &s) {
                    return s.snapshotStreamParams.snapshotStreamID == streamId;
                });
