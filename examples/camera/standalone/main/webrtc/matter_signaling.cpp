...
static esp_err_t init_webrtc_callback_task(void)
{
    ...
    BaseType_t ret = xTaskCreateWithCaps(webrtc_callback_task, "webrtc_cb_task",
                                         24 * 1024, // 16KB stack
                                         NULL,
                                         5, // Priority
                                         &g_webrtc_callback_task, MALLOC_CAP_SPIRAM);
...
static WEBRTC_STATUS matterSendMessage(void *pSignalingClient, webrtc_message_t *pMessage)
{
    ...
    ESP_LOGI(TAG, "matterSendMessage: initialized=%d, state=%d", client_data->initialized, client_data->state);
    ...
    ESP_LOGI(TAG, "Routing KVS message type=%d for sessionId=%u via Matter transport", pMessage->message_type,
             sessionId);
 ...
 */
static WEBRTC_STATUS matterSetCallbacks(void *pSignalingClient, uint64_t customData,
                                        WEBRTC_STATUS (*on_msg_received)(uint64_t, webrtc_message_t *),
                                        WEBRTC_STATUS (*on_signaling_state_changed)(uint64_t, webrtc_signaling_state_t),
                                        WEBRTC_STATUS (*on_error)(uint64_t, WEBRTC_STATUS, char *, uint32_t))
{
 ...
 */
static WEBRTC_STATUS matterSetIceUpdateCallback(void *pSignalingClient, uint64_t customData,
                                                WEBRTC_STATUS (*on_ice_servers_updated)(uint64_t, uint32_t))
{
    if (pSignalingClient == NULL) {
        return WEBRTC_STATUS_NULL_ARG;
 ...
 */
extern "C" WEBRTC_STATUS matter_signaling_handle_ice_candidate(uint16_t sessionId, const char *candidate,
                                                               const char *mid)
{
