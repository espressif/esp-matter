...
{
    // Only logging the track addition here as it's not used in the current implementation. In future, we can add
    // functionality to handle
    ChipLogProgress(Camera, "Remote track added for the sessionID: %u, type: %s", mRequestArgs.sessionId,
                    track->GetType().c_str());
}