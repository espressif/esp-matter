...
extern "C" void app_main()
{
    ...
    cam_config.camera_av_stream_management.feature_flags =
        cluster::camera_av_stream_management::feature::audio::get_id() |
        cluster::camera_av_stream_management::feature::video::get_id() |
        cluster::camera_av_stream_management::feature::snapshot::get_id();

    endpoint_t *endpoint = camera::create(node, &cam_config, ENDPOINT_FLAG_NONE, NULL);
    ABORT_APP_ON_FAILURE(endpoint != nullptr, ESP_LOGE(TAG, "Failed to create camera endpoint"));

#if CONFIG_ENABLE_SNTP_TIME_SYNC

    cluster::time_synchronization::feature::time_zone::config_t tz_cfg;
    cluster_t *time_sync_cluster = cluster::get(endpoint, TimeSynchronization::Id);
    ...
    if (avsm_err != CHIP_NO_ERROR) {
        ChipLogError(Camera, "Failed to initialize CameraAVStreamManagementServer on endpoint %u: %" CHIP_ERROR_FORMAT,
                     camera_endpoint_id, avsm_err.Format());
    }

    WebRTCTransportProvider::WebRTCTransportProviderCluster *server =
        WebRTCTransportProvider::GetServer(camera_endpoint_id);
    if (server == nullptr) {
