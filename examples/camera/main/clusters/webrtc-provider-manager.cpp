...
// Constants
constexpr uint16_t kMaxConcurrentWebRTCSessions = 3;

} // namespace

void WebRTCProviderManager::SetCameraDevice(
...
void WebRTCProviderManager::Init() {
  ...
  webrtc_bridge_register_handler(
      (webrtc_bridge_msg_cb_t)&webrtc_bridge_message_received_cb);
}
...
void WebRTCProviderManager::SetWebRTCTransportProvider(
    std::unique_ptr<WebRTCTransportProviderServer> aWebRTCTransportProvider) {
  mWebRTCTransportProvider = std::move(aWebRTCTransportProvider);
}

CHIP_ERROR
WebRTCProviderManager::HandleSolicitOffer(const OfferRequestArgs &args,
  ...
  if (transport == nullptr) {
    mWebrtcTransportMap[args.sessionId] =
        std::unique_ptr<WebrtcTransport>(new WebrtcTransport());
    mSessionIdMap[ScopedNodeId(args.peerNodeId, args.fabricIndex)] =
        args.sessionId;
  ...
  if (mWebrtcTransportMap.size() > kMaxConcurrentWebRTCSessions) {
    ChipLogProgress(
        Camera, "Resource exhaustion detected: maximum WebRTC sessions (%u)",
...
void WebRTCProviderManager::UnregisterWebrtcTransport(uint16_t sessionId) {
  ChipLogProgress(Camera, "UnregisterWebrtcTransport called for sessionId: %u",
                  sessionId);
  ...
  ChipLogProgress(Camera,
                  "Successfully unregistered transport for sessionId: %u",
                  sessionId);
}

CHIP_ERROR
WebRTCProviderManager::HandleProvideOffer(const ProvideOfferRequestArgs &args,
                                          WebRTCSessionStruct &outSession) {
  ...
  if (transport->GetPeerConnection()) {
    transport->GetPeerConnection()->SetRemoteDescription(args.sdp,
                                                         SDPType::Offer);
...
CHIP_ERROR
WebRTCProviderManager::HandleProvideAnswer(uint16_t sessionId,
                                           const std::string &sdpAnswer) {
  ChipLogProgress(Camera, "HandleProvideAnswer called with sessionId: %u",
                  sessionId);

  // Check if the provided sessionId matches your current sessions
  WebrtcTransport *transport = GetTransport(sessionId);
  if (transport == nullptr) {
    ChipLogError(Camera, "Session ID %u does not match the current sessions",
                 sessionId);
    return CHIP_ERROR_INVALID_ARGUMENT;
  }

  if (sdpAnswer.empty()) {
    ChipLogError(Camera, "Provided SDP Answer is empty for session ID %u",
                 sessionId);
    return CHIP_ERROR_INVALID_ARGUMENT;
  }
  ...
  transport->GetPeerConnection()->SetRemoteDescription(sdpAnswer,
                                                       SDPType::Answer);
...
CHIP_ERROR WebRTCProviderManager::HandleProvideICECandidates(
    uint16_t sessionId, const std::vector<ICECandidateStruct> &candidates) {
  ChipLogProgress(Camera,
                  "HandleProvideICECandidates called with sessionId: %u",
                  sessionId);
  ...
  if (candidates.empty()) {
    ChipLogError(
        Camera, "Candidate list is empty. At least one candidate is expected.");
    return CHIP_ERROR_INVALID_ARGUMENT;
  }

  for (const auto &candidate : candidates) {
    ChipLogProgress(
        Camera, "Applying candidate: %s",
        std::string(candidate.candidate.begin(), candidate.candidate.end())
            .c_str());
    std::string mid = candidate.SDPMid.IsNull()
                          ? ""
                          : std::string(candidate.SDPMid.Value().begin(),
                                        candidate.SDPMid.Value().end());
    transport->AddRemoteCandidate(
        std::string(candidate.candidate.begin(), candidate.candidate.end()),
        mid);
  }

  // Schedule sending Ice Candidates when remote candidates are received. This
  // keeps the exchange simple
  transport->MoveToState(WebrtcTransport::State::SendingICECandidates);
  ScheduleICECandidatesSend(sessionId);
...
CHIP_ERROR WebRTCProviderManager::HandleEndSession(
    uint16_t sessionId, WebRTCEndReasonEnum reasonCode,
  ...
  if (transport != nullptr) {
    ChipLogProgress(Camera, "Delete Webrtc Transport for the session: %u",
                    sessionId);
...
CHIP_ERROR
WebRTCProviderManager::ValidateStreamUsage(
  ...
  return avsmController.ValidateStreamUsage(streamUsage, videoStreamId,
                                            audioStreamId);
}
...
void WebRTCProviderManager::ScheduleOfferSend(uint16_t sessionId) {
  ...
  DeviceLayer::SystemLayer().ScheduleLambda([this, sessionId]() {
    ...
    WebrtcTransport::RequestArgs args = transport->GetRequestArgs();
    ChipLogProgress(Camera, "Sending Offer command to node " ChipLogFormatX64,
                    ChipLogValueX64(args.peerNodeId));
    ...
    caseSessionMgr->FindOrEstablishSession(
        args.peerId, &mOnConnectedCallback, &mOnConnectionFailureCallback,
...
void WebRTCProviderManager::ScheduleAnswerSend(uint16_t sessionId) {
  ...
  DeviceLayer::SystemLayer().ScheduleLambda([this, sessionId]() {
    ...
    ChipLogProgress(Camera, "Sending Answer command to node " ChipLogFormatX64,
    ...
    // WebRTC Answer requires a large payload session establishment.
    caseSessionMgr->FindOrEstablishSession(
        peerId, &mOnConnectedCallback, &mOnConnectionFailureCallback,
        TransportPayloadCapability::kLargePayload);
  });
}
...
void WebRTCProviderManager::ScheduleEndSend(uint16_t sessionId) {
  ...
  DeviceLayer::SystemLayer().ScheduleLambda([this, sessionId]() {
    ...
    WebrtcTransport::RequestArgs requestArgs = transport->GetRequestArgs();
    chip::ScopedNodeId peerId = requestArgs.peerId;
    ChipLogProgress(Camera, "Sending End command to node " ChipLogFormatX64,
                    ChipLogValueX64(peerId.GetNodeId()));
...
void WebRTCProviderManager::ScheduleICECandidatesSend(uint16_t sessionId) {
  ...
  DeviceLayer::SystemLayer().ScheduleLambda([this, sessionId]() {
    ...
    chip::ScopedNodeId peerId = requestArgs.peerId;
    ChipLogProgress(Camera,
                    "Sending ICECandidates command to node " ChipLogFormatX64,
                    ChipLogValueX64(peerId.GetNodeId()));
...
void WebRTCProviderManager::OnDeviceConnected(
    void *context, Messaging::ExchangeManager &exchangeMgr,
    const SessionHandle &sessionHandle) {
  WebRTCProviderManager *self =
      reinterpret_cast<WebRTCProviderManager *>(context);
  VerifyOrReturn(self != nullptr,
                 ChipLogError(Camera, "OnDeviceConnected:: context is null"));

  // Derive sessionId from sessionHandle by looking up the peer ScopedNodeId
  // (NodeId + FabricIndex)
  ...
  if (sessionIt == self->mSessionIdMap.end()) {
    ChipLogError(Camera,
                 ...
                 "[%d:" ChipLogFormatX64 "]",
                 peerScopedNodeId.GetFabricIndex(),
                 ChipLogValueX64(peerScopedNodeId.GetNodeId()));
    return;
  }
  ...
  if (transport == nullptr) {
    ChipLogError(Camera,
                 "OnDeviceConnected:: transport not found for sessionId: %u",
                 sessionId);
    return;
  }

  ChipLogProgress(Camera,
                  "CASE session established, sending command with Command "
                  "Type: %d, for sessionID: %u",
  ...
  case WebrtcTransport::CommandType::kEnd: {
    // Determine the end reason - check if it's due to privacy mode or resource
    // exhaustion
    ...
    err =
        self->SendEndCommand(exchangeMgr, sessionHandle, sessionId, endReason);
    // Release the Video and Audio Streams from the CameraAVStreamManagement
    // cluster and update the reference counts.
  ...
  if (err != CHIP_NO_ERROR) {
    ChipLogError(Camera,
                 "OnDeviceConnected::SendCommand failed: %" CHIP_ERROR_FORMAT,
                 err.Format());
  }
}

void WebRTCProviderManager::OnDeviceConnectionFailure(
    void *context, const ScopedNodeId &peerId, CHIP_ERROR err) {
  LogErrorOnFailure(err);
  WebRTCProviderManager *self =
      reinterpret_cast<WebRTCProviderManager *>(context);
  VerifyOrReturn(
      self != nullptr,
      ChipLogError(Camera, "OnDeviceConnectionFailure: context is null"));
}
...
void WebRTCProviderManager::LiveStreamPrivacyModeChanged(
...
CHIP_ERROR
WebRTCProviderManager::SendOfferCommand(Messaging::ExchangeManager &exchangeMgr,
                                        const SessionHandle &sessionHandle,
                                        uint16_t sessionId) {
  ...
  auto onFailure = [](CHIP_ERROR error) {
    ChipLogError(Camera, "Offer command failed: %" CHIP_ERROR_FORMAT,
                 error.Format());
  };
  ...
  if (transport == nullptr) {
    ChipLogError(
        Camera,
        "SendOfferCommand failed, WebTransport not found for sessionId: %u",
        sessionId);
  ...
  return Controller::InvokeCommandRequest(
      &exchangeMgr, sessionHandle, args.originatingEndpointId, command,
      onSuccess, onFailure,
...
void WebRTCProviderManager::OnLocalDescription(const std::string &sdp,
                                               SDPType type,
  ...
  if (state == WebrtcTransport::State::SendingAnswer &&
      type != SDPType::Answer) {
    return;
  }
...
void WebRTCProviderManager::OnConnectionStateChanged(bool connected,
                                                     const uint16_t sessionId) {
  ChipLogProgress(Camera, "Connection state changed for session %u: %s",
                  sessionId, connected ? "connected" : "disconnected");
  ...
  if (connected) {
    ...
    DeviceLayer::SystemLayer().ScheduleLambda([this, sessionId]() {
      ...
      ChipLogProgress(
          Camera,
          "Peer connection closed for session %u, cleaning up resources",
          sessionId);
...
CHIP_ERROR WebRTCProviderManager::SendAnswerCommand(
  ...
  auto onFailure = [](CHIP_ERROR error) {
    ChipLogError(Camera, "Answer command failed: %" CHIP_ERROR_FORMAT,
                 error.Format());
  };
  ...
  // Now invoke the command using the found session handle
  return Controller::InvokeCommandRequest(
      &exchangeMgr, sessionHandle, requestArgs.originatingEndpointId, command,
      onSuccess, onFailure,
      /* timedInvokeTimeoutMs = */ NullOptional,
      /* responseTimeout = */ NullOptional,
      /* outCancelFn = */ nullptr, /*allowLargePayload = */ true);
...
CHIP_ERROR WebRTCProviderManager::SendICECandidatesCommand(
    Messaging::ExchangeManager &exchangeMgr, const SessionHandle &sessionHandle,
    uint16_t sessionId) {
  auto onSuccess = [](const ConcreteCommandPath &commandPath,
                      const StatusIB &status, const auto &dataResponse) {
  ...
  auto onFailure = [](CHIP_ERROR error) {
    ChipLogError(Camera, "ICECandidates command failed: %" CHIP_ERROR_FORMAT,
                 error.Format());
  };

  WebrtcTransport *transport = GetTransport(sessionId);
  if (transport == nullptr) {
    ChipLogError(Camera, "WebTransport not found for the sessionId: %u",
                 sessionId);
    return CHIP_ERROR_INTERNAL;
  }
...
CHIP_ERROR WebRTCProviderManager::SendEndCommand(
    Messaging::ExchangeManager &exchangeMgr, const SessionHandle &sessionHandle,
  ...
  auto onFailure = [](CHIP_ERROR error) {
    ChipLogError(Camera, "End command failed: %" CHIP_ERROR_FORMAT,
                 error.Format());
  };
...
CHIP_ERROR WebRTCProviderManager::AcquireAudioVideoStreams(uint16_t sessionId) {
  ...
  WebrtcTransport::RequestArgs args = transport->GetRequestArgs();
  return mCameraDevice->GetCameraAVStreamMgmtDelegate()
      .OnTransportAcquireAudioVideoStreams(args.audioStreamId,
                                           args.videoStreamId);
}
...
CHIP_ERROR WebRTCProviderManager::ReleaseAudioVideoStreams(uint16_t sessionId) {
  ...
  return mCameraDevice->GetCameraAVStreamMgmtDelegate()
      .OnTransportReleaseAudioVideoStreams(args.audioStreamId,
                                           args.videoStreamId);
}
