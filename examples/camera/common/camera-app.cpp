...
{
    CameraAvStreamManagement::CameraAVStreamManagementCluster *avsm_server =
        CameraAvStreamManagement::GetServer(camera_endpoint_id);
    ...
    if (mCameraDevice->GetCameraHALInterface().HasSpeaker()) {
        ReturnErrorOnFailure(avsm_server->SetSpeakerMuted(mCameraDevice->GetCameraHALInterface().GetSpeakerMuted()));
        ReturnErrorOnFailure(
    ...
    if (mCameraDevice->GetCameraHALInterface().GetCameraSupportsImageControl()) {
        ReturnErrorOnFailure(avsm_server->SetImageRotation(mCameraDevice->GetCameraHALInterface().GetImageRotation()));
        ReturnErrorOnFailure(
...
void CameraApp::CreateAndInitializeCameraAVStreamMgmt(CameraAvStreamManagement::CameraAvStreamManagementConfig &config)
{
    ...
    if (mCameraDevice->GetCameraHALInterface().GetCameraSupportsNightVision()) {
        ...
        camera_av_stream_management::attribute::create_night_vision_illum(avsm_cluster,
                                                                          to_underlying(TriStateAutoEnum::kOn));
    }
...
void CameraAppInit(CameraDeviceInterface *cameraDevice,
                   CameraAvStreamManagement::CameraAvStreamManagementConfig &config)
{
