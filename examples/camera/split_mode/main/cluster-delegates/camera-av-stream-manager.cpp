                                                      ...
                                                      Optional<std::vector<uint16_t>> &audioStreams)
{
    ...
    // If stream id(s) are provided, it's sufficient to have verified that the provided usage is supported by the
    // camera. If the vector is empty, look for a stream ID that matches the usage. A match does not need to be exact.
    ...
    if (videoStreams.HasValue()) {
        ...
        if (videoStreams.Value().empty()) {
            ...
            // We've been provided with stream ID(s), and we know the stream usage is supported by the camera, classify
            // as an exact match
            exactlyMatchedVideoStream = true;
        }
    }
                                                   ...
                                                   uint32_t &outBandwidthbps)
{
    ...
    if (videoStreamId.HasValue() && !videoStreamId.Value().IsNull()) {
        ...
        for (const chip::app::Clusters::CameraAvStreamManagement::Structs::VideoStreamStruct::Type &stream :
             allocatedVideoStreams) {
            if (stream.videoStreamID == vStreamId) {
                outBandwidthbps += stream.maxBitRate;
                ChipLogProgress(Camera, "GetBandwidthForStreams: VideoStream %u maxBitRate: %lu bps", vStreamId,
                                stream.maxBitRate);
                break;
            }
        }
    ...
    if (audioStreamId.HasValue() && !audioStreamId.Value().IsNull()) {
        ...
        for (const chip::app::Clusters::CameraAvStreamManagement::Structs::AudioStreamStruct::Type &stream :
             allocatedAudioStreams) {
            if (stream.audioStreamID == aStreamId) {
                outBandwidthbps += stream.bitRate;
                ChipLogProgress(Camera, "GetBandwidthForStreams: AudioStream %u bitRate: %lu bps", aStreamId,
                                stream.bitRate);
                break;
            }
        }
                                                                               ...
                                                                               uint16_t &outStreamID)
{
    ...
    std::optional<uint16_t> reusableStreamId =
        GetCameraAVStreamManagementCluster()->GetReusableVideoStreamId(allocateArgs);
                                                   ...
                                                   StreamAllocationAction action)
{
    ...
    case StreamAllocationAction::kModification:
        ...
        for (VideoStream &stream : mCameraDeviceHAL->GetCameraHALInterface().GetAvailableVideoStreams()) {
            if (stream.videoStreamParams.videoStreamID == allocatedStream.videoStreamID && stream.isAllocated) {
                ...
                ChipLogProgress(Camera, "Restarting video stream with ID: %u due to modifications",
    ...
    case StreamAllocationAction::kReuse:
        ChipLogProgress(Camera, "Reusing existing video stream with ID: %u without changes",
                        allocatedStream.videoStreamID);
        break;
    }
}
...
Protocols::InteractionModel::Status CameraAVStreamManager::VideoStreamModify(
    const uint16_t streamID, const chip::Optional<bool> waterMarkEnabled, const chip::Optional<bool> osdEnabled)
{
    for (VideoStream &stream : mCameraDeviceHAL->GetCameraHALInterface().GetAvailableVideoStreams()) {
...
Protocols::InteractionModel::Status CameraAVStreamManager::SnapshotStreamAllocate(
    const SnapshotStreamAllocateArgs &allocateArgs, uint16_t &outStreamID)
{
    ...
    // Try to reuse an allocated stream.
    std::optional<uint16_t> reusableStreamId =
        GetCameraAVStreamManagementCluster()->GetReusableSnapshotStreamId(allocateArgs);
    ...
    if (mCameraDeviceHAL->GetCameraHALInterface().AllocateSnapshotStream(allocateArgs, outStreamID) ==
        CameraError::SUCCESS) {
        return Status::Success;
    }
...
void CameraAVStreamManager::OnAttributeChanged(AttributeId attributeId)
{
    ...
    case NightVision::Id: {
        mCameraDeviceHAL->GetCameraHALInterface().SetNightVision(
            GetCameraAVStreamManagementCluster()->GetNightVision());
        break;
    }
    ...
    case ImageRotation::Id: {
        mCameraDeviceHAL->GetCameraHALInterface().SetImageRotation(
            GetCameraAVStreamManagementCluster()->GetImageRotation());
        break;
    }
...
{
    if (mCameraDeviceHAL->GetCameraHALInterface().CaptureSnapshot(streamID, resolution, outImageSnapshot) ==
        CameraError::SUCCESS) {
        return Status::Success;
    }
...
CameraAVStreamManager::AllocatedSnapshotStreamsLoaded()
{
    ...
    for (auto &halStream : halStreams) {
        auto it = std::find_if(
            persistedStreams.begin(), persistedStreams.end(), [&](const SnapshotStreamStruct &persistedStream) {
    ...
    for (const auto &persistedStream : persistedStreams) {
        if (halStreamIds.find(persistedStream.snapshotStreamID) == halStreamIds.end()) {
            ...
            CameraError halErr =
                mCameraDeviceHAL->GetCameraHALInterface().AllocateSnapshotStream(snapshotStreamArgs, streamID);
                                                           ...
                                                           const std::vector<uint16_t> &videoStreams)
{
    ...
    for (uint16_t audioStreamID : audioStreams) {
        for (AudioStream &stream : mCameraDeviceHAL->GetCameraHALInterface().GetAvailableAudioStreams()) {
            if (stream.audioStreamParams.audioStreamID == audioStreamID && stream.isAllocated) {
                if (stream.audioStreamParams.referenceCount < UINT8_MAX) {
                    ...
                    ChipLogError(Camera, "Attempted to increment audio stream %u ref count beyond max limit",
                                 audioStreamID);
                }
                break;
            }
        ...
        if (GetCameraAVStreamManagementCluster()->UpdateAudioStreamRefCount(
    ...
    for (uint16_t videoStreamID : videoStreams) {
        for (VideoStream &stream : mCameraDeviceHAL->GetCameraHALInterface().GetAvailableVideoStreams()) {
            if (stream.videoStreamParams.videoStreamID == videoStreamID && stream.isAllocated) {
                if (stream.videoStreamParams.referenceCount < UINT8_MAX) {
                    ...
                    ChipLogError(Camera, "Attempted to increment video stream %u ref count beyond max limit",
                                 videoStreamID);
                }
                break;
            }
        ...
        // Update the counts in the SDK allocated stream attributes
        if (GetCameraAVStreamManagementCluster()->UpdateVideoStreamRefCount(
        ...
        for (AudioStream &stream : mCameraDeviceHAL->GetCameraHALInterface().GetAvailableAudioStreams()) {
            if (stream.audioStreamParams.audioStreamID == audioStreamID && stream.isAllocated) {
                if (stream.audioStreamParams.referenceCount > 0) {
                    ...
                    ChipLogError(Camera, "Attempted to decrement audio stream %u ref count when it was already 0",
                                 audioStreamID);
                }
                break;
            }
        ...
        for (VideoStream &stream : mCameraDeviceHAL->GetCameraHALInterface().GetAvailableVideoStreams()) {
            if (stream.videoStreamParams.videoStreamID == videoStreamID && stream.isAllocated) {
                if (stream.videoStreamParams.referenceCount > 0) {
                    ...
                    ChipLogError(Camera, "Attempted to decrement video stream %u ref count when it was already 0",
                                 videoStreamID);
                }
                break;
            }
