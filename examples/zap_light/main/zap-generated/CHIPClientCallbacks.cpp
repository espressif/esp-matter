  ...
  if (CHIP_NO_ERROR != error) {                                                \
    ...
    if (onFailureCallback != nullptr) {                                        \
      Callback::Callback<DefaultFailureCallback> *cb =                         \
          Callback::Callback<DefaultFailureCallback>::FromCancelable(          \
              onFailureCallback);                                              \
      cb->mCall(cb->mContext,                                                  \
                static_cast<uint8_t>(EMBER_ZCL_STATUS_INVALID_VALUE));         \
    }                                                                          \
    return retval;                                                             \
  }
...
#define CHECK_MESSAGE_LENGTH_WITH_RETVAL(value, retval)                        \
  if (!CanCastTo<uint16_t>(value)) {                                           \
    ChipLogError(                                                              \
        Zcl, "CHECK_MESSAGE_LENGTH expects a uint16_t value, got: %d", value); \
    if (onFailureCallback != nullptr) {                                        \
      Callback::Callback<DefaultFailureCallback> *cb =                         \
          Callback::Callback<DefaultFailureCallback>::FromCancelable(          \
              onFailureCallback);                                              \
...
#define CHECK_MESSAGE_LENGTH(value)                                            \
  CHECK_MESSAGE_LENGTH_WITH_RETVAL(value, true)
#define CHECK_MESSAGE_LENGTH_VOID(value)                                       \
  CHECK_MESSAGE_LENGTH_WITH_RETVAL(value, )
...
#define GET_CLUSTER_RESPONSE_CALLBACKS(name)                                   \
  ...
  /* #6559: Currently, we only have one commands for the IMInvokeCommands and  \
   * to a device, so the seqNum is always set to 0. */                         \
  CHIP_ERROR err = gCallbacks.GetResponseCallback(                             \
      sourceIdentifier, 0, &onSuccessCallback, &onFailureCallback);            \
                                                                               \
  if (CHIP_NO_ERROR != err) {                                                  \
    if (onSuccessCallback == nullptr) {                                        \
      ChipLogDetail(Zcl, "%s: Missing success callback", name);                \
...
app::CHIPDeviceCallbacksMgr &gCallbacks =
    app::CHIPDeviceCallbacksMgr::GetInstance();

bool emberAfOtaSoftwareUpdateProviderClusterApplyUpdateResponseCallback(
    EndpointId endpoint, app::CommandSender *commandObj, uint8_t action,
  ...
  GET_CLUSTER_RESPONSE_CALLBACKS(
      "OtaSoftwareUpdateProviderClusterApplyUpdateResponseCallback");

  Callback::Callback<
      OtaSoftwareUpdateProviderClusterApplyUpdateResponseCallback> *cb =
      Callback::Callback<
          OtaSoftwareUpdateProviderClusterApplyUpdateResponseCallback>::
...
bool emberAfOtaSoftwareUpdateProviderClusterQueryImageResponseCallback(
    EndpointId endpoint, app::CommandSender *commandObj, uint8_t status,
  ...
  ChipLogProgress(Zcl, "  delayedActionTime: %" PRIu32 "", delayedActionTime);
  ChipLogProgress(Zcl, "  imageURI: %.*s", static_cast<int>(imageURI.size()),
                  imageURI.data());
  ChipLogProgress(Zcl, "  softwareVersion: %" PRIu32 "", softwareVersion);
  ChipLogProgress(Zcl, "  softwareVersionString: %.*s",
                  static_cast<int>(softwareVersionString.size()),
  ...
  ChipLogProgress(Zcl, "  metadataForRequestor: %zu",
                  metadataForRequestor.size());

  GET_CLUSTER_RESPONSE_CALLBACKS(
      "OtaSoftwareUpdateProviderClusterQueryImageResponseCallback");

  Callback::Callback<OtaSoftwareUpdateProviderClusterQueryImageResponseCallback>
      *cb = Callback::Callback<
          OtaSoftwareUpdateProviderClusterQueryImageResponseCallback>::
          FromCancelable(onSuccessCallback);
  cb->mCall(cb->mContext, status, delayedActionTime, imageURI, softwareVersion,
